...
@Service.NamedByType(Constraints.Long.Min.class)
@Service.Singleton
@Weight(Weighted.DEFAULT_WEIGHT - 30)
class LongMinValidatorProvider implements Validation.ConstraintValidatorProvider {
    @Override
    public Validation.ConstraintValidator create(TypeName type, Annotation constraintAnnotation) {
        long max = constraintAnnotation.longValue().orElse(Long.MIN_VALUE);
