 ...
 * http://localhost:7001/greet/greeting
 *
...
public class GreetResource {
    ...
    private GreetingMessage createResponse(final String who) {
        String msg = String
                .format("%s %s!", greetingProvider.getGreeting(), who);
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public GreetingMessage getMessage(@PathParam("name") final String name) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response updateGreeting(final GreetingMessage message) {
        if (message.getMessage() == null) {
            GreetingMessage entity =
                    new GreetingMessage("No greeting provided");
