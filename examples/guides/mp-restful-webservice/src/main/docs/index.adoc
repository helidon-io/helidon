...
. Write an application-scoped bean to hold and manage the current greeting.
. Write a JAX-RS resource to respond to the incoming requests.
. Write a main class to start the server.
...
----
include::../resources/META-INF/microprofile-config.properties[tag=configContent]
----
...
----
include::../resources/logging.properties
----

=== Create an "empty" `beans.xml` 
We need to make sure JAX-RS searches for beans and, in particular, our
resource serving our REST endpoings. 
[source,java]
...
=== Create a JAX-RS bean to hold the greeting message
The app contains a default greeting which the user can set via HTTP. 
...
system will inject the argument because the argument class `GreetingMessage` 
is defined as a JAX-RS bean. Further, because you defined it to have
application scope, JAX-RS will inject the same instance every time
it creates a new instance of the `GreetResource` so all will
...
<2> Retrieves the message from the `GreetingMessage` bean.
<3> Formats the response as JSON.

=== Add the code for the resource's endpoints

==== Write a method to return the default greeting

=== Add an `Application` class
JAX-RS looks for an `Application`. Create `GreetApplication.java` which
extends `javax.ws.rs.core.Application`, annotate the class with
`javax.ws.rs.core.Application` and `javax.enterprise.context.ApplicationScoped("/")`.
...
----
<1> Automatically reads server configuration from `microprofil-config.properties`
