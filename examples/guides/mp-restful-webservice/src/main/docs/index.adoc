...
and so the metrics
reported have the same values for this example and for the Helidon SE RESTful web 
service example. In a real application we would probably measure the `get` methods and
the `update` method separately.)
+
[source,java]
----
