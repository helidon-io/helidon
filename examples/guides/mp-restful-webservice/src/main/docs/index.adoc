...
Create `HealthResource.java` to define the endpoints for checking activeness and readiness.
You or your IDE will add these imports:
[source,java]
----
...
is the only input in our simple implementation to the activeness check.
+
[source,java]
----
...
=== Send some requests
Use the same `curl` commands from the beginning of the guide to send requests to
the server:
...
----
<1> Requests all application-scoped metrics.
...
----
Note that:
. The name of the counter is automatically qualified with the package and class name of our JAX-RS
resource (`io_helidon_guides_mp_restfulwebservice_greet_resource_accessctr`). If we
had used `absolute=true` in the `@Counted` annotation then the name would be
simply `accessctr`.
...
metric but all the _base_ and _vendor_ metrics. For example, you will see a `vendor:requests_count` 
counter. This will be larger than our counter because that counter also tracks 
requests to the `/metrics` path; our `accessctr` counter tracks only requests to
