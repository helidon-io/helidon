...
. Read the config file and set the initial greeting from it:
+
[source,java]
----
...
entry is missing.
+
. Implement the responses to the three types of request.
...
<2> Creates the JSON response by adding the message an building it.
...
. Add a method to set up routing for your app
[source,java]
----
...
----
    startServer();
----
is enough to get your app running.
