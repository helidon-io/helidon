...
class GeneratedMetadataTest {
    ...
    void testMetadata() throws IOException {
        ...
        for (JsonValue type : types) {
            ...
            if (jsonObject.getString("annotatedType").equals(MyTarget.class.getName())) {
                builder = jsonObject;
            } else if (jsonObject.getString("annotatedType").equals(MyAbstract.class.getName())) {
                abstractBuilder = jsonObject;
            }
        }
        assertThat("Type should be generated for MyTargetBlueprint", builder, notNullValue());
        assertThat("Type should be generated for MyAbstractBlueprint", abstractBuilder, notNullValue());

        // now we have both builder and abstract builder, we can validate fields
        validateMyTarget(builder);
        validateMyAbstract(abstractBuilder);
    }

    private void validateMyTarget(JsonObject builder) {
    ...
    private void validateMyAbstract(JsonObject abstractBuilder) {
        assertThat(abstractBuilder.getString("description"), is("abstract builder"));
        assertThat(abstractBuilder.getString("type"), is(MyAbstract.class.getName()));
        ...
        assertThat(option.getString("method"), is(MyAbstract.Builder.class.getCanonicalName() + "#abstractMessage(java.lang.String)"));
    }
}
