...
class ConfigProducerTest {
    ...
    void testConfig() {
        ...
        // the config source should be ignored, as we only load sources from config-profile.yaml
        ...
        assertThat(helidonConfig.get("app.value").asString().asOptional(), is(Optional.of("file")));
    }
    ...
    void testMetaConfigured() {
        ...
        assertThat(config.get("app.value2").asString().asOptional(), is(Optional.of("from-config-profile")));
    }
}
