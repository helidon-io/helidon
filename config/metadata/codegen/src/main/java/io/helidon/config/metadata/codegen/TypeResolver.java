...
class TypeResolver {
    ...
    TypeName superType(TypeInfo typeInfo, TypeName typeName) {
        for (var t : typeHierarchy(typeInfo)) {
            if (t.typeName().genericTypeName().equals(typeName)) {
                return t.typeName();
            }
        }
        return null;
    ...
    TypeInfo subtype(TypeInfo typeInfo, TypeName typeName) {
        var rawTypeName = typeName.genericTypeName();
        for (var superTypeInfo : typeHierarchy(typeInfo)) {
    ...
    boolean isSubtype(TypeInfo typeInfo, TypeName typeName) {
        return subtype(typeInfo, typeName) != null;
    }
}
