    ...
    static class Basic {
        ...
        static CrossOriginConfig.Builder applyConfig(CrossOriginConfig.Builder builder, Config config) {
            ...
            config.get("allow-origins")
                    .asList(String.class)
                    .ifPresent(
                            s -> builder.allowOrigins(CorsSupportHelper.parseHeader(s).toArray(new String[]{})));
            config.get("allow-methods")
                    .asList(String.class)
                    .ifPresent(
                            s -> builder.allowMethods(CorsSupportHelper.parseHeader(s).toArray(new String[]{})));
            config.get("allow-headers")
                    .asList(String.class)
                    .ifPresent(
                            s -> builder.allowHeaders(CorsSupportHelper.parseHeader(s).toArray(new String[]{})));
            config.get("expose-headers")
                    .asList(String.class)
                    .ifPresent(
                            s -> builder.exposeHeaders(CorsSupportHelper.parseHeader(s).toArray(new String[]{})));
