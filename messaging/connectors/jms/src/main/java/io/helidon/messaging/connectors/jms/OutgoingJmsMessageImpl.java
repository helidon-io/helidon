...
class OutgoingJmsMessageImpl<PAYLOAD> implements OutgoingJmsMessage<PAYLOAD> {
    ...
    private final List<PostProcessor> postProcessors = new ArrayList<>(2);

    OutgoingJmsMessageImpl(PAYLOAD payload) {
        this.payload = payload;
    }

    OutgoingJmsMessageImpl() {
    }
    ...
    void mapper(JmsMessage.CustomMapper<PAYLOAD> mapper) {
        this.mapper = mapper;
    }
    ...
    static <PAYLOAD> OutgoingJmsMessageImpl<PAYLOAD> fromJmsMessage(javax.jms.Message jmsMessage) throws JMSException {
        OutgoingJmsMessageImpl<PAYLOAD> msg = new OutgoingJmsMessageImpl<>();
        msg.postProcessors().add(m -> {
