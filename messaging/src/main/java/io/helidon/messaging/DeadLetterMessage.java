...
public interface DeadLetterMessage<T> extends Message<T> {
     ...
     * @throws NullPointerException if {@code originalMessage}, {@code sourceChannel}, or {@code failure} is
     *                              {@code null}
     * @throws IllegalArgumentException if {@code sourceChannel} is blank or {@code attempts} is less than one
     */
    static <T> DeadLetterMessage<T> create(Message<T> originalMessage,
                                           String sourceChannel,
                                           int attempts,
                                           RuntimeException failure) {
        Message<T> actualMessage = Objects.requireNonNull(originalMessage, "originalMessage");
        String actualChannel = Objects.requireNonNull(sourceChannel, "sourceChannel");
        RuntimeException actualFailure = Objects.requireNonNull(failure, "failure");
        return new DefaultDeadLetterMessage<>(actualMessage, actualChannel, attempts, actualFailure);
    }
