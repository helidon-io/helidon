...
class BatchDefaultsTest {
    ...
    @Test
    void messagingRuntimeRejectsNullSingleMessageArguments() {
        AtomicReference<MessageBatch<?>> received = new AtomicReference<>();
        MessagingRuntime runtime = new MessagingRuntime() {
            @Override
            public <T> void emitBatch(String channel, MessageBatch<? extends T> batch) {
        ...
        assertThrows(NullPointerException.class, () -> runtime.emit(null, message));
        assertThrows(NullPointerException.class, () -> runtime.emit("orders", nullMessage));
