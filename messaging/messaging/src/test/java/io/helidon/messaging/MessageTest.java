...
class MessageTest {
    ...
    void supportsStandardBuilderShape() {
        ...
        assertThrows(NullPointerException.class, () -> Message.builder().build());
        assertThrows(NullPointerException.class, () -> Message.builder().entity(null));
    }
    ...
    void replacesTextHeadersAndSnapshotsBuilderState() {
        ...
        assertThat(message.headerValue("trace").orElseThrow(), is(MessageHeaderValue.TextValue.create("last")));
    }
    ...
    void appendsOrderedDuplicateTypedHeaders() {
        Message<String> message = Message.builder("payload")
                .addHeader("a", "first")
                .addHeader("b", MessageHeaderValue.BooleanValue.create(true))
                .addHeader(MessageHeader.create("a", MessageHeaderValue.BinaryValue.create(new byte[] {1, 2})))
                .build();

        assertThat(message.headers().entries(),
                   is(List.of(MessageHeader.create("a", "first"),
                              MessageHeader.create("b", MessageHeaderValue.BooleanValue.create(true)),
                              MessageHeader.create("a", MessageHeaderValue.BinaryValue.create(new byte[] {1, 2})))));
        assertThat(message.headerValue("a").orElseThrow(),
                   is(MessageHeaderValue.BinaryValue.create(new byte[] {1, 2})));
    ...
    void defaultsSnapshotsAndReplacesLocalMetadata() {
        ...
        assertThat(snapshot.localMetadata().values(),
                   is(Map.of("diagnostic", MessageHeaderValue.TextValue.create("first"),
                             "typed", MessageHeaderValue.IntegerValue.create(42))));
