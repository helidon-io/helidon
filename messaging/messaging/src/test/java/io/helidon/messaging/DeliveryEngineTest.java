...
class DeliveryEngineTest {
    ...
    void reservationTryStartFailedRetainsPendingCapacityUntilInFlightIsAvailable() throws Exception {
        ...
        try (DeliveryEngine engine = engine(config, "orders")) {
            ...
            assertThat(reservation.tryStartFailed(batch, mappingFailure).isEmpty(), is(true));
            assertThat(engine.tryReserveConnectorDelivery("orders", 1).isEmpty(), is(true));

            releaseActive.countDown();
            await(active);
            try (ConnectorDelivery delivery = reservation.tryStartFailed(batch, mappingFailure).orElseThrow()) {
                IllegalStateException result = assertThrows(IllegalStateException.class, delivery::await);
                assertThat(result, sameInstance(mappingFailure));
            }
        }
    }
    ...
    void reservationTryStartFailedNullFailureReleasesPendingCapacity() {
        ...
        try (DeliveryEngine engine = engine(config, "orders")) {
            ...
            assertThrows(NullPointerException.class, () -> reservation.tryStartFailed(batch, null));

            ConnectorDeliveryReservation recovered = engine.tryReserveConnectorDelivery("orders", 1)
                    .orElseThrow();
            recovered.close();
            MessagingRejectedException cancelled = assertThrows(MessagingRejectedException.class,
    ...
    void reservationTryStartFailedNullFailureCannotCancelConcurrentStart() throws Exception {
        ...
        try (DeliveryEngine engine = engine(config, "orders")) {
            ...
            assertThrows(IllegalStateException.class,
                         () -> reservation.tryStartFailed(batch(List.of(message(3))), null));
        ...
        try (DeliveryEngine engine = engine(config, "a", "b")) {
            ...
            Runnable aToB = () -> {
                ...
                try {
                    ...
                    throw new AssertionError("nested delivery unexpectedly ran");
    ...
    private static Optional<ConnectorDelivery> trySubmitConnectorDelivery(
    ...
    private static Optional<ConnectorDelivery> tryStart(ConnectorDeliveryReservation reservation,
                                                        List<? extends Message<?>> messages,
                                                        Runnable action) {
        Optional<ConnectorDelivery> delivery = reservation.tryStart(batch(messages));
        delivery.ifPresent(_ -> action.run());
        return delivery;
    }
        ...
        try (DeliveryEngine engine = engine(config, "orders")) {
            ...
            AsyncTask waiter = async(() -> {
                try {
                    ...
                    throw new AssertionError("interrupted connector wait returned normally");
    ...
    private static void awaitInterruption(CountDownLatch interrupted) {
        try {
            ...
            throw new AssertionError("blocking task unexpectedly resumed");
        } catch (InterruptedException e) {
