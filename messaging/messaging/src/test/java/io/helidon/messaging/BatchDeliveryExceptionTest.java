...
class BatchDeliveryExceptionTest {
    @Test
    void validatesBatchItemOutcomeFailureCauses() {
        BatchItemOutcome failed = BatchItemOutcome.failed(0);
        BatchItemOutcome indeterminate = BatchItemOutcome.indeterminate(1);

        assertThat(failed.failure().isEmpty(), is(true));
        assertThat(indeterminate.failure().isEmpty(), is(true));
        assertThrows(NullPointerException.class, () -> BatchItemOutcome.failed(0, null));
        assertThrows(NullPointerException.class, () -> BatchItemOutcome.indeterminate(0, null));
    }
