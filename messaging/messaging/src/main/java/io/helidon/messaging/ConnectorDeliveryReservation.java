 ...
 * delivery from its transport. The actual delivery supplied to {@link #start(MessageBatch)},
 * {@link #tryStart(MessageBatch)}, {@link #startFailed(MessageBatch, RuntimeException)}, or
 * {@link #tryStartFailed(MessageBatch, RuntimeException)} must not exceed the reserved message count. Starting atomically
...
public interface ConnectorDeliveryReservation extends AutoCloseable {
    ...
    default ConnectorDelivery startFailed(MessageBatch<?> batch, RuntimeException failure) {
        throw unsupportedFailedStart(batch, failure);
    }
     ...
     * <p>
     * Runtime implementations apply the same failure-policy and structured-outcome semantics as
     * {@link #startFailed(MessageBatch, RuntimeException)}. Unlike that method, this operation returns empty when
     * in-flight capacity is currently unavailable, leaving the reservation available for another start attempt.
     * <p>
     * The default implementation closes this reservation and rethrows the supplied failure so implementations compiled
     * against an earlier version fail safely instead of dispatching an invalid batch.
     *
     * @param batch retained delivery metadata used for failure accounting, drop, or dead-letter handling
     * @param failure pre-dispatch mapping failure
     * @return admitted delivery task and settlement lease, or empty when in-flight capacity is currently unavailable
     * @throws RuntimeException the supplied failure when this operation is not implemented by the runtime
     * @throws MessagingRejectedException if the delivery exceeds this reservation or the reservation is unavailable
     * @throws IllegalStateException if this reservation was already started or another start is in progress
     */
    ...
    private RuntimeException unsupportedFailedStart(MessageBatch<?> batch, RuntimeException failure) {
