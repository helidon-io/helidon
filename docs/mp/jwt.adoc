...
Enable MP JWT authentication using `@LoginConfig(authMethod = "MP-JWT")`:
...
Authenticated applications must also configure `mp.jwt.verify.issuer` so
incoming tokens are accepted only from the expected issuer.
