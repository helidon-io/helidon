...
== Usage
A test can be annotated with `io.helidon.microprofile.tests.junit5.HelidonTest` to mark it as a
...
it is important to set property `mp.initializer.allow=true` in order for the CDI container to start, when used with
`@HelidonTest`.
