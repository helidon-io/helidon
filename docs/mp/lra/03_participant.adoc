...
* link:{microprofile-lra-javadoc-url}/ws/rs/LRA.html#timeLimit--[timeLimit] max time limit before LRA gets cancelled automatically by xref:02_coordinator.adoc[coordinator]
...
WARNING: Expected to be called by LRA xref:02_coordinator.adoc[coordinator] only!

Compensate method is called by xref:02_coordinator.adoc[coordinator] when xref:01_introduction.adoc[LRA] is cancelled,
...
If participant has <<status-participant-method,@Status annotated method], xref:02_coordinator.adoc[coordinator]
...
Complete method is called by xref:02_coordinator.adoc[coordinator] when LRA is successfully closed.
If the method responds with 500 or 202, coordinator will eventually try the call again.
If participant has <<status-participant-method,@Status annotated method], xref:02_coordinator.adoc[coordinator] retrieves the status to find out if retry should be done.
...
to xref:02_coordinator.adoc[coordinator].
When xref:02_coordinator.adoc[coordinator] decides all the participants have finished, method annotated with @Forget is called.
...
Method annotated with @Leave called with LRA context(with header link:{microprofile-lra-javadoc-url}/ws/rs/LRA.html#LRA_HTTP_CONTEXT_HEADER[LRA_HTTP_CONTEXT_HEADER]) informs xref:02_coordinator.adoc[coordinator] that current participant is leaving the LRA.
...
xref:02_coordinator.adoc[Coordinator] can call it and decide if compensate or complete retry is needed.

==== JAX-RS variant with supported LRA context values:

* Header link:{microprofile-lra-javadoc-url}/ws/rs/LRA.html#LRA_HTTP_CONTEXT_HEADER[LRA_HTTP_CONTEXT_HEADER] - id of the LRA transaction
* link:{microprofile-lra-javadoc-url}ParticipantStatus.html[ParticipantStatus] - Status of the participant reported to xref:02_coordinator.adoc[coordinator]
