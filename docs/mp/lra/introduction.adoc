...
Unlike well known XA protocol, LRA is asynchronous and therefore much more scalable. Every LRA JAX-RS resource (xref:participant.adoc[participant]) defines endpoints to be invoked when transaction needs to be xref:participant.adoc#complete-participant-method[completed] or xref:participant.adoc#compensate-participant-method[compensated].
