...
== {feature-name} (LRA)
Distributed transactions for microservices are known as SAGA design patterns and are defined by the {microprofile-lra-spec-url}[MicroProfile Long Running Actions specification].
Unlike well known XA protocol, LRA is asynchronous and therefore much more scalable. Every LRA JAX-RS resource (xref:03_participant.adoc[participant]) defines endpoints to be invoked when transaction needs to be xref:03_participant.adoc#complete-participant-method[completed] or xref:03_participant.adoc#compensate-participant-method[compensated].

LRA transactions need to be coordinated over REST API by the LRA coordinator.
xref:02_coordinator.adoc[Coordinator]
...
transaction. LRA transaction is first started, then joined by xref:03_participant.adoc[participant].
Participant reports the successful finish of transaction by calling complete. Coordinator then calls the JAX-RS
xref:03_participant.adoc#complete-participant-method[complete] endpoint that was registered during the join of each
xref:03_participant.adoc[participant]. As the completed or compensated participants don't have to be on same instance,
...
xref:02_coordinator.adoc[Coordinator] calls compensate on all the joined participants.
...
When startExample method finishes successfully, close is reported to xref:02_coordinator.adoc[coordinator]
