...
The gRPC Microprofile APIs are an extension to xref:../introduction/01_introduction.adoc[Helidon MP] to allow building
...
Protobuf serializable classes but this is not the case with Helidon gRPC. Helidon supports
 xref:../../se/grpc/09_marshalling.adoc[pluggable Marshallers]
...
xref:../../se/grpc/04_service_implementation.adoc[non-microprofile Helidon gRCP services].
...
the xref:../../se/grpc/03_routing.adoc[basic gRPC server documentation]).
