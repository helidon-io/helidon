...
- a unified way for MicroProfile servers to export monitoring data telemetry to
...
For your project to also emit the OpenTelemetry HTTP server semantic
conventions, add the following dependency:
...
To use an OTLP exporter, add its runtime implementation as well:
...
## OpenTelemetry Metrics HTTP Server Semantic Conventions

Your project can export OpenTelemetry metrics that comply with the OpenTelemetry
metrics semantic conventions.

First, include the additional dependency on
`helidon-webserver-observe-telemetry-metrics` described above.

Next, configure OpenTelemetry to transmit the metrics data from your service to
a backend using Helidon configuration. This step is necessary because Helidon
must use the OpenTelemetry metrics API, rather than the MicroProfile Metrics
API, for semantic-conventions metrics. The semantic conventions use the dot
(`.`) character in metric and tag names, which is not allowed in MicroProfile
Metrics.

The following example sends OTLP metrics data once a second to a Prometheus
server running locally at the configured endpoint. Start Prometheus with the
`--web.enable-otlp-receiver` option to enable its OTLP receiver.

> [!NOTE]
> Although these settings might be suitable for a test or demonstration, use
> different settings—certainly a longer interval—for a production deployment.
> See the [Helidon Telemetry documentation][helidon-telemetry-doc] for more
> information.

Example configuration for a test or demonstration of OpenTelemetry metrics
semantic conventions:

<!--@mdc ::code-callout -->
```yaml [application.yaml]
telemetry:                              # <1>
  service: mp-with-otel-sem-conv        # <2>
  signals:
    metrics:                            # <3>
      exporters:
        - type: otlp                    # <4>
...
```
1. Introduces telemetry-related settings.
2. Provides the required OTLP service name.
3. Introduces the OpenTelemetry metrics settings.
4. Configures an OTLP metrics exporter to transmit metrics data to a local
   Prometheus server.
5. Configures a metrics reader which gathers data periodically, once per the
   configured interval, and uses the configured exporter to transmit the
   gathered metrics data.
6. Enables the updated HTTP metrics semantic-conventions behavior. It defaults
   to `false` to preserve the output from earlier Helidon 4.x releases.
<!--@mdc :: -->
...
> [!NOTE]
> The `auto-http-metrics.use-updated-http-metrics` setting controls the updated
> OpenTelemetry HTTP metric behavior. It defaults to `false` to retain the
> earlier Helidon 4.x duration, response-status, and `http.route` output. Set it
> to `true` to use the updated behavior. With updated behavior, Helidon records
> `http.route` only when a non-blank matching route is available; for matched
> JAX-RS requests, the value is the JAX-RS route template.
...
server.features.observe.observers.metrics.auto-http-metrics.use-updated-http-metrics=true # <4>
```
...
measured.
4. Use the updated OpenTelemetry HTTP metric behavior.
<!--@mdc :: -->
...
```
1. This class is now annotated with `@Counted`, which aggregates count data from
   all the method that have a `Count` annotation.
...
[helidon-telemetry-doc]: ../../se/telemetry/opentelemetry.md
