...
* a unified Java API which all application programmers can use to expose telemetry data from their services.
...
You can add metrics to your service in these ways:
...
* Write code in your service which explicitly invokes the metrics API to register metrics, retrieve previously-registered metrics, and update metric values.
...
* xref:example-application-code[]
* xref:example-configuration[]
