...
| link:{jakarta-jsonp-spec-url}[{version-lib-jakarta-jsonp-api}]
...
| xref:{rootdir}/mp/jaxrs/server-configuration.adoc[Jakarta RESTful Web Services (JAX-RS)]
| link:{jakarta-jaxrs-spec-url}[{version-lib-jakarta-jaxrs-api}]
| API to develop web services following the REST pattern

| xref:{rootdir}/mp/jpa.adoc[Jakarta Persistence (JPA)]
...
| xref:{rootdir}/mp/lra/introduction.adoc[MicroProfile Long-Running Actions (LRA)]
...
| xref:{rootdir}/mp/metrics/introduction.adoc[MicroProfile Metrics]
| link:{microprofile-metrics-spec-url}[{version-lib-microprofile-metrics-api}]
| Telemetry data in Prometheus format
...
| Tracking service calls in distributed environment
...
| xref:{rootdir}/mp/grpc/mp-server-side-services.adoc[gRPC]
