...
Messages are managed by methods annotated by `@Incoming` and `@Outgoing`, invocation is always driven by messaging core either at assembly time or for every message coming from the stream.
