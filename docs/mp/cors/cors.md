...
1.  Identify the resources and sub-resources, in other words, the paths declared
    in the resource class which you want to support CORS.
...
Helidon MP aborts the server start-up if you use the `@Cors.*` annotations on a
resource method other than an `@OPTIONS` method.
...
```
1. You can disable CORS processing for all resources by setting `cors.enabled`
...
2. Sets up sharing for the `/greet/greeting` resource only via `PUT` requests
   and only from the specified origins.
<!--@mdc :: -->
...
```
1. Changes the declared settings to restrict cross-origin use of `/greet` to
   only `GET` and only from `foo.com` and `there.com`.
...
[io-helidon-webse]: ../../config/io.helidon.webserver.cors.CorsPathConfig.md#configuration-options
