...
Simply add the Helidon MP `@CrossOrigin` annotation to a particular method in your resource class to
allow Helidon MP to enforce the CORS protocol for all of that resource's endpoints.

== Understanding the `@CrossOrigin` Annotation
The preferred way to configure CORS in Helidon MP is to use the link:{javadoc-base-url-api}/CrossOrigin.html[`@CrossOrigin`] annotation.
