...
|`batch-size` | Size of the batch in which are messages being received before emitting to downstream.
Larger batch usually performs better but takes more memory. Default value: `10`
...
== Destination
When configuring destination https://docs.oracle.com/en/middleware/standalone/weblogic-server/14.1.1.0/jmspg/lookup.html#GUID-184B8BA2-C18C-41B7-B3D3-EFC6F47E6D7A[WebLogic CDI(Create Destination Identifier)] syntax needs to be applied.
...
- Direct connection to UDD member
When UDD member is named according to convention `jms-server-name@udd-name`.

image::msg/udd_members.png[UDD members]

Member can be accessed directly as non-distributed destination on particular JMS server:
