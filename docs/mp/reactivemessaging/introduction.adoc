...
== Overview
Reactive messaging is vastly different from the older method of using message-driven beans. One significant difference is that blocking is no longer the only way to apply backpressure to the message source.
...
**Caution**: Messaging methods are not meant to be invoked directly!

=== Emitter
To send messages from imperative code, we can inject a special channel source called an emitter.
