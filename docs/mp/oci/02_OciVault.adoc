...
=== Encryption
To encrypt a text, submitted with a GET request to `/encrypt` endpoint:
...
=== Decryption
To decrypt a text, submitted with a GET request to `/decrypt` endpoint:
...
=== Signature
To retrieve a signature for submitted text via GET request to `/sign` endpoint:
...
To verify the correctness of the signature to the submitted text via GET request to `/verify` endpoint:
...
To create a secret with a provided name, using GET request submitted to `/secret`
