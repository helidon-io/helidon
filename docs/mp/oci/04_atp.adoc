...
public class AtpResource {
    ...
    AtpResource(OciAutonomousDb autonomousDb, <1>
                ...
                @ConfigProperty(name = "oracle.ucp.jdbc.PoolDataSource.atp.tnsNetServiceName") String atpTnsNetServiceName) { <2>
...
<2> ATP ServiceName is read from config. This is `netServiceName` of your database instance running inside OCI ATP as can be found in `tnsnames.ora` file.
...
----
    ...
    public Response generateWallet() {
        ...
        try {
            ...
            this.atpDataSource.setURL(walletArchive.getJdbcUrl(this.atpTnsNetServiceName)); <3>
            try(
