...
9. Messaging processing method connecting together channels `test-channel-in`
   and `test-channel-out`
10. Actual JUnit 5 test method which is going to block the thread until 3 items
are intercepted on `test-channel-out` channel’s downstream and assert those with
expected values.
<!--@mdc :: -->
