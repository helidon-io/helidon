...
Helidon Microprofile Server is used to collect and deploy JAX-RS application. You can either instantiate the server directly
as is done in the xref:../guides/quickstart.adoc[Helidon MP Quickstart example]
or use its built-in `create`. It will create the `server` instance. Calling `start` method will run the server.
...
include::{rootdir}/includes/config/io.helidon.microprofile.server.Server.adoc[]
...
See <<Usage>> for same simple examples.
