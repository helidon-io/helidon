...
For example, if you wanted to run code when a CDI container starts, then you would have to write a particular kind of _observer method_ in an enabled CDI-managed bean that observes the initialization of the application scope, as shown in the example below:
...
Or, you can use `ServerCdiExtension` to handle bad requests for illegal characters in URI as shown in the example below:
