...
(Note that the Oracle Universal Connection Pool does not distinguish
...
property of the connection pool, and `user` is a property of the
vendor-supplied `DataSource` implementation.  In some cases, the
Oracle Universal Connection Pool will set the given property on _both_
the connection pool itself _and_ on the vendor-supplied `DataSource`
it manages.)

In general, the properties that can be set on the
`oracle.jdbc.pool.OracleDataSource` vendor-supplied `DataSource` can
...
way, regardless of your choices of vendor-supplied `DataSource` and
connection pool.
