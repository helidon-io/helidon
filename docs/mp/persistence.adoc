...
----
<1> Helidon MP's Jakarta Persistence integration supports version
...
Here is an example of a persistence unit named `prod` with a helpful
description:
...
<1> Because Helidon MP's JPA integration is for container-managed JPA,
    ...
    attribute] must always be set to `JTA`.
...
<!-- ... -->
----
<1> This is a Hibernate ORM-specific property and will be ignored if
    the JPA provider you have <<JPA-Setup,set up>> is Eclipselink.
    See
...
<2> This is an Eclipselink-specific property (and (a) is required and
    ...
    be ignored if the JPA provider you have <<JPA-Setup, set up>> is
    Hibernate ORM.  See
