...
Jakarta Persistence may be used in an entirely standalone manner,
which requires no integration at all.  This "application-managed mode"
places the burden of error handling, thread safety and transaction
management on the user.  This documentation does not cover
application-managed mode JPA.
...
thread safety and transaction management on behalf of the user.  This
documentation covers this "container-managed mode" usage of JPA
exclusively.

=== Maven Coordinates (JPA) [[JPA-Maven-Coordinates]]
