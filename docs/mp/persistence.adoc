...
The integrations supported for each technology, their
<<DS-Setup,setup>>, their <<DS-Configuration,configuration>>, and
their <<DS-Usage,usage>> are described below.
...
application, once it has been <<DS-Project-Setup,set up>> and
<<DS-Configuration,configured>>, create a
