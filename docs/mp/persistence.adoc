...
** <<Using-the-HikariCP-Connection-Pool, Using the Hikari CP Connection Pool>>
...
The way you set up named data source integration differs depending on
which connection pool you use.  Each setup is described below.

==== Using the HikariCP Connection Pool

===== Maven Coordinates [[Maven-Coordinates]]

To use the HikariCP, xref:../about/managing-dependencies.adoc[ensure
your dependencies are managed] and then ensure the following
dependency element is present as a child element of your project's
`pom.xml`'s `<dependencies>` element:
