...
All configuration for Helidon MP's named data source integration
support follows a common pattern:

> _objecttype_._datasourcename_._propertyname_=_value_

The name of a given configuration property always begins with the
fully-qualified Java type of the object being configured.  Most
configuration for Helidon MP's named data source integration concerns
the behavior of `javax.sql.DataSource` objects, so most configuration
property names begin with `javax.sql.DataSource.`.
