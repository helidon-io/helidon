...
* The _propertyname_ portion comes next, identifying the
  connection-pool- or driver-specific configuration property name,
  comes last.
...
====== Example: System Properties on the Command Line
...
====== Example: Environment Variables Set By the Shell
...
to an Oracle Database on `localhost` port `1521`, using the Oracle JDBC
"thin" driver, with a service name of `XE`, a `user` of `scott`, and a
`password` of `tiger`:
...
In general, the properties that can be set on the Oracle
Universal Connection Pool can be inferred from the "setter" methods
found in
...
In general, the properties that can be set on the Oracle JDBC driver
can be inferred from the "setter" methods found in
https://docs.oracle.com/en/database/oracle/oracle-database/21/jajdb/oracle/jdbc/pool/OracleDataSource.html[the
javadoc for the `OracleDataSource` class].
...
In general, the properties that can be set on the H2 JDBC driver can
be inferred from the "setter" methods found in
http://www.h2database.com/javadoc/org/h2/jdbcx/JdbcDataSource.html[the
javadoc for the `JdbcxDataSource` class].
