...
its Github repository].  Properties that should be "passed through" to
the vendor-supplied `DataSource` are prefixed with `dataSource.` as
...
application, once it has been set up and configured, create a
