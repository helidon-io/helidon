...
You list your application's _persistence units_ as
...
for container-managed JPA, the kind of named data source is always a
<<JTA, JTA>> data source.  The `<jta-data-source>` element, a child of
the `<persistence-unit>` element, is where you link a persistence unit
to a <<DS, named data source>> you previously <<DS-Configuration,
configured>>.
...
<1> This links this persistence unit to a <<DS, data source>> named
    ...
    in the <<DS-Configuration>> section above.
