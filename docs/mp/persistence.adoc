...
** <<DS-Driver-Setup, Setting Up Your Database Driver>>
...
==== Setting Up Your Database Driver [[DS-Driver-Setup]]
...
managed], and then ensure that a `<dependency>` element describing
your JDBC driver is present as a child element of your project's
`pom.xml` file's `<dependencies>` element.  A couple of examples
follow.  They are not exhaustive.
...
===== Maven Coordinates (Oracle JDBC) [[DS-Oracle-Driver-Setup]]
...
<dependency>
    ...
    <!--
         ...
         (b) Java 17 includes JDBC 4.3, not JDBC 4.2 or any other version
    ...
    <!-- 
       ...
       If instead you opt for the explicit version here, always check
