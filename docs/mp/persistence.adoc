...
<1> The `@Transactional` annotation indicates that this method should
    be invoked in the scope of a JTA transaction.  The object
    containing this method must be one that Helidon MP's CDI container
    has created, i.e. it must be managed.  (CDI beans are managed, as
    are Jakarta RESTful Web Services resource classes.)
