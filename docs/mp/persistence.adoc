...
managed], and then ensure the following `<dependency>` element is
...
* The name of a given configuration property always begins with the
  *_objecttype_* portion: a fully-qualified Java class name of the
  ...
  `javax.sql.DataSource` objects, so most configuration property
  names begin with `javax.sql.DataSource`.
...
the `test` data source to `baz` looks like this in Java
`.properties` format:
...
<2> The _datasourcename_ portion of the configuration property name is
    `test`.
