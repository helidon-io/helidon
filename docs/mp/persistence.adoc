...
`<properties>` element to specify them.

NOTE: Helidon MP's Jakarta Persistence integration is for
container-managed JPA, so many of the vendor-independent properties
https://jakarta.ee/specifications/persistence/3.0/jakarta-persistence-spec-3.0.html#a12384[described
in the specification] **do not apply**, particularly those describing
database connectivity information.  See <<JPA-JTA-Data-Source, the JTA
