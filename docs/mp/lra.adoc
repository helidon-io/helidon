...
==== Participant URL Validation

The Helidon LRA coordinator validates participant URLs by default before storing
or calling them. Participant URLs must use `http` or `https`, must include a
host, and must not include user information. Unless explicitly allowed, the
coordinator also rejects unresolved hosts and hosts that resolve to loopback,
link-local, site-local, multicast, unique-local, or other restricted addresses.

Use `helidon.lra.coordinator.participant-url.validation` to customize this
validation for deployments that deliberately use local addresses or a fixed
host allow list.
...
helidon.lra.coordinator:
  participant-url:
    validation:
      ...
      allowed-hosts:
        - "lra-participants.example.com"
----

Set `enabled` to `false` only for deployments that must preserve the previous
behavior. Use `allowed-hosts` for trusted internal names that should bypass
