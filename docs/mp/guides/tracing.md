...
```
1. This will enable tracing for all class methods, except for the constructor
   and methods that are private.
...
1. The `getMessage` method will be traced since it is externally invoked by
   `GreetResource`.
2. The `getMessage2` method will not be traced, even with the @Traced
   annotation, since it is called internally by `getMessage`.
<!--@mdc :: -->
...
```
1. This is the `WebTarget` needed to send a request to the second service at
   port `8081`.
