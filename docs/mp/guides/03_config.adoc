...
The generated project already accesses configuration data in the `Neo4jDataProvider` class  as  follows:
...
----
<1> This class is application scoped so a single instance of `Neo4jDataProvider` will be shared across the entire application.
<2> Define a thread-safe reference that will refer to the message member variable.
<3> The value of the configuration property `app.greeting` is injected into the `Neo4jDataProvider`.
...
----
<1> Inject the value of `app.greeting` into the `Neo4jDataProvider` object.
...
This object is not initialized when the `Neo4jDataProvider` constructor is called, so you need to provide
a method (`onStartup`) that observes `@Initialized`.  This method will be called when `Neo4jDataProvider` is ready for
use.
...
<2> Inject the `Config` object into the `Neo4jDataProvider` object.
