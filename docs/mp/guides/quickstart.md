...
> [!TIP]
> You can also generate the Quick Start project using the [Helidon CLI](../../about/cli.md). Run the following command to create a Maven project in your current directory: `helidon init --batch -Dflavor=mp -Dapp-type=quickstart`
...
2.  Build the application by running `mvn package`. The project builds an application jar and saves all runtime dependencies in the `target/libs` directory, making it easy to start the application.
...
To deploy the application to Kubernetes, you must have access to a Kubernetes cluster. Some lightweight options for running Kubernetes include Docker Desktop or minikube. This quick start assumes you’re using Docker Desktop. See [Docker Desktop](https://docs.docker.com/desktop/) in the Docker documentation.
