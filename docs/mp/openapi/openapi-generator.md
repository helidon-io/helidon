...
Helidon MP clients and servers. As a result, you can use the generator to create
...
Use the Helidon *client* generator and its `mp` library to create a [Helidon
...
To access a remote service that implements the endpoints declared in the OpenAPI
document, your code uses the generated client library first to establish a
connection to the remote service and then to call remote service endpoints by
invoking local methods passing POJO business objects or Java types as arguments.

Use the tool’s Helidon *server* generator and its `mp` library to create server
endpoint stubs for a Helidon MP service. You build on these stubs by extending a
...
the generated server code plus Helidon MP underneath it allows you to focus on
...
As you generate a Helidon MP *server*, you can choose whether you want Java
...
When you generate a Helidon MP server, the generator creates a separate
...
When you generate a Helidon MP client, the generated code contains a separate
...
Your code plus the server code from the Helidon generator all running on Helidon
MP combine to fully implement the server API declared in the original OpenAPI
document. Build your project to get a tailored Helidon MP server `.jar` file or
Docker image and your server is ready to run.
...
The Helidon MP client generator creates a MicroProfile REST client interface for
each API. Each generated API interface is annotated so your code can `@Inject`
the API into one of your own beans and then use the interface directly to invoke
the remote service. Alternatively, you can also explicitly use the
[`RestClientBuilder`][restclientbuilde] to create an instance programmatically
and then invoke its methods to contact the remote service. The [Helidon MP REST
Client][helidon-micropro] documentation describes both approaches in more
detail.

In the following example, `ExampleResource` (itself running in a server) invokes
a remote Pet service and shows one way to use the generated `PetApi` REST client
interface.
...
5. Invokes the remote service using the injected field and the parameter from
