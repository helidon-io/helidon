...
The WebLogic Server and Helidon integrations can be secured with IDCS over OIDC protocol.
...
<2> Enable redirect to IDCS SSO login page.
<3> Propagate JWT token obtained from IDCS after login to subsequent JAX-RS calls.
<4> For which hosts called by JAX-RS client can JWT token be used.

Helidon will redirect clients accessing protected JAX-RS resources to IDCS login page.
After the successful login, Helidon negotiates JWT bearer token and maps user's roles to current security context.
...
<2> JAX-RS resource method accessible only by authenticated users.
<3> Roles user needs to be authorised to use, roles are provided by idcs-role-mapper.
<4> JAX-RS client call, JWT token is added automatically when token propagation is configured.

While JWT token can be directly injected and used in your JAX-RS resource for subsequent calls to Weblogic IDCS protected resources,
with properly configured outbound token propagation token can be propagated automatically.
