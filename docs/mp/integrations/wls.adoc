...
The Oracle WebLogic Server and Helidon integration enables interaction between a Helidon microservice application and an application installed on Oracle WebLogic Server.
The following integrations are explained in this document:

* The REST integration allows Oracle WebLogic Server applications and Helidon microservices to communicate through RESTful web service invocations.
* The JMS integration allows Helidon microservices to publish and consume messages from Oracle WebLogic JMS Server.
...
The REST services integration with Helidon enables bidirectional REST calls between Helidon and Oracle WebLogic Server.
...
Thanks to the Jakarata EE support you can create same RESTful resource or client which will work in both environments.
...
While Oracle Weblogic Server supports Jakarta EE 8, Helidon supports JAX-RS or newly Jakarta RESTful Web Services from {jakarta-version}.
...
Oracle Weblogic Server provides Java Message Service (JMS) and acts as a messaging broker accessible even from outside the cluster.
To access Oracle Weblogic JMS Server from outside, client library is needed.
Helidon JMS connector for reactive messaging can be configured to use {wls-thin-link} for either consuming or
emitting messages.
...
Legacy thin T3 clients must be used with specialized xref:../../mp/reactivemessaging/weblogic.adoc[WebLogic JMS Connector] in order to work.

Updated version of thin T3 client compatible with modern Jakarta runtimes is available at {osdc-link} as `wlthint3client.jakarta`.
After the download, thin T3 client artefact needs to be installed in the maven repository accessible from
your application build.

[source, shell]
.Example of installing thin T3 client artefact to the local maven repository
----
...
[source,xml]
.Dependencies for Reactive Messaging with thin T3 client
----
<dependency>
...
After adding the Maven dependencies,
configure the Helidon JMS connector including the JMS environment properties and the JMS resources such as the connection factory,
...
[source, yaml]
.Example messaging configuration with JMS connector and thin T3 client
----
...
<2> Example of WLS Cluster t3 connection URL
...
[source, java]
.Consuming messages from Oracle Weblogic JMS Server
----
@Incoming("from-wls-q")
...
[source, java]
.Producing messages to Oracle Weblogic JMS Server
----
@Outgoing("to-wls-q")
...
For more information about setting up JMS integration between Helidon and Oracle Weblogic Server, see {wls-jms-link}.
