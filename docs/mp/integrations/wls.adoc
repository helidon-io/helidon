...
Use the `clientgen` WebLogic Web Services Ant task by using the `com.oracle.webservices.wls.jaxws-wlswss-client.jar` file, installed earlier, to generate the *jakarta* based client artifacts that client applications use to invoke the WebLogic Web Services. 

Add a call to the `clientgen` Ant task, as shown in the following example:
[source, xml]
----
<target>
...
You can create the RESTful Web Service to invoke the WebLogic Web Service, as shown in the following example:
[source, java]
----
@Path("/helidonclient")
@ApplicationScoped
