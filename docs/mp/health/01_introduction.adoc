...
To enable MicroProfile Health add the xref:../introduction/02_microprofile.adoc[helidon-microprofile bundle]  dependency to your project's `pom.xml` (see xref:../../about/04_managing-dependencies.adoc[Managing Dependencies]).
...
(or use the xref:../introduction/02_microprofile.adoc[helidon-microprofile bundle] )
...
include::{rootdir}/includes/config/io.helidon.health.HealthSupport.adoc[leveloffset=-1]
...
Generate Helidon MP Quickstart project following these xref:../guides/02_quickstart.adoc[Instruction]
...
Full example code is available link:{helidon-tree-url}/examples/microprofile[here].
