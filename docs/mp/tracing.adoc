...
include::{rootdir}/includes/tracing/common-config.adoc[]
...
----
<1> There is a new span name for the `getmessage` method, since your code called that method during the invocation of `/greet`.
...
Jaeger maintains several metrics about its own activity (briefly outlined in the
 link:{jaeger-doc-base-url}}/monitoring/#metrics[Jaeger client documentation]). This document explains how you can
 integrate those Jaeger tracing metrics with Helidon's metrics.
