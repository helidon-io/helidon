...
keys are obfuscated with "\*" characters. Keys matching the configured `secrets`
patterns are always obfuscated. Set `unsafe-values` to `true` to restore the
previous behavior and include all values except those matching `secrets`
patterns.
