...
- <<Usage, Usage>>
** <<Requiring Encryption, Requiring Encryption>>
** <<Using Symmetric Encryption (AES), Using Symmetric Encryption (AES)>>
** <<Using Asymmetric Encryption (RSA), Using Asymmetric Encryption (RSA)>>
...
|${ENC=...}
|Shared secret encryption, versioned base64 value
|${ENC=AQAJJYB...}

|${GCM=...}
|Legacy shared secret encryption, base64 value
...
* <<Encrypting Values (AES), Encrypting Values (AES)>>
* <<Shared Secret (AES), Shared Secret (AES)>>
...
----
java -jar <path-to-app-libs-dir>/helidon-config-encryption-{helidon-version}.jar enc masterPassword secretToEncrypt [iterations]
----
...
[source,bash]
.Encrypt secret using the legacy GCM format
----
...
* <<Encrypting Values (RSA), Encrypting Values (RSA)>>
* <<Configuring Config Encryption Filter (RSA), Configuring Config Encryption Filter (RSA)>>
...
==== Configuring Config Encryption Filter (RSA)
