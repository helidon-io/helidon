...
| \${ENC=...}   | Shared secret encryption, versioned base64 value                           | \${ENC=AQAJJYB...}                                                                                                                                                                                                                                                                                                                                                 |
| \${GCM=...}   | Legacy shared secret encryption, base64 value                              | \${GCM=D/UgMzsNb265HU1NDvdzm7tACHdsW6u1PjYEcRkV/OLiWcI+ET6Q4MKCz0zHyEh9}                                                                                                                                                                                                                                                                                           |
...
```shell [Terminal]
java -jar <path-to-app-libs-dir>/helidon-config-encryption-4.4.0-SNAPSHOT.jar enc masterPassword secretToEncrypt [iterations]
```

The tool returns the string to be entered into configuration as the value of a
property. The optional fourth argument sets the PBKDF2 iteration count used for
the generated value. It must be between 600000 and 10000000; values below the
lower bound are rejected even when explicitly requested. If omitted, the tool
uses 600000 iterations.

Encrypt a secret using the legacy GCM format:

```shell [Terminal]
java -jar <path-to-app-libs-dir>/helidon-config-encryption-4.4.0-SNAPSHOT.jar aes masterPassword secretToEncrypt
```

Legacy `${GCM=...}` values continue to be supported for compatibility. New
shared-secret values should use `${ENC=...}`.
