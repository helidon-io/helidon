...
include::{rootdir}/includes/config/io.helidon.config.mp.MpConfigBuilder.adoc[leveloffset=-1]

Current properties may be set in `application.yaml` or in `microprofile-config.properties` with `mp.config` prefix.

See xref:#_spec_name_profiles[Config Profiles] for more information.
