...
Helidon configuration sources can use different formats for the configuration data. You can specify the format on per-source bases, mixing and matching formats as required.
...
See xref:config-sources.adoc#_creating_microprofile_config_sources_for_manual_setup_of_config[manual setup of config] section for more information.
...
For more information, see xref:{rootdir}/security/configuration-secrets.adoc[Configuration Secrets].
...
This is a Helidon specific feature available since version 2.3.0. See
 xref:config-sources.adoc#_creating_microprofile_config_sources_from_meta_config[Microprofile Config Sources] for detailed information.
...
[CARD]
.MP Config Guide
[link={rootdir}/guides/config.adoc]
--
