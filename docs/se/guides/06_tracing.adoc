...
<4> Make the a new span a child of the threadContext span.
<5> Start the span.  The threadContext timestamp is used as the starting time for the span.
<6> Finish the span.  The threadContext timestamp is used as the ending time for the span.
...
<2> Create and start a span that is a child of the threadContext span.
