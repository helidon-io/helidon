...
include::{rootdir}/includes/prerequisites.adoc[tag=common-prereqs]
...
Refer to xref:../config/01_introduction.adoc[Helidon Config] for the full configuration concepts documentation.
...
"app-greeting".  See xref:../config/06_advanced-configuration.adoc[Advanced Config] for more information.
...
See xref:../config/06_advanced-configuration.adoc[Advanced Config] for more information.
...
[source,yaml]
.Create a file named `cofnig-profile.yaml` in the `helidon-quickstart-se` directory with the following contents:
----
sources:
  - type: "classpath" // <1>
    properties:
...
methods as described in xref:../config/03_hierarchical-features.adoc[Hierarchical Config] and xref:../config/06_advanced-configuration.adoc[Advanced Config>].
...
See xref:../config/05_mutability-support.adoc[Config Mutability Support] for a full discussion on this topic.
...
introduction section at xref:../config/01_introduction.adoc[Helidon Config] has much more information including
the following:
