...
Such component cannot run without the metrics implementation on the runtime path, and the Maven dependency makes sure the Helidon metrics implementation is present at runtime.
...
3. Pass the `util` config node to `UtilComponent.Builder`'s `config` method.
4. Use the builder to create a new `UtilComponent` instance.

Alternatively, the app might support its own config setting for `observability` with settings `off`, `normal`, `max`. At levels `normal` or `max`, the app enables `UtilComponent` metrics for `normal` or `max` settings, and disables metrics for `off`.
[source,java]
