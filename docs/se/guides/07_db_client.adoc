...
private static Routing createRouting(Config config, DbClient dbClient) {
        ...
        LibraryService libraryService = new LibraryService(dbClient); // <2>
        HealthSupport health = HealthSupport.builder()
                .addLiveness(HealthChecks.healthChecks())   // Adds a convenient set of checks
                .addLiveness(DbClientHealthCheck.create(dbClient))  // <3>
                .build();
        ...
        return Routing.builder()
                ...
                .register("/library", libraryService)  // <4>
                .build();
    }
----
<1> Add dbClient as a parameter of createRouting method.
<2> Instantiate our LibraryService with the dbClient.
<3> Add Health check to control the application behavior.
<4> Register the LibraryService to the Routing.
