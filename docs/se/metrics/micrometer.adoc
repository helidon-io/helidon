...
Helidon provides no special API beyond what Micrometer offers itself for access to Micrometer meter registries and meters.
...
Helidon provides an link:{helidon-github-tree-url}/examples/integrations/micrometer/se[example SE application] using Micrometer integration.
...
+
In general, the function should look at the request--the `Content-Type`, query parameters, etc.--to
