...
meters. Application code obtains the global registry using
...
1.  Use `Services.get(MeterRegistry.class)` to get a reference to the global
    [`MeterRegistry`][meterregistry] instance.
2.  Use the `MeterRegistry` instance with builders from
...
global `MeterRegistry`. Use `Services.get(MeterRegistry.class)` to get a
reference to the global meter registry.

To locate an existing meter or register a new one, your code:

1.  Creates a builder of the appropriate type of meter using `MetricsFactory`,
    setting the name and possibly other characteristics of the meter.
  ...
  features:
    observe:
      observers:
        metrics:
          auto-http-metrics:
            use-updated-http-metrics: true # <4>
            paths:
              - path: "/greet"              # <1>
                methods: ["GET","HEAD"]
              - path: "/greet/{name}"       # <2>
...
[metricsfactory]: https://helidon.io/docs/v4/apidocs/io.helidon.metrics.api/io/helidon/metrics/api/MetricsFactory.html
