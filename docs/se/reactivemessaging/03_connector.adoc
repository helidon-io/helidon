...
Supplied xref:../config/01_introduction.adoc[Helidon Config] is merged with
...
==== Implicit Config
Implicit config without any hard-coding is possible with xref:../config/01_introduction.adoc[Helidon Config] following
...
As the API is the same for xref:../../mp/reactivemessaging/01_introduction.adoc[MicroProfile Reactive Messaging] connectors, all that is needed to make connector work in both ways is annotating it with
...
xref:../../mp/reactivemessaging/03_connector.adoc[Messaging Connector Bean].
