...
by Messaging API, or configured implicitly for using registered xref:connector.adoc[connector]
...
xref:../../mp/reactivemessaging/message.adoc[message wrapping] as MicroProfile messaging.
 ...
 * xref:kafka.adoc[Kafka connector]
 * xref:jms.adoc[JMS connector]
