...
link:{faulttolerance-javadoc-base-url}/io/helidon/faulttolerance/package-summary.html[Fault Tolerance SE API Javadocs].

=== Updating your POM

In order to use Fault Tolerance you first need to add the <<Maven Coordinates, maven dependency>> to your `pom.xml`.

=== Single<T> and Multi<T>

In what follows we shall assume the reader is familiar with the two core Helidon types
`Single<T>` and `Multi<T>` from the link:{reactive-base-url}/io/helidon/common/reactive/package-summary.html[io.helidon.common.reactive]
package. Most simply,
