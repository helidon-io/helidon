...
<2> For convenience extract the `openapi` config.
...
For instance, the example above add the UI service to the `OpenApiFeature.Builder` _before_ applying configuration to the `OpenApiFeature.Builder`.
