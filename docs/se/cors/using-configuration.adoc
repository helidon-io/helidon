...
to add CORS support to your resources. The example in xref:using-the-api.adoc#se-api-routing-example[Sample Routing Setup Using the `CrossOriginConfig` API]
...
* Use these same configuration techniques to control the behavior of the CORS-enabled built-in services.
 xref:support-in-builtin-services.adoc[Learn more].
