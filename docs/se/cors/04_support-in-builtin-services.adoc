...
You tell the browser app the URL for your application.
The browser app uses the URL to retrieve, from your application itself, the OpenAPI document
which describes the application's endpoints.
...
+
--
Add a dependency on the Helidon {helidon-variant} CORS artifact to your `pom.xml`.

Even if your own endpoints do not use CORS with your own endpoints, you need to add the CORS dependency
if you want the built-in services to support CORS.
...
The link:{quickstart-example}[Helidon {helidon-variant} QuickStart example] uses these services, so use that as a template for your
...
as described in the <<se/cors/03_using-configuration.adoc,CORS configuration>> documentation.
