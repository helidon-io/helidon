 ...
 * Process the request and <<sending-response,send>> a response.
 ...
 * Throw an exception or call `request.next(exception)` to begin
 xref:05_error-handling.adoc[error handling].
...
+
* call `req.next(throwable)` to forward the handling to the xref:05_error-handling.adoc[error handlers]
+
[source,java]
----
.any("/hello", (req, res) -> { // <1>
