...
The support for Neo4j is implemented in drivel level. Just add the dependency, add configuration in `application.yaml` file and Neo4j driver will be configured by Helidon and can be used with `Neo4j` support object.
...
This example implements a simple Neo4j REST service using MicroProfile. For this example a working Neo4j database is required. Neo4j Movie database is used for this example.
...
Goto the Neo4j browser and play the first step of the movies graph: link:http://localhost:7474/browser/?cmd=play&arg=movies[`:play movies`]
...
Finally, we are able to use the `Neo4j` driver. Domain objects (_Movie_, _Actor_, _Person_) code is omitted.
...
Movies now ca be returned as JSON objects:
...
----
<1> Use `Neo4j` support object to initialise and configure the driver.
<2> Use `Neo4jMetricsSupport` to add _Neo4j_ metrics to `/metrics` output.
<3> Use `Neo4jHealthCheck` to add _Neo4j_ health support.
...
Neo4j metrics can be propagated to user as `MicroProfile` metrics. This is implemented in a separate Maven module. Just add
...
If your application is highly dependent on Neo4j database, health and liveness checks are essential for the correct work of this application.
