...
=== What's New in Helidon SE Config {helidon-version}
If you used a previous version of Helidon SE Config, take note of these important changes: 

* When loaded from Config, the Resource class has been refactored to require a config node `resource` that will be used to read it.
* Meta configuration has been refactored to be done through `ServiceLoader` services. If you created
a custom ConfigSource, PollingStrategy or RetryPolicy, see the <<insert link to new advanced doc from Tomas>> 
* Helidon Config now implements MicroProfile Config (not explicitly, you can cast between MP Config and Helidon Config).
* There is a change in behavior between MP methods and SE methods of config related to system
property handling:
** MP TCK requires that system properties are fully mutable (as soon as the property is changed, it
