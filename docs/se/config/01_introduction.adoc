...
=== Introducing the Config System [[introducing-the-config-system]]
...
link:{config-javadoc-base-url}/Config.Builder.html[`Config.Builder`].
+
* using a xref:09_config-profiles.adoc[config profile] to choose the sources to be used
* creating a xref:06_advanced-configuration.adoc#Config-Advanced-metaconfig[meta-configuration]
...
retrieve config data as various Java types. See the link:{config-javadoc-base-url}/Config.html[`Config`]
...
| Where config comes from |xref:02_config-sources.adoc[Config sources],xref:09_config-profiles.adoc[Config Profiles],
xref:06_advanced-configuration.adoc#Config-Advanced-metaconfig[meta-configuration]
| What format config data is expressed in |xref:02_config-sources.adoc#parsers[Config parsers],
xref:08_supported-formats.adoc[supported formats]
| How to filter, override, and dereference values |xref:06_advanced-configuration.adoc#filters-and-overrides[Filters and overrides]
| What happens when config data changes |xref:05_mutability-support.adoc#polling[Config polling]
| How to deal with loading errors |xref:02_config-sources.adoc#retry[Config retry policies]
|===
...
| How config data is translated into Java types |xref:04_property-mapping.adoc[Config mappers]
| How to navigate config trees |xref:03_hierarchical-features.adoc[Navigation]
|===
...
| Writing extensions |xref:07_extensions.adoc[Extensions]
|===
