...
as configuration. See the xref:02_config-sources.adoc[config sources] section
and the link:{config-javadoc-base-url}/ConfigSources.html#file-java.lang.String-[`ConfigSources.file`]
JavaDoc.

=== In-memory Config Sources
The config system provides several ways to create a `Config` tree from data
already in memory. See the link:{config-javadoc-base-url}/ConfigSources.html[`ConfigSources` javadoc]
...
link:{config-javadoc-base-url}/ConfigSources.html#prefixed-java.lang.String-java.util.function.Supplier-[`ConfigSources.prefixed`] method.
...
implementation of
link:{config-javadoc-base-url}/ConfigSources.MergingStrategy.html[`ConfigSources.MergingStrategy`]
...
This section is now available in xref:09_config-profiles.adoc[config profiles],
...
the configuration specified by a meta-config file by invoking the link:{config-javadoc-base-url}/MetaConfig.html#config-io.helidon.config.Config-[`MetaConfig.config(Config)`]
...
The xref:01_introduction.adoc[introduction] section shows how to use
...
only if the classpath includes the corresponding parsers. The introduction section on xref:01_introduction.adoc#built-in-formats[built-in formats]
section describes this further.

See javadoc link:{config-javadoc-base-url}/Config.Builder.html#config-io.helidon.config.Config-[`Config.Builder.config(Config)`].

==== Meta-configuration File Format
This section is now available in xref:09_config-profiles.adoc#Config-Profile-Format[config profile format]
...
== Configuration Key
As described in the xref:03_hierarchical-features.adoc#accessByKey[hierarchical features
...
xref:07_extensions.adoc#Config-SPI-ConfigFilter[filters] and
xref:07_extensions.adoc#Config-SPI-OverrideSource[overrides].
...
The built-in link:{config-javadoc-base-url}/ConfigFilters.html#valueResolving--[value-resolving]
filter enables the token substitution described below.

See the link:{config-javadoc-base-url}/spi/ConfigFilter.html[`ConfigFilter`] JavaDoc
...
See the link:{config-javadoc-base-url}/spi/OverrideSource.html[`OverrideSource]` JavaDoc
for more detail.
...
link:{config-javadoc-base-url}/ConfigSources.CompositeBuilder.html[CompositeBuilder]
...
a link:{config-javadoc-base-url}/RetryPolicies.Builder.html[RetryPolicies.Builder].
