...
<2> `Supplier<ConfigSource>` for the file
 `app.conf` loaded from the threadContext `classpath`.
<3> Specifies the prefix `data` for the associated source.
<4> Supplier<ConfigSource> for the file `app.conf` loaded from the threadContext `classpath`.
...
first on the file system in threadContext directory, then on classpath, checked in this order:
...
expression (which can use wildcards) to match config keys read from the threadContext
...
config source, which identifies a threadContext environment (see `env.yaml` examples
