...
- <<Loading Config using Meta-configuration, Loading Config using Meta-configuration>>
...
[IMPORTANT]
=========
You can achieve this with less configuration using xref:config-profiles.adoc[config profiles],
 which make meta-configuration obsolete. A profile file is a meta-configuration file selected by the rules defined in the link above.
=========
...
that declares the sources to load and their attributes. _meta-configuration_ file format is defined at xref:config-profiles.adoc#Config-Profile-Format[config profile format]
...
the configuration specified by a meta-config file by invoking the link:{config-javadoc-base-url}/io/helidon/config/MetaConfig.html#config-io.helidon.config.Config-[`MetaConfig.config(Config)`]
...
See javadoc link:{config-javadoc-base-url}/io/helidon/config/Config.Builder.html#config-io.helidon.config.Config-[`Config.Builder.config(Config)`].
