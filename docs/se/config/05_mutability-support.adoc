...
Your application can retrieve it by invoking the link:{config-javadoc-base-url}/Config.html#timestamp--[timestamp method]:
[source,java]
...
=== Config Context
The config system maintains a link:{config-javadoc-base-url}/Config.Context.html[`Config.Context`]
...
link:{config-javadoc-base-url}/spi/PollingStrategy.html[`PollingStrategy`] for each
config source you want to monitor. See the section on
xref:07_extensions.adoc#Config-SPI-PollingStrategy[polling strategies] in the
...
on the link:{config-javadoc-base-url}/PollingStrategies.html[`PollingStrategies`] class:
...
Each `Config` instance exposes the link:{config-javadoc-base-url}/Config.html#changes--[`Config.changes()`]
