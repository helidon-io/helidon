...
4. If the matcher finds no match or multiple case-insensitive matches, it throws a `ConfigEnumMappingException` with a message explaining the specific problem.

The following example illustrates how to use the built-in `enum` converter.
[source,java]
----
class Example {
...
If your application must deal with such an `enum`, simply pass your own mapping function (instead of the `enum` type) to the `Config.as(Class)` method. A xref:customConfigAs[later section] describes this technique which works for all types, not only `enum` types.
