...
The config system includes several built-in parsers, such as for the Java properties, YAML, JSON, and HOCON formats. See xref:#built-in-formats[this section] for
...
| Where config comes from |xref:#sources[Config sources],xref:config-profiles.adoc[Config Profiles],
