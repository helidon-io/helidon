...
+
* using a xref:config-profiles.adoc[config profile] to choose the sources to be used
* creating a xref:advanced-configuration.adoc#Config-Advanced-metaconfig[meta-configuration]
...
| Where config comes from |xref:config-sources.adoc[Config sources],xref:config-profiles.adoc[Config Profiles],
xref:advanced-configuration.adoc#Config-Advanced-metaconfig[meta-configuration]
| What format config data is expressed in |xref:config-sources.adoc#parsers[Config parsers],
...
| What happens when config data changes |xref:mutability-support.adoc#polling[Config polling]
| How to deal with loading errors |xref:config-sources.adoc#retry[Config retry policies]
|===
