...
The xref:introduction.adoc[config system introduction] explains the design of the config
...
objects - see link:{config-javadoc-base-url}/ConfigSources.html[`ConfigSources`]).
Implement a link:{config-javadoc-base-url}/spi/ConfigSource.html[`ConfigSource`] to
...
Implement the
link:{config-javadoc-base-url}/spi/ConfigParser.html[`ConfigParser`] SPI to allow the config system to handle additional formats.
...
Prevent auto loading of parsers for a given builder by invoking `Config.Builder#disableParserServices()`.
...
link:{config-javadoc-base-url}/spi/OverrideSource.html[`OverrideSource`] SPI one or more times and pass instances of
 those implementations to the config builder's
link:{config-javadoc-base-url}/Config.Builder.html#overrides-java.util.function.Supplier-[`overrides`] method.
...
link:{config-javadoc-base-url}/spi/OverrideSourceProvider.html[`OverrideSourceProvider`] service loader SPI
...
link:{config-javadoc-base-url}/spi/ConfigFilter.html[`ConfigFilter`] interface.
...
link:{config-javadoc-base-url}/Config.Builder.html[`addFilter` methods]. The config
...
link:{config-javadoc-base-url}/Config.Builder.html#disableFilterServices--[`disableFilterServices`]
method.
...
|link:{config-javadoc-base-url}/spi/ConfigFilter.html[`ConfigFilter`]
...
types. (See link:{config-javadoc-base-url}/ConfigMappers.html[`ConfigMappers`].)

To handle mappings to other types the application can register
custom mappers with the config system by implementing the
link:{config-javadoc-base-url}/spi/ConfigMapperProvider.html[`ConfigMapperProvider`]
 SPI.
...
runtime classpath. The application can prevent auto loading of mappers for a
...
Implementations of link:{config-javadoc-base-url}/spi/PollingStrategy.html[`PollingStrategy`]
may trigger regular check whether a source has new data.
Implementation of link:{config-javadoc-base-url}/spi/ChangeWatcher.html[`ChangeWatcher`]
...
link:{config-javadoc-base-url}/spi/RetryPolicy.html[`RetryPolicy`]
...
link:{config-javadoc-base-url}/RetryPolicies.html[`RetryPolicies`], such as
...
Ideally the retry policy implementation should be able to abort the execution of the retry policy, even while a
 function call is in progress, but the policy must respond to cancels between function calls.
