...
:plantumlconfig: {docdir}/_plantuml-config.txt
:rootdir: {docdir}/../..
...
The xref:01_introduction.adoc[config system introduction] explains the design of the config
...
link:{config-javadoc-base-url}/spi/OverrideSource.html[`OverrideSource`] SPI one or more times and pass instances of those implementations to the config builder's
link:{config-javadoc-base-url}/Config.Builder.html#overrides-java.util.function.Supplier-[`overrides`]
method.
