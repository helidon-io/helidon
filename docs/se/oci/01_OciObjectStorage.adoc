...
In order to use the integration, the following setup should be made.
...
Just run the application either from maven or your IDE.

To upload a file type in the terminal:

[source,bash]
----
curl -i -d @frank.png -X POST -H "Content-Type:application/image" http://localhost:8080/files/file/pom.xml
----
...
[source,bash]
----
curl -X GET http://localhost:8080/files/file/frank.png
----
