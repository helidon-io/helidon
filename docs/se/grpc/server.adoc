...
If your application uses `GrpcRouting.builder(config)` and includes the
`helidon-security-integration-grpc` dependency, Helidon automatically adds
`GrpcSecurity.create(config.get("security"))` when `security.grpc-server`
exists in configuration. You can still add `GrpcSecurity` explicitly; Helidon
does not add a duplicate interceptor that uses the same security configuration.

[source,java]
.Example using automatic configuration
----
GrpcRouting.builder(config)
    .register(new GreetService())
    .build();
----
