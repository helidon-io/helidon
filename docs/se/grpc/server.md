...
The default gRPC protocol can be configured using `server.protocols.grpc`. See
the [gRPC configuration reference][grpc-config] for all options.
...
Configure security as a gRPC server service under `grpc.grpc-services.security`.
The `security` root config still defines the Helidon security providers. The
example below uses HTTP Basic authentication, so it also needs the
...
Automatic gRPC security loading is supported from `grpc.grpc-services.security`.
The `server.protocols.grpc` tree configures the gRPC protocol only, not gRPC
services or their security rules. For named sockets, register gRPC security
programmatically with that socket’s gRPC routing.

You can also register gRPC security programmatically:

```java
GrpcRouting.builder()
        .intercept(GrpcSecurity.create(security, config.get("grpc.grpc-services.security")))
