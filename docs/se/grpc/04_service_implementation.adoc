...
* It allows you to configure some of the Helidon value-added features, such
  as xref:08_security.adoc[security] and xref:07_metrics.adoc[metrics collection]
...
* It provides built in support for xref:06_health_checks.adoc[health checks].
...
NOTE: The `complete` method shown in the example above is just one of many helper
      ...
      link:{grpc-server-javadoc-base-url}/GrpcService.html[here].
