...
The API defines the notion of a _handler_ that can be combined with other handlers to
...
as that would result in a much higher response time.
...
NOTE: Note also that `Single<T>`, much like `CompletableFuture<T>` extends `CompletionStage<T>`,
 so conversion among these types is fairly straightforward.
...
of retries is reached; finer control is
...
[source,java]
----
Single<T> single = Fallback.create(throwable -> Single.just(lastKnownValue)).invoke(this::mayFail);
single.thenAccept(t -> ...);
----
...
impact on the application performance. It is often preferred to avoid calls to non-essential
...
All the examples so far have been focusing on invocations returning
a single value of type `Single<T>`. If the invoation in question can return
...
produce zero or more values instead of just at most one value. Note that
a `Flow.Publisher<T>`, unlike a `Single<T>`,  can report an error after
producing one or more values, making it more difficult to use if all
