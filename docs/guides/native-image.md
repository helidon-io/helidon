<!--@frontmatter
description: "Build a GraalVM native image for a Helidon application"
navigation:
  icon: i-lucide-zap
-->
# Native Image
...
| [GraalVM for JDK 25][graalvm-for-jdk]           | `native-image` support requires GraalVM for JDK 25. When running in the Graal JVM (not native-image) Helidon supports GraalVM for JDK 25 or newer. |
...
export GRAALVM_HOME=/usr/local/graalvm-jdk-26/Contents/Home/
```
...
supports see the [Helidon Quickstart Guide](quickstart.md).
...
> it will take a while because it is downloading all the Maven dependencies and
> caching them in a Docker layer. Subsequent builds will be much faster as long
> as you don’t change the `pom.xml` file. If the pom is modified then the
...
[native-images]: https://www.graalvm.org/jdk25/reference-manual/native-image/
...
[graalvm-for-jdk]: https://www.graalvm.org/release-notes/JDK_25/
[downloading-and]: https://www.graalvm.org/jdk25//docs/getting-started/
[limitations]: https://www.graalvm.org/jdk25/reference-manual/native-image/metadata/Compatibility/
