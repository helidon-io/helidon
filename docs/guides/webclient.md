...
Introduction](../modules/webclient.md).
...
2. From the `MeterRegistry`, create a `Counter` metric using a builder from
   `MetricsFactory` and the specified metric name.
...
2. Create a counter metric from `MeterRegistry` using a builder from
   `MetricsFactory`.
