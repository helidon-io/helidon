...
For the removed Gson media and security providers, follow the migration steps
...
`io.helidon.http.media:helidon-http-media-gson` and `GsonSupport` are removed.
Replace this media dependency and any explicit `GsonSupport` registration with
a supported media implementation, such as `helidon-http-media-jackson` and
`JacksonSupport`. Adapt Gson-specific annotations, type adapters, and naming
settings to the selected implementation, and verify that existing request and
response JSON retains the expected format.
...
| Config Vault (`helidon-security-providers-config-vault`) | Replace the `config-vault` secrets and encryption provider and its API calls with an explicit secrets and encryption implementation. This provider has no drop-in replacement in Helidon Core. |

Config Vault is separate from the HashiCorp Vault integration described under
[Extensions](#extensions). Changing integration coordinates alone does not
replace `ConfigVaultProvider`.
