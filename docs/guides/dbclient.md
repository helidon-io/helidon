...
sample Helidon project that can be used to run some basic examples using the
Helidon DbClient.
...
_Prerequisite product versions for Helidon 27.0.1-SNAPSHOT_:
...
2.  Unzip the downloaded file into your directory.
    - Only the h2-<version>.jar, located in the h2/bin folder, will be needed.
...
<dependencies>
    ...
    <dependency>
        <groupId>io.helidon.dbclient</groupId>
        <artifactId>helidon-dbclient-json</artifactId> <!-- (7) -->
    </dependency>
    <!-- ... -->
</dependencies>
```
...
The `helidon-dbclient-json` module maps DB rows to Helidon JSON `JsonObject`
instances and maps `JsonObject` instances to named or indexed statement
parameters. MongoDB statements also accept Helidon JSON `JsonObject` and
`JsonArray` instances directly as statement parameters.

> [!NOTE]
> In Helidon 27, the JPMS module name of
> `helidon-dbclient-metrics-hikari` is
> `io.helidon.dbclient.metrics.hikari`. Applications upgrading from Helidon 4
> that declare `requires helidon.dbclient.metrics.hikari;` must change the
> declaration to `requires io.helidon.dbclient.metrics.hikari;`. Maven
> coordinates and Java packages are unchanged.
...
2. Helidon DB Client executes the `select-book` SQL script from
   application.yaml.
...
method sends the whole row of the database as a Helidon JSON `JsonObject`.
