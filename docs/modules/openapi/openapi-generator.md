<!--@frontmatter
description: "Helidon OpenAPI Generator"
-->
...
Helidon clients and servers. As a result, you can use the generator to create
...
Use the Helidon *client* generator and its `se` library to create a Helidon
client based on [Helidon WebClients](../webclient.md). The resulting
...
endpoint stubs for a Helidon service. You build on these stubs by extending a
...
the generated server code plus Helidon underneath it allows you to focus on
...
[earlier links](#configuration) to the lists of configuration options for the
Helidon generators groups options and additional properties in separate tables.
...
<td>
Directory where the generator should place files.<br />
We strongly recommend
<code>&lt;output&gt;target/generated-sources&lt;/output&gt;</code> or a subdirectory
below there.
</td>
<td><code>.</code><br />
...
<td>
Whether Maven should include the output directory as a source root (that is,
include it automatically in the build).<br />
We advise
<code>&lt;addCompileSourceRoot&gt;true&lt;/addCompileSourceRoot&gt;</code>.
</td>
<td><code>false</code></td>
</tr>
</tbody>
...
<td>
Affects the APIs which are used, Helidon 3.x vs Helidon 4.x or
<code>javax</code> vs. <code>jakarta</code>
</td>
</tr>
<tr>
...
that become obsolete. Similarly, you must understand how changes in the OpenAPI
document or the generation options affect the project dependencies and update
the project `pom.xml` accordingly.
...
As you generate a Helidon *server*, you can choose whether you want Java
...
When you generate a Helidon server, the generator creates a separate
...
first-path-segment` which groups operations into APIs not by `tags` value but by
the first segment of each operation’s path.

When you generate a Helidon client, the generated code contains a separate
...
more APIs or services and generates either an abstract class or an interface
your choice for each API. You need to extend each generated API class or
implement each generated API interface by writing your own classes.
...
1. Business logic: create a very simple data store - a real app would use a
   database.
2. Business logic: make sure the pet being added does not already exist. Send
   the invalid request status code if it does.
...
2. The `tags` parameter conveys the tag values to be matched in selecting pets
   to
...
- In that subclass override the relevant method.
  ...
  Customized `AddPetOp` class:
    ...
    1. Extracts the parameter from the request. This happens to use the same
       logic as in the generated method, but you can customize that as well if
       you need to.
...
The generated Helidon client includes the class `ApiClient`. This class
...
The Helidon client generator gives you as much flexibility as you need in
...
```
1. The same `ApiClient` instance can be reused to invoke multiple APIs handled
   by
the same server.
...
The Helidon client generator creates an `ApiResponse` interface. Each
...
4. Wait for the content to arrive, extracting the result and converting it to
   the proper type.
<!--@mdc :: -->
...
- [Helidon WebClient documentation](../webclient.md)
...
[webclient]: https://helidon.io/docs/core/v28/apidocs/io.helidon.webclient.api/io/helidon/webclient/api/WebClient.html
[webclientconfig]: https://helidon.io/docs/core/v28/apidocs/io.helidon.webclient.api/io/helidon/webclient/api/WebClientConfig.Builder.html
