...
> [!NOTE]
> Helidon support for OpenTelemetry configuration and semantic conventions as
> described below is currently an [`@Features.Preview`][preview-feature]
> feature. We intend to support it going forward, but we might change its
> external API and behavior in backward-incompatible ways across dot releases.

## Overview

Helidon supports OpenTelemetry in several important ways:

- Implements the [neutral Helidon tracing API](../tracing.md) using
  OpenTelemetry
...
In most cases, an application that prepares OpenTelemetry programmatically and
then bypasses Helidon’s OpenTelemetry ownership should initialize both of those
before either contract is requested from the registry, as shown in the
following example.
...
- Creates a Helidon `Tracer` using the OpenTelemetry instance and registers that
  tracer in the Helidon service registry.

> [!NOTE]
> Application code should retrieve the Helidon `OpenTelemetry` and `Tracer`
> instances from the service registry, for example using
> `Services.get(OpenTelemetry.class)` and `Services.get(Tracer.class)`.

Some third-party OpenTelemetry instrumentation uses `GlobalOpenTelemetry`
...
Helidon offers an implementation of its neutral tracing API that uses
OpenTelemetry. Add the following dependency to use OpenTelemetry for tracing.
...
To allow deployers and end users to use Helidon configuration to control
OpenTelemetry behavior, add the following dependency.
...
Helidon’s tracing observability support automatically creates a new tracing
span for each HTTP request if your project includes the following dependency.
...
- Setting `registered` to `true` makes the configured owner active and has the
  effect described in the [section](#assigning-the-application-instance) about
  application instances in the Helidon service registry.
- Setting `global` to `true` publishes that application instance to
  `GlobalOpenTelemetry` for OpenTelemetry instrumentation that reads the
  JVM-wide OpenTelemetry global. If the global already exists, Helidon leaves it
...
If you want Helidon to create spans automatically for incoming HTTP requests
and emit them using OpenTelemetry semantic conventions, add the dependencies
described earlier in this page for:

- [automatic HTTP request tracing](#enabling-automatic-spans-for-http-requests)
- [Helidon OpenTelemetry configuration][helidon-otel-config-section]
...
### Oracle APM Documentation

- [Configure OpenTelemetry and Other Tracers][oracle-apm]
...
[preview-feature]: https://helidon.io/docs/core/v27/apidocs/io.helidon.common.features.api/io/helidon/common/features/api/Features.Preview.html
...
[helidon-opentele]: https://helidon.io/docs/core/v27/apidocs/io.helidon.telemetry.otelconfig/io/helidon/telemetry/otelconfig/package-summary.html
[opentelemetrycon]: https://helidon.io/docs/core/v27/apidocs/io.helidon.telemetry.otelconfig/io/helidon/telemetry/otelconfig/OpenTelemetryConfig.html
[builder]: https://helidon.io/docs/core/v27/apidocs/io.helidon.telemetry.otelconfig/io/helidon/telemetry/otelconfig/OpenTelemetryConfig.BuilderBase.html
...
[helidon-otel-config-section]: #adding-opentelemetry-configuration-and-builder-support
[otel-provider-section]: #using-the-opentelemetry-implementation-of-the-helidon-tracing-api
