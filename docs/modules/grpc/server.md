...
the [Helidon gRPC Example][grpc-se-example] for a complete example.
...
[pom.xml][pom-xml] file in the Helidon gRPC example for guidance.
...
Helidon stores connection-level metadata in the gRPC context. Handlers and
interceptors can obtain `GrpcConnectionContext` using
`ServerContextKeys.CONNECTION_CONTEXT`.
...
The requested SNI host is normalized client request data. The matched SNI host
is the configured virtual-host host or wildcard pattern selected by SNI.
...
The gRPC protocol can be configured using `server.protocols.grpc` or
`server.sockets.<socket-name>.protocols.grpc`. See the [gRPC configuration
reference][grpc-config] for all options.
...
HTTP routing security protects HTTP routes only. gRPC requests are dispatched by
the gRPC protocol, so secure gRPC services with
`helidon-webserver-grpc-security`.

```xml [pom.xml]
<dependency>
  <groupId>io.helidon.webserver</groupId>
  <artifactId>helidon-webserver-grpc-security</artifactId>
</dependency>
```

The gRPC security service is discovered from the classpath and enabled by
default. Configure its settings under `grpc-services.security`; set
`grpc-services.security.enabled=false` to disable it. The `security` root config
still defines the Helidon security providers. The example below uses HTTP Basic
authentication, so it also needs the
`helidon-security-providers-http-auth` dependency:
...
Service names may use the full gRPC service name or the final service name
segment. Method names use the bare gRPC method name, such as `Upper`, not the
full `StringService/Upper` method path.

You can also register gRPC security programmatically:

```java
GrpcRouting.builder()
        .intercept(GrpcSecurity.create(security, config.get("server.protocols.grpc.grpc-services.security")))
        .service(new StringService());
```

The gRPC security configuration reference is available at
[GrpcSecurityConfig][grpc-security-config].
...
The following gRPC examples for Helidon are available:
...
[here]: https://helidon.io/docs/core/v27/apidocs/io.helidon.grpc.core/io/helidon/grpc/core/ResponseHelper.html
