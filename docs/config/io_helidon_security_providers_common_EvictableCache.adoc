...
|`cache-evict-delay-millis` |long |`1 minute` |Delay from the creation of the cache to first eviction
|`cache-evict-period-millis` |long |`5 minutes` |How often to evict records
|`cache-overall-timeout-millis` |long |`60 minutes` |Configure record timeout since its creation.
|`cache-timeout-millis` |long |`60 minutes` |Configure record timeout since last access.
