...
:description: Configuration of io.helidon.reactive.webserver.WebServer
:keywords: helidon, config, io.helidon.reactive.webserver.WebServer
...
Type: link:{javadoc-base-url}/io.helidon.reactive.webserver/io/helidon/reactive/webserver/WebServer.html[io.helidon.reactive.webserver.WebServer]
...
|`requested-uri-discovery` |xref:{rootdir}/config/io_helidon_common_http_RequestedUriDiscoveryContext.adoc[RequestedUriDiscoveryContext] |{nbsp} |Sets the requested URI discovery context for use by the web server's default socket.
|`sockets` |xref:{rootdir}/config/io_helidon_reactive_webserver_SocketConfiguration.adoc[SocketConfiguration[&#93;] |{nbsp} |Adds an additional named server socket configuration. As a result, the server will listen
 on multiple ports.
