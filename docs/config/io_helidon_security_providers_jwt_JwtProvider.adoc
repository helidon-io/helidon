...
|`atn-token.jwt-audience` |string |{nbsp} |Audience expected in inbound JWTs.
 Required when `authenticate` is `true` and `atn-token.verify-signature` is `false`.
|`atn-token.jwt-issuer` |string |{nbsp} |Issuer expected in inbound JWTs.
 Required when `authenticate` is `true` and `atn-token.verify-signature` is `false`.
|`atn-token.verify-signature` |boolean |`true` |Configure whether to verify signatures.
 ...
 If this is `false` and `authenticate` is `true`, both `atn-token.jwt-issuer`
 and `atn-token.jwt-audience` must be configured.
...
|`authenticate` |boolean |`true` |Whether to authenticate requests.
 Set to `false` for outbound-only providers that should not authenticate inbound requests.
 For outbound-only providers without verification keys, also set `atn-token.verify-signature`
 to `false`.
