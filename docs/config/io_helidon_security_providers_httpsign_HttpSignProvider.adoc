...
|`inbound-date-validity` |Duration |`PT5M` |Configure the maximum accepted age or future skew for the signed `Date` header.
 Inbound signatures with a signed `Date` header outside this window are rejected.
...
- all methods: date, (request-target), host are mandatory; authorization if present (unless we are
