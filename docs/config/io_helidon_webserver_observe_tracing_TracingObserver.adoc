...
|`env-config` |xref:{rootdir}/config/io_helidon_tracing_config_TracingConfig.adoc[TracingConfig] |`TracingConfig.ENABLED` |Use the provided configuration as a default for any request.

 @return default web server tracing configuration
|`path-configs` |xref:{rootdir}/config/io_helidon_webserver_observe_tracing_PathTracingConfig.adoc[PathTracingConfig[&#93;] |`new @java.util.ArrayList@(@java.util.List@.of(PathTracingConfig.builder()
                                  .path(&quot;/metrics/*&quot;)
