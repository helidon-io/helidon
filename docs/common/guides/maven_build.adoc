...
Helidon provides a Maven plugin that provides the following goals:
...
For full documentation of the plugin please see

https://github.com/oracle/helidon-build-tools/tree/master/helidon-maven-plugin[Helidon Maven Plugin README]

If you use the Helidon application parent POM you will have this plugin configured
for you. If you need to configure the `helidon-maven-plugin` you can do so by passing
system properties to Maven (for example `-Djlink.image.addClassDataSharingArchive=false`)
or by overriding the configuration in your project's POM. Here are a couple of examples:
