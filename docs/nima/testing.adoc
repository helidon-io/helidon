...
Helidon provides a rich set of extensions based on JUnit 5 for Helidon testing. Testing can be done with automatic server start-up, configuration, and shutdown. Testing can also be done without full server start-up with `DirectClient` when no real sockets are created.

== API

There are two main annotations that you can use to test the Helidon web server.
