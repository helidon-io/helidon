...
already configured to metrics and health-checks, but we will remove health-checks to simplify this example.
...
----
<1> Register base and vendor metrics.
...
You can create application-specific metrics and integrate them with Helidon. The application has complete
control over each metric, so it is the application's responsibility to create and update the metrics.  For example,
an application may use the same counter for multple methods, or one counter per method.
Helidon maintains an application `MetricRegistry` which is used to managed all of the application metrics.
Helidon returns these metrics in reponse to a call to `/metrics/application`.

This section demonstrates how to use Helidon metrics in your application.  In all of these example, the scope and lifetime
of the metric is at the application-level.

==== Counter metric

The `Counter` metric is a monotonically increasing or decreasing numeric value. Typically, a counter is updated
in response to a REST API call.  The following example will demonstrate how to use a `Counter` to track the number of
times the `/cards` endpoint is called.
...
<2> Declare a `Counter` member variable to hold the counter.
...
----
<1> The count is one since the method was called once.


==== Meter metric

The `Meter` metric is used to measure the how many times an event occurs within a certain time period.  As soon as a `Meter` object
is created, the internal clock, used to calculate the rates, starts running. This metric also includes the field from the
`Counter` metric.
...
----
<1> These are the `Meter` metric with a set of rate fields along with the count.
<2> The `/cards` endpoint was called 3 times


==== Timer metric

A `Timer` metric aggregates timing durations and provides statistics.  It also includes throughput statistics using
an internal `Meter` metric. The timer measures duration in nanoseconds.  In the following example, a `Timer` metric is used to
measure the duration of a method's execution.
...
----
<1> Import metrics classes.
<2> Declare a `Timer` member variable to hold the counter.
...
----
<1> These are the same fiels used by  `Meter`
<2> These are the `Timer` fields that measure how long it took to execute the `getDefaultMessageHandler` method. These values
...
----
<1> Import metrics classes.
<2> Declare a `Histogram` member variable to hold the histogram.
...
----
<1> This is the histogram data.  If you invoke the `/cards` endpoint again, you will see some numbers change such as `count`, `mean`, and `stddev`.


==== Gauge metric

The `Gauge` metric measures a discreet value at a point in time, such as a temperature.  The metric is normally
not tied to a REST endpoint, rather it should be registered during application startup.  When the `/metrics/application` endpoint
...
[source,java]
.Replace `Main.createRouting` method with the following code:
----
    private static Routing createRouting(Config config) {
...
[source,java]
.Update the `GreetingCards` class with the following code to use the `Counter` metric which will simplfy the JSON output:
----
package io.helidon.examples.quickstart.se;
...
The following example shows how to integrate the Helidon SE applcation with Kubernetes.
...
* Configure metrics that are updated by the applicaiton when an application REST endpoint is invoked.
