...
A `Timer` metric aggregates timing durations and provides duration statistics.  It also includes plus throughput statistics using
...
<2> These are the `Timer` fields that measure how long it took to execute the `getDefaultMessageHandler` method.


==== Histogram metric

The `Histogram` metric calculates the distribution of a set of values within ranges.  This metric does
not relate to time at all.  The following example will record a set of numbers in a `Histogram` metric when
