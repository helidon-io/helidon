...
already configured for both metrics and health-checks, but you will remove health-checks to simplify this example.
...
This section demonstrates how to use Helidon metrics in your application.  You can create
application-specific metrics and integrate them with Helidon. The application has complete
control over each metric, so it is the application's responsibility to create and update the metrics.
For example, an application may use the same counter for multiple methods, or one counter per method.
...
even when it is updated in response to a REST request.
