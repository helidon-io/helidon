...
Docker is only required if you are going to install the image locally.
...
Add the following plugin to the `pom.xml`. This adds a goal to
the Maven `package` phase that containerizes the application
using a locally available Docker daemon.
