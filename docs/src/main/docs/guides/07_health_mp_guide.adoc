...
[source,bash]
.Build and run the application.  Issue the curl command with -v within 5 seconds and you see the application is not ready
----
...
[source,bash]
.Get all the health check data, including custom
----
curl http://localhost:8080/health
----
...
You can specify a custom port and root context for the health endpoint  You can only change the
root path and port for the overall health endpoint.  For example, you cannot use different ports
as in http://localhost:8080/myhealth and http://localhost:8081/myhealth/live.  Likewise, you
cannot use different paths like http://localhost:8080/health and http://localhost:8080/probe/live.

This example below will change the root path.
...
[source,bash]
.Build and run the application, then verify the health endpoint using the new 'myhealth' root
----
...
<2> The name of the new socket, it can be any name, we are using 'health'
...
[source,bash]
.Delete the contents of application.yaml so that the default health endpoint path and port are used. Rebuild and start the application, then verify the health endpoint
----
curl http://localhost:8080/health
----
