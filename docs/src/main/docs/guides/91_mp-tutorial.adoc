...
Notice that the `startServer()` method has been updated to use a
...
The first of these two methods implements a new HTTP GET service 
...
The second method implements a new HTTP POST service which produces
