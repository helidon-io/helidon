...
This POM files contains just the basics that we need to get started.  It
 establishes the Maven coordinates for the new project, it includes 
 the Helidon BOM POM (in the `dependencyManagement` section) and adds a 
 dependency for the MicroProfile 2.2 bundle which allows the use of
 MicroProfile 2.2 features in the application.

The `properties` section tells Maven to use Java 8 for the source
...
The application class must extend `javax.ws.rs.core.Application`. 
...
The `ApplicationPath` annotation sets the URL path that will be used to 
 access the application.

In the application class, override the 
...
This class is annotated with `Path` which sets the path for this resource
 as `/greet`.  The `RequestScoped` annotation defines that this bean is
...
In this class a `public JsonObject getDefaulMessage()` method is defined
 ...
 declares that this method will return JSON data. The method body creates
...
In this class, a `main` method is defined which starts the Helidon MP 
 server and prints out a message with the listen address.  Notice that
...
The `bean-discovery-mode` tells Helidon to look for the annotations
...
New entries are created in the `properties` to set the name of the `libs`
 directory where the dependencies will be placed.   In the `build` section,
 ...
 manifest classpath.  Additionally, the `maven-dependency-plugin` is
...
In addition to predefined server properties, application-specific 
 configuration information can be added to this file.  Add the following
 entry to the file, that will be used to set the content of greeting
 message.
...
This class also has the `ApplicationScoped` annotation, so it will persist
 for the life of the application.  The class contains an `AtomicReference` 
...
This updated class adds a `GreetingProvider` and uses constructor injection
 to get the value from the configuration file.  The logic to create the 
...
Also notice that the `startServer()` method has been updated to use a
 Config Builder, which is implemented in the new method `buildConfig()`.
 This method defines two sources for configuration information, first the
