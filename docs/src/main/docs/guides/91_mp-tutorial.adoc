...
This POM files contains just the basics that we need to get started.  It
 ...
 dependecny for the MicroProfile 2.2 bundle which allows the use of
 MicroProfile 2.2 features in the application.

The `properties` section tells Maven to use Java 8 for the srouce
 and target version, and sets the `mainClass` which will be used later
...
TIP: The warning message `JAR will be empty - 
 no content was marked for inclusion!` can be ignored for now 
 because there is no actual content in the application yet.
...
This class is annotated with the MicroProfile annotation `ApplicationScoped`
...
In the application class, we need to override the 
...
In the output you can see the JSON output from the `getDefaultMessage()`
 ...
 `7001`.  Later we will see how to configure the port.
