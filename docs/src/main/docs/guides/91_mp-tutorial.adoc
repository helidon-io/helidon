...
This POM file contains just the basics that we need to get started.  

<1> Establish the Maven coordinates for the new project.
...
<4> Add a dependency for the MicroProfile 2.2 bundle which allows the use of
...
TIP:  You can learn more about scopes and contexts; and how they are used from the https://docs.jboss.org/cdi/api/2.0/index.html[Specification].
...
TIP: So far this is just a JAX-WS application, with no Helidon or MicroProfile
 specific code in it.  There are many JAX-WS tutorials available if you 
...
Helidon MP applications are packaged into a JAR file and the dependencies
 are copied into a `libs` directory.  Some more detailed explanation of 
...
Endpoints for readiness and liveness checks are also provided by default.
 Obtain the default results using these URLs:
...
<1> Both endpoints return a HTTP 200 return code.
<2> Both endpoints report a status of "UP" but neither report any
 checks currently.
...
Content-Type: application/json
Date: Thu, 22 Aug 2019 13:25:37 -0400
transfer-encoding: chunked
connection: keep-alive

{"outcome":"DOWN","status":"DOWN","checks":[{"name":"greeting","state":"DOWN","status":"DOWN","data":{"greeting":"Hey"}}]} <2>
----
...
HTTP/1.1 200 OK <2>
Content-Type: application/json
Date: Thu, 22 Aug 2019 13:29:58 -0400
connection: keep-alive
content-length: 116

{"outcome":"UP","status":"UP","checks":[{"name":"greeting","state":"UP","status":"UP","data":{"greeting":"Hello"}}]} <3>
----
...
Note the PORTs. The application canbe exercised as before but use
...
That concludes this tutorial.  The tutorial has demonstrated how to build
...
There were several links to more detailed information included in the
 tutorial.  These links can be explored to learn more details about Helidon
