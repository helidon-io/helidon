...
The @Timed and @Metered can also be used to annotate a method.  For this example. you can just annotate the same method with these metrics.
...
across requests. When GET /metrics/applcation is invoked, Helidon will return the current value of the metric stored in the MetricRegistry.
...
application scoped class. When GET /metrics/applcation is invoked, Helidon will call the Gauge getter, store that value
...
[source,bash]
.You will see an output similar to this.  Repeat the `kubectl get pod` command until you get 2/2 and Running. This may take up to 1 minute.
----
