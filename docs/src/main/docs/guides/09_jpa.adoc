...
== Add a Java Class to Represent a Greeting
...
<1> Some of the annotations in this example have sensible defaults,
but the example specifies them explicitly for clarity.  This
annotation says that JPA will access this class' fields directly,
rather than via getter and setter methods.

<2> The `@Entity` annotation identifies this class as a JPA entity.
The `name` element's value can be used in JPQL queries.

<3> The `@Table` annotation identifies the database table to which
this class will be mapped.

<4> JPA entities should be `Serializable`.

<5> The `@Column` annotation specifies what column in the database the
annotated field maps to.  The elements of the `@Column` annotation
further describe the column's semantics.

<6> This field will be mapped to the primary key of the database table.

<7> This field will be mapped to an ordinary (basic) column.
