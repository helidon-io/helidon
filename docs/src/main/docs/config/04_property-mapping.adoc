...
`ConfigMappingException` (if some error occurred performing the data mapping).
...
to the target class. For both you write your own conversion function.
...
This feature is available in Object mapping module, and is added through java service
load mechanism. This is no longer part of core Config module, as it depends on reflection
and introduces a lot of magic.
