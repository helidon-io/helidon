...
Scripts that solve the mapping problem via explicit conversion of variables to
...
`"server.executor-service.max-pool-size"`. (Note that only 1 variant remains
if the name is already lowercase.)
