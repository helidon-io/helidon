...
The config system supports using environment variables as a config source. Since
environment variable names are normally restricted to alphanumeric characters and
underscore, this config source produces _additional_ variants to enable setting or
overriding config entries with dotted and/or hyphenated keys.

Variants are produced for any environment variable name that matches _all_ of the
following:

<1> length > 2 (`"X_Y"` to `"x.y"` is the shortest meaningful mapping)
<2> does not begin or end with a `"_"`
<3> does not contain `"__"`
<4> contains one or more `"_"` characters
...
added as `"com.ACME.size"` and `"SERVER_EXECUTOR-SERVICE_MAX-POOL-SIZE"` is added
as `"SERVER.EXECUTOR-SERVICE.MAX-POOL-SIZE"`.
