...
alphanumeric characters and underscore, this config source _adds_ variants that
...
using `"FOO_BAR_dash_BAZ"`. (Though it is conventional to use all uppercase for
environment variable names, the lowercase mapping means that `"foo_bar"` and
`"foo_bar_dash_baz"` are equally valid.)


One use case for this mapping is containers, where passing environment variables
at launch is common and config overrides are desirable. Scripts (as entry points
or standalone) that solve the mapping problem via explicit conversion of variables
to system properties can also be simplified.

Variants are produced for any environment variable name that matches _all_ of
the following:

<1> does not begin or end with a `"_"`
<2> does not contain `"__"`
<3> contains one or more `"_"` characters

For each such name, two variants are added with the names mapped as follows:

<1> Replace any `"\_dash_"` substrings with `"-"`, e.g.
`"SERVER_EXECUTOR_dash_SERVICE_MAX_dash_POOL_dash_SIZE"` becomes
`"SERVER_EXECUTOR-SERVICE_MAX-POOL-SIZE"`.

<2> Replace `"_"` with `"."` and add as a variant, e.g. `"com_ACME_size"` is
added as `"com.ACME.size"` and `"SERVER_EXECUTOR-SERVICE_MAX-POOL-SIZE"` is
added as `"SERVER.EXECUTOR-SERVICE.MAX-POOL-SIZE"`.

<3> Convert the result of step 2 to lowercase and add as a variant, e.g.
`"com.ACME.size"` is added as `"com.acme.size"` and
`"SERVER.EXECUTOR-SERVICE.MAX-POOL-SIZE"` is added as
`"server.executor-service.max-pool-size"`. (Note that if the name is already
lowercase, only 1 variant remains.)
...
By default config source implementations use the Java Service
`java.nio.file.spi.FileTypeDetector` API to infer the source media type from
...
* invoking the link:{javadoc-base-url-api}/Config.html#loadSourcesFrom-java.util.function.Supplier...[`Config.loadSourcesFrom`]
method, or
* invoking the link:{javadoc-base-url-api}/Config.html#builderLoadSourcesFrom-java.util.function.Supplier...[`Config.builderLoadSourceFrom`]
method.
...
Config config = Config.create(sourceFromMetaConfig);       // <3>
----
