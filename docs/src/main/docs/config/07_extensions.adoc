...
* `genericTypeMappers()` returns a map with entries for specific `GenericType` conversions,
    e.g. when the provider supports only mapping for `GenericType<Map<String, Integer>>`
...
* `mapper(GenericType)` returns a conversion function (optional) that coverts a config node
    to the GenericType (if supported by this provider) - e.g. in case this provider supporst
...
1. check if a conversion function exists for the class requested (from method `mappers()`)
2. check if a conversion function is provided by any `ConfigMapperProvider` with method `mapper(Class)`
3. check if a conversion function exists for a generic type for the class requested (from method `genericTypeMappers`)
4. check if a conversion function is provided by any `ConfigMapperProvider` with method `mapper(GenericType)` for
