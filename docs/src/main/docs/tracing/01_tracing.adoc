...
- `EnvTracingConfig` - the overall configuration of traced components of Helidon
...
----
routing.register(TracingConfiguration.create(config.get("tracing")));
----
...
----
routingBuilder.register(TracingConfiguration.create(config.get("tracing"));
----

[source,java]
.Configuration with Web Server using a builder
----
routingBuilder.register(TracingConfiguration.builder()
    .addPathConfig(PathTracingConfig.builder()
        .path("/metrics")
        .tracingConfig(EnvTracingConfig.DISABLED)
        .build();
    .build());
----