...
ifdef::se-flavor[]
The xref:{rootdir}/{flavor-lc}/open-telemetry.adoc[telemetry doc page] describes how to use the Helidon `telemetry` config section and the related builder to exert full control over  OpenTelemetry and OpenTelemetry tracing behavior.
endif::se-flavor[]
ifdef::mp-flavor[]
...
In contrast, this page describes support for controlling OpenTelemetry tracing using the `tracing` config section and builder. Intended primarily to ease migration from other tracing providers (such as Jaeger) to OpenTelemetry, Helidon's support for OpenTelemetry using _tracing_ does not afford as much control as do the Helidon _telemetry_ settings. For example, using OpenTelemetry `tracing` config you can choose either an OTLP gRPC span exporter or an OTLP HTTP one; other span exporters are available only using the `telemetry` settings.

[NOTE]
.Avoid specifying both `telemetry` and `tracing`
Helidon applies settings under `telemetry` or prepared using the telemetry builder over OpenTelemetry settings under `tracing` or set up with the tracer builder. Specifying both does not confuse Helidon but might confuse users.
