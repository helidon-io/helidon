...
ifdef::se-flavor[]
This page describes support for controlling OpenTelemetry tracing using the `tracing` config section and link:{javadoc-base-url}/io.helidon.tracing.providers.opentelemetry/io/helidon/tracing/providers/opentelemetry/OpenTelemetryTracerConfig.html[`OpenTelemetryTracerConfig` builder]. Users typically adopt this approach to ease migration from older tracing configuration to OpenTelemetry.
...
The xref:{rootdir}/{flavor-lc}/telemetry/open-telemetry.adoc[telemetry doc page] describes how to use the Helidon `telemetry` config section and the related builder to exert more control over  OpenTelemetry and OpenTelemetry tracing behavior.

endif::se-flavor[]
[NOTE]
.Avoid specifying both `telemetry` and `tracing`
If you provide settings under both `telemetry` and `tracing`, Helidon uses the `telemetry` settings. Specifying both does
not confuse Helidon but it might confuse users. Set `global: false` under `tracing` to make explicit that the `tracing`
section should not contribute the application-wide `OpenTelemetry` and `Tracer`.
...
tracing.global=true                          # <2>
tracing.int-tags.0.example=1                 # <3>
tracing.tags.0.direction=north               # <4>
----
endif::mp-flavor[]
<1> Specifies the OpenTelemetry service name.
<2> Indicates the configured tracer should contribute the application-wide `OpenTelemetry` and `Tracer` from the
Helidon service registry (defaults to `true`). Set this to `false` only when another section, such as `telemetry`,
owns the application-wide `OpenTelemetry`.
<3> Assigns an integer-valued tag `example` the value `1`.
<4> Assigns a string-valued tag `direction` the value `north`.

Set `tracing.global-open-telemetry` to `true` only if OpenTelemetry instrumentation in the application needs Helidon to
publish the selected application `OpenTelemetry` instance to `GlobalOpenTelemetry`. OpenTelemetry globals are JVM-wide
and can be assigned only once, so this must happen before any Java agent, OpenTelemetry autoconfiguration, or application
code initializes `GlobalOpenTelemetry`. If an OpenTelemetry global already exists, leave `tracing.global-open-telemetry`
set to `false` and configure Helidon to use the existing instance instead.
