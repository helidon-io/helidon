...
* `Collection`, `Collection`, `Stream`, or `Optional` with the query row type as the generic parameter
...
|Slice
|Contains the page data as a `Collection` or `Stream` and a `PageRequest` to retrieve this page.

|Page
|Contains the page data as a `Collection` or `Stream`, the total result size across all pages, and a `PageRequest`
