...
You add {metrics} to your service by writing {writing-code-content}
...
Helidon supports meters inspired by link:{micrometer-url}[Micrometer] and summarized in the following table:
...
link:{micrometer-api-url}/counters.html[Counters]
...
link:{micrometer-api-url}/distribution-summaries.html[Distribution summaries]
...
of the samples. |
link:{micrometer-api-url}/timers.html[Timers]

|
link:{metrics-javadoc-base-url}/io/helidon/metrics/api/Gauge.html[`Gauge<? extends Number>`]
...
link:{micrometer-api-url}/gauges.html[Gauges]
|====

// end::meter-types-summary[]

=== Categorizing Types of {Metrics_uc}
Helidon distinguishes among _scopes_, or categories, of {metrics}.
...
link:{javadoc-base-url}/io.helidon.common.features.api/io/helidon/common/features/api/Features.Preview.html[`@Features.Preview`] feature which Helidon intends to keep, but its external interface or behavior might evolve between dot releases.
...
Publishers in Helidon's Micrometer-based metrics implementation use Micrometer `MeterRegistry` implementations. Each Helidon meter registry owns a composite Micrometer registry containing one registry for each enabled publisher. This has these important effects:

* {meters_uc} which Helidon or your code registers in a Helidon meter registry are registered in all active publisher registries owned by that registry.
...
NOTE: Make sure at least one publisher configured for each Helidon meter registry is enabled. If not, that Helidon registry has no active publisher registries, its registered metrics are no-ops, and Helidon logs a warning when the registry is created.
...
When you add the
`helidon-webserver-observe-metrics` dependency
