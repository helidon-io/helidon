...
The `auto` subsection lets you control the metrics Helidon gathers automatically.
These settings control the key performance indicator {metrics} gathered in past releases of Helidon as well as the {metrics} for OpenTelemetry semantic conventions.
ifdef::se-flavor[]
