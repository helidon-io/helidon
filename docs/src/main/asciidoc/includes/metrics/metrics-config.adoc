...
[NOTE]
====
The `auto-http-metrics.use-updated-http-metrics` setting controls the updated OpenTelemetry HTTP metric behavior. It
defaults to `false` to retain the earlier Helidon 4.x duration, response-status, and `http.route` output. Set it to
`true` to use the updated behavior. With updated behavior, Helidon records `http.route` only when a non-blank matching
route is available; for matched JAX-RS requests, the value is the JAX-RS route template.
====
