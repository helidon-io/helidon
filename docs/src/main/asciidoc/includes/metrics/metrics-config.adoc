...
The `auto` subsection lets you control the metrics which Helidon gathers automatically.
These settings control the Helidon key performance indicator {metrics} as well as the {metrics} for OpenTelemetry semantic conventions.
ifdef::se-flavor[]
See the <<../../se/open-telemetry.adoc#enabling-automatic-metrics-for-inbound-http,Helidon OpenTelemetry documentation>> for more information.
...
By default, if you specify the `auto` section Helidon does not measure built-in endpoints such as metrics, health, and openapi but does measure all application endpoints. You can add items under  `auto.paths` to control more exactly which endpoints to measure.
