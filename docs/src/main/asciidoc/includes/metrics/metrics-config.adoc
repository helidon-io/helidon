...
The `auto` subsection lets you control metrics Helidon gathers automatically. Currently these settings apply only to the {metrics} collected for the OpenTelemetry semantic conventions, not the other Helidon-provided key performance indicator {metrics}.
ifdef::se-flavor[]
