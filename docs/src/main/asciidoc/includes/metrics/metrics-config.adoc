...
[NOTE]
====
The deprecated `auto-http-metrics.use-updated-http-metrics` setting temporarily controls the updated OpenTelemetry HTTP
metric behavior. It defaults to `false` to retain the earlier Helidon 4.x duration, response-status, and `http.route`
output. Set it to `true` to use the updated behavior; the setting is planned for removal in the next major release.
====
