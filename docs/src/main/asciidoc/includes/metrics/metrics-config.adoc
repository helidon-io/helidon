...
your configuration file, such as `application.yaml`.
...
include::{rootdir}/config/io_helidon_metrics_api_MetricsConfig.adoc[tag=config,leveloffset=+1]
...
Helidon 27 always reports the system-provided `gc.time` {metric} as a `Gauge`.
The former `metrics.gc-time-type` compatibility setting is no longer supported; remove it from configuration.
Application code that looked up `gc.time` as a `Counter` should use the `MeterRegistry` gauge lookup APIs instead.
The deprecated `metrics.rest-request-enabled` compatibility setting is no longer supported in Helidon 27;
replace it with `metrics.rest-request.enabled`.
...
include::io_helidon_webserver_observe_metrics_MetricsObserver_extra.adoc[tag=config,leveloffset=+1]

ifdef::se-flavor[]
See the <<../../se/telemetry/open-telemetry.adoc#enabling-automatic-metrics-for-inbound-http,Helidon OpenTelemetry documentation>> for more information.
...
[NOTE]
====
The `auto-http-metrics.use-updated-http-metrics` setting is deprecated and defaults to `true`. The default records
OpenTelemetry HTTP request duration in seconds after the response is sent, preserves the final response status, and
includes `http.route` only when a non-blank matching route is available. Set it to `false` only as a temporary
compatibility workaround for the earlier 4.x output (milliseconds, exception-derived status, and an always-present
route attribute whose value is empty when no matching route is available).
====
...
The xref:{rootdir}/config/io_helidon_webserver_observe_metrics_AutoHttpMetricsConfig.adoc[AutoHttpMetricsConfig documentation] describes the configuration more fully.
...
The names above use the default `CAMEL` built-in meter name format. With the `SNAKE` format, Helidon reports the
in-flight and long-running metrics as `requests.in_flight` and `requests.long_running`, respectively.
