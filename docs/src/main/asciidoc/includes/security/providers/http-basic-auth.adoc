...
NOTE: HTTP Basic authentication is not supported for production use.
...
HTTP Basic authentication is not supported for production use. We recommend it only for local testing and demo
purposes.
