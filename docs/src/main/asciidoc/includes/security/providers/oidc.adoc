...
==== Available OIDC config options
The OIDC provider reads the same `oidc` configuration node into `io.helidon.security.providers.oidc.common.OidcConfig`.
The provider configuration table above lists the provider wrapper options, such as outbound token propagation.
The table below lists the OIDC settings available on that same configuration node.

include::{rootdir}/config/io_helidon_security_providers_oidc_common_OidcConfig.adoc[leveloffset=+3,tag=config]

==== Available client credentials config options
When `outbound-type` is set to `CLIENT_CREDENTIALS`, the nested `client-credentials` configuration uses these options.

include::{rootdir}/config/io_helidon_security_providers_oidc_common_ClientCredentialsConfig.adoc[leveloffset=+3,tag=config]
...
tenant has the name `@default`. An identified tenant name must match a configured tenant; unknown tenant names are
rejected by default. Set `fallback-to-default-tenant-enabled: true` only when unknown tenant names should use the
...
CORS is (now) a single component configured either through config (key `cors`), or programmatically via
`io.helidon.webserver.cors.CorsFeature`. To add proper CORS setup for the OIDC endpoint, use one of these.
Component specific CORS setup will be removed from Helidon.
