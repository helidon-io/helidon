...
a small cookie instead, or to `NONE` to disable redirect attempt counting. The `redirect-attempt-param` value is used
as the cookie name prefix when the `COOKIE` strategy is used; the full cookie name also includes a tenant and original
URI hash.
