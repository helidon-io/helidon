...
server:
  ...
  # For throughput limiting, use server.concurrency-limit.throughput.amount (0 is unlimited) with a matching duration.
  # Listener-wide connection admission capacity. Connection-oriented bindings acquire a permit before
  # accepting or admitting work and release it when the connection ends or admission/setup fails.
