...
Add the following LangChain4j configuration for the OpenAI provider to `./src/main/resources/application.yaml`.

Model configured under `langchain4j.models` has arbitrary name `pirate-chat-model`,
it uses `open-ai` provider defined under `langchain4j.providers`.
With a single configured chat model, default auto-discovery resolves it automatically.
If you configure multiple chat models, use `@Ai.ChatModel("pirate-chat-model")` to select one explicitly.
