...
|https://www.oracle.com/technetwork/java/javase/downloads[Java{nbsp}SE{nbsp}26] (http://jdk.java.net[Open{nbsp}JDK{nbsp}26]) |Helidon requires Java 26+.
...
|=======
include::prerequisites.adoc[tag=prerequisites-table-rows]
| https://www.graalvm.org/release-notes/JDK_{graalvm-jdk-version}/[GraalVM for JDK {graalvm-jdk-version}]| `native-image` support requires GraalVM for JDK {graalvm-jdk-version}. When running in the Graal JVM (not native-image) Helidon supports GraalVM for JDK {graalvm-jdk-version} or newer.
|=======
include::prerequisites.adoc[tag=prerequisites-setup]
...
| https://www.azul.com/downloads/?package=jdk-crac#zulu[Azul Zulu JDK CRaC 25+] |
