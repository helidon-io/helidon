...
:ui-config-prefix: server.features.openapi.services.ui
// end::preamble[]
...
Add a runtime dependency on the SmallRye UI.
...
a `{ui-config-prefix}` section to your configuration file, such as `application.yaml`.
