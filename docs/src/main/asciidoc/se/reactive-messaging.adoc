...
the same style of connector orchestration and configuration while keeping the SE programming model explicit.
Helidon SE Reactive Messaging supports the same connector configuration format across the Helidon messaging APIs.
...
message wrapping across Helidon messaging implementations.
...
used across Helidon messaging implementations.
This allows connectors to be reused as long as they can operate without CDI.
...
by config using the `mp.messaging` key layout.
...
Implicit config without any hard-coding is possible with xref:config/introduction.adoc[Helidon Config] using the `mp.messaging` key layout.
