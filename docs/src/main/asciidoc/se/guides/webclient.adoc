...
have to point your client to the correct/final endpoint. On a cross-origin redirect, WebClient strips the `Authorization`
and `Cookie` headers by default. It also rejects `307` and `308` redirects that would replay a request entity to another
origin. Set `follow-cross-origin-entity-redirects` to `true` only when every possible redirect target is trusted.
