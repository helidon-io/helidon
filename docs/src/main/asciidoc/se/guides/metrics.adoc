...
Your Helidon SE application can also control KPI settings programmatically by creating a custom meter registry
with the registry-specific metrics configuration and assigning that registry to the metrics observer.
[source,java]
...
<2> Enable extended KPI {metrics}.
<3> Set the long-running request threshold.
<4> Prepare a link:{metrics-javadoc-base-url}/io/helidon/metrics/api/MetricsConfig.html[`MetricsConfig`] with the just-prepared KPI metrics config.
<5> Create a custom link:{metrics-javadoc-base-url}/io/helidon/metrics/api/MeterRegistry.html[`MeterRegistry`] using that metrics config.
<6> Assign the same metrics config to the observer; with a custom registry, the observer uses these settings for registry-specific behavior as well as endpoint settings.
...
<10> If constructing the server fails, close the application-owned custom meter registry.
<11> Otherwise, stop the `WebServer` when the application is ready to shut down.
<12> Close the custom meter registry only after confirming the server and its metrics observer have stopped. If the server remains running after a failed stop, retain the registry and retry server shutdown instead of closing it.
