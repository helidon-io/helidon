...
<10> When the application is ready to shut down, stop the `WebServer` in an inner `finally` block.
<11> Close the application-owned custom meter registry in an outer `finally` block so it is released even if construction, startup, or shutdown fails.
