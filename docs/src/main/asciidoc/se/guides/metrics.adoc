...
<9> Add the `ObserveFeature` to the `WebServer`.
<10> When the application is ready to shut down, stop the `WebServer`.
<11> Close the application-owned custom meter registry to release registry and publisher resources.
