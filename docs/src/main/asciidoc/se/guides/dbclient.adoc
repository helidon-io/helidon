...
The `helidon-dbclient-json` module maps DB rows to Helidon JSON `JsonObject` instances and maps `JsonObject`
instances to named or indexed statement parameters. MongoDB statements also accept Helidon JSON `JsonObject` and
...
Instead of sending the `INFO` content of the targeted book, the `getJsonBook` method sends the whole row of the
database as a Helidon JSON `JsonObject`.
