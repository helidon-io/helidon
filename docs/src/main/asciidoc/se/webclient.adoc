...
The telemetry webclient services provide metrics and spans which follow the OpenTelemetry semantic conventions for clients. These are separate from the `services.metrics` and `services.tracing` services described elsewhere on this page.
...
To allow metrics and tracing, add the following single dependency to your project:
...
To activate webclient telemetry collection using configuration, add the `telemetry` config section under `client.services` which enables `metrics` and `tracing`.

.Enabling telemetry using configuration
[source,yaml]
----
client:
  services:
...
To enable some but not all telemetry services, add `metrics` or `tracing` to the `telemetry` section. The following example activates only telemetry metrics.

.Enabling metrics telemetry using configuration
[source,yaml]
----
client:
  services:
...
To trigger webclient telemetry collection from your client code, add one or more webclient telemetry services to the webclient builder. This example shows adding only telemetry metrics.
