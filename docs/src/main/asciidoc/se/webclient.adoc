...
For redirects that preserve the request method and entity (`307` and `308`), WebClient does not replay a request
entity to a different origin unless `follow-cross-origin-entity-redirects` is enabled.
...
When `follow-redirects` is enabled, `307` and `308` redirects keep the original method and request entity.
By default, WebClient rejects those redirects if the target has a different origin and the request has an entity.
Set `follow-cross-origin-entity-redirects` to `true` only when the entity is safe to replay to the redirect target.
...
NOTE: The `passphrase` value in the config file can be encrypted if stronger security is required.
...
When `base-address` is a Unix domain socket address, it selects only the physical transport.
Configure `base-uri` or use absolute request URIs to provide the logical HTTPS authority.
The logical host and port are used for the `Host` header, HTTP/2 `:authority`, SNI, and endpoint identification.
...
* xref:discovery.adoc#_web_client_discovery_integration[`discovery`]
* `metrics`
* `tracing`
