...
`server.sockets.<socket-name>.protocols.grpc`. See the
...
The gRPC security service is discovered from the classpath and enabled by default. Configure its settings under
`grpc-services.security`; set `grpc-services.security.enabled=false` to disable it. The `security` root config defines
the Helidon security providers.
