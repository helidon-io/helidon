...
** <<Securing gRPC Services, Securing gRPC Services>>
...
The default gRPC protocol can be configured using `server.protocols.grpc`. See the
xref:{rootdir}/config/io_helidon_webserver_grpc_GrpcConfig.adoc[gRPC configuration reference]
for all options.
...
HTTP routing security protects HTTP routes only. gRPC requests are dispatched by the
gRPC protocol, so secure gRPC services with `helidon-webserver-grpc-security`.
...
The `security` root config still defines the Helidon security providers.
...
Service names may use the full gRPC service name or the final service name segment.
Method names use the bare gRPC method name, such as `Upper`, not the full
`StringService/Upper` method path.
...
xref:{rootdir}/config/io_helidon_webserver_grpc_security_GrpcSecurityConfig.adoc[GrpcSecurityConfig].
