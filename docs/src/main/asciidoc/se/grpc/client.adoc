...
of the `grpc.status` label must always be set to the string "OK". Support for
metrics of unsuccessful calls may be supported in the future, hence the need to include
...
NOTE: Metrics are only available for gRPC clients running alongside the
Helidon Webserver, that is, for gRPC clients running in a server environment.
