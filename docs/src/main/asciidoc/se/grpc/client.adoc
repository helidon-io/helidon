...
The resource-owning streaming methods are `serverStreaming`, `clientStreaming`, and `bidirectional`. They use Java
`Stream` for streaming requests or responses and apply transport readiness rather than eagerly buffering an entire
stream. A request stream is transferred to the call and must not be reused. A returned response stream owns the active
RPC and must be closed, including when an operation short-circuits before consuming every response. The calling thread
consumes a client-streaming request stream. If producing elements can block, closing the request stream must unblock
production so an early peer termination can return promptly. Use try-with-resources, for example:

[source,java]
----
include::{sourcedir}/se/grpc/ClientSnippets.java[tag=snippet_8_resource_stream, indent=0]
----
...
To enable tracing support, include the following dependency in your project:
...
Tracing support is discovered from the classpath via the gRPC client service SPI and enabled by default.
The `grpc-services` configuration is optional and can be used to configure or disable tracing:
...
`tracing:` section. Set `grpc-client.grpc-services.tracing.enabled=false` to disable only tracing, or set
`grpc-client.grpc-services-discover-services=false` to disable automatic discovery of all gRPC client services.
