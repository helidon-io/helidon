...
When the item type of a list is known, use the list helpers instead of creating a
`GenericType` descriptor explicitly:

[source,java]
.Serializing and deserializing a list
----
include::{sourcedir}/se/json/JsonSnippets.java[tag=snippet_33, indent=0]
----
