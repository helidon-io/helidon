...
- <<filters, Filters and Token Substitution>>
...
== Filters, and Token Substitution [[filters]]
When your application retrieves a config value, the config system can transform it
before returning the value, according to _filters_, and
...
xref:extensions.adoc#Config-SPI-ConfigFilter[filters]

Your application can add filters explicitly to a config builder
and the config system by default uses the Java service loader mechanism to
locate all available filters and add them automatically to all
...
The filter in module `io.helidon.config:helidon-config-overrides` provides support for overriding values using a pattern.

The overrides filter allows you to create an external source containing key/value
pairs which replace the value otherwise returned for the name. Such a source can be then configured
on the filter, which will apply the patterns.

Key features of the overrides filter:

* it can use config sources or config instance that supports changes as the source of the overrides
* it uses wildcards in key expressions, allowing better customization options
* overrides can affect only keys that already exist in the original source

The configuration of the filter can be done through its builder API, or through a config source.
If config source is used, then the key is the expression that can use wildcards, while the value is the target value to use for keys matching that expression.

Example of an override config source (using properties format):

[source,properties]
.Example `overrides.properties`
----
prod.*.logging.level = WARNING
----

The wildcard is in place for any word, so the following keys would match:

- `prod.first-pod.logging.level`
- `prod.second-pod.logging.level`

But the following key would NOT match:

- `prod.first-pod.component.logging.level`

The overrides filter can be configured with a custom config source, a custom config, using the actual config instance it filters (the base key is `overrides.expressions`), or using a builder, which allows configuration based on regular expression as well as wildcard expressions.

Example configuration using a builder:
[source,java]
...
----
<1> Adds an override config filter
<2> Adds a file config source with the override values
<3> Adds the application configuration file
