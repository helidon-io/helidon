...
|`asNodeList()` |Returns a ConfigValue<List<Config>>. For nodes of type `OBJECT` contains child nodes as a `Collection`.
...
----
<1> Get child nodes of the `data.providers` _list_ node as a `Collection` of `Config` instances.
