...
* Replace `Metrics.createMeterRegistry()` and `Metrics.createMeterRegistry(metricsConfig)` with
  `MeterRegistry.create()` and `MeterRegistry.create(metricsConfig)`.
...
* Replace `Metrics.tag(key, value)` with `Tag.create(key, value)`, and create multiple tags by collecting individual
  `Tag.create` results.
