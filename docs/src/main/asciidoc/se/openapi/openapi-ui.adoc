...
== API
With `helidon-openapi` in your `pom.xml` and the SmallRye UI runtime dependency added, the OpenAPI support automatically includes the default UI behavior, possibly modified by any UI settings you have in your configuration. You do not have to do anything else to enable the UI.
...
If your `pom.xml` includes the Helidon OpenAPI dependency and the SmallRye UI runtime dependency, then any `OpenApiFeature` object your application builds prepares the default OpenAPI UI behavior, possibly modified as above by any UI settings you have in your configuration.
