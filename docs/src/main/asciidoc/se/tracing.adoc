...
Helidon includes support for distributed tracing through its own API, backed by either
 https://opentelemetry.io/docs/instrumentation/js/api/tracing/[OpenTelemetry], https://www.jaegertracing.io/[Jaeger], or https://zipkin.io/[Zipkin].
Tracing is integrated with WebServer and Security.

NOTE: As OpenTelemetry has subsumed https://opentracing.io[OpenTracing], Helidon support for OpenTracing is deprecated and will likely be removed in a future release.
