...
cached with the resolved logical resource, so newly added sidecars are discovered after cache eviction or handler
restart. Classpath sidecar bytes
cached in memory still count against the configured memory cache capacity.
