...
against the selected representation, and `304 Not Modified` or `412 Precondition Failed` responses retain its
`Content-Encoding`, `Vary`, and ETag metadata. Byte ranges and `Content-Range` apply to the encoded sidecar bytes. A
matching strong sidecar ETag in `If-Range` permits a partial response; otherwise Helidon ignores the range and sends the
complete selected sidecar.
