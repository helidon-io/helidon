...
the removal has been observed creates a new cache record with current metadata.

Helidon does not detect modifications or replacements of an existing resource, including a resource deleted and recreated
between requests without its absence being observed. Retargeting an already-resolved symbolic link at runtime is also not
supported. Resources explicitly cached in memory are snapshots and continue to be served if their source file is removed.
Recreate the static-content service after modifying or replacing existing resources; a normal application restart does this.
