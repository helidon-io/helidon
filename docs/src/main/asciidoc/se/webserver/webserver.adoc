...
while wildcard matches use the configured provider order. Encoded responses include `Vary: Accept-Encoding`. If none of
the advertised encodings is supported by the WebServer, the response is returned uncompressed unless the client rejects
`identity`, in which case WebServer responds with `406 Not Acceptable`. When automatic response encoding is disabled,
