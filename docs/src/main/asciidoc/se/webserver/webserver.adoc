...
WebServer caches located services by service instance identity until the server stops. Return stable service instances for
stable item kinds so the cached route tree and lifecycle callbacks are reused.
