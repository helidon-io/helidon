...
When the port is omitted, WebServer advertises the local listener port. The default protocol is `h3`; maximum age and
the `persist` parameter are omitted by default.
