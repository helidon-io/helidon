...
* Restart the service, replacing the resource while the service is stopped and before it accepts further static-content
  requests.
* Drain traffic and wait for in-flight requests to complete, delete the resource, request it over HTTP on every service node
  and verify that it is no longer served, recreate the resource, and then resume traffic.
