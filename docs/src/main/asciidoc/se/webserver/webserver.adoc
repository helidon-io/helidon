...
When pre-compressed lookup is disabled, range requests are served from the identity representation even if listener
content encoding is configured and the request includes `Accept-Encoding`.
