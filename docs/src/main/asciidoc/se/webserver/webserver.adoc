...
by later requests. Once a resource has been resolved, however, Helidon can cache its location and metadata, including its
content length and last-modified time. Replacing, modifying, deleting, or retargeting an already-resolved resource at runtime
is not supported, even if its cached record is later evicted. Recreate the static-content service after deploying changes to
existing resources; a normal application restart does this.
