...
Pre-compressed sidecars win ties over runtime encoders, and non-identity candidates at the same quality win over
`identity`; identity wins only with a higher quality or when no acceptable sidecar or runtime encoder is available. If
two sidecars have the same quality, Helidon uses the `pre-compressed-encodings` order; the default order prefers `br`
before `gzip`. If no acceptable sidecar or runtime encoder exists and the client rejects `identity`, Helidon responds
with `406 Not Acceptable`. Dynamically encoded static responses ignore byte ranges, omit `Content-Length`, and use weak
entity tags.
