...
classpath handler-level `pre-compressed-cross-origin-sourcing-enabled` option can be set to `true` only when the
application intentionally sources sidecars from different classpath entries. Helidon does not validate that sidecar files
are fresher than the logical resource.
