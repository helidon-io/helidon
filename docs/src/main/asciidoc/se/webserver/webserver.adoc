...
`/big-file.html.gz`, are handled as ordinary static-file requests. A sidecar is selected only when the request explicitly
accepts that concrete coding; `Accept-Encoding: *` does not select pre-compressed sidecars. Sidecar lookup hits and
misses are cached with the resolved logical resource, so newly added sidecars are discovered after cache eviction or
handler restart. Classpath sidecar bytes
