...
`/big-file.html.gz`, are handled as ordinary static-file requests. `Accept-Encoding: *` can select any available
sidecar coding that is not explicitly rejected. Sidecar lookup hits and misses are cached with the resolved logical
resource, so newly added sidecars are discovered after cache eviction or handler restart. Classpath sidecar bytes
...
runtime candidate is the best acceptable representation. Wildcard values can select sidecars or runtime encoders.
