...
:rootdir: {docdir}/..
:version-lib-opentelemetry: 1.29.0
...
:helidon-otelcfg-javadoc: {otel-config-javadoc-base-url}/io/helidon/telemetry/opentelemetry-config/
...
* Implementing the xref:{docdir}/tracing.adoc[neutral Helidon tracing API] using OpenTelemetry
* Allowing users to assign OpenTelemetry settings as follows:
...
* Conforming to the link:{semconv-link}[OpenTelemetry semantic conventions] for automatically-created spans

OpenTelemetry models observability as a set of link:https://opentelemetry.io/docs/concepts/signals/[_signals_] which are origins of monitoring data--for example metrics, tracing, and logging. The OpenTelemetry runtime as a whole has certain characteristics, and each of the signals has its own extensive settings to control its behavior.
Similarly, Helidon's config support for OpenTelemetry has certain config attributes which apply to OpenTelemetry as a whole, others which pertain to individual signals, and still more which describe lower-level elements within a signal.

You see this hierarchy represented in the Helidon OpenTelemetry configuration format, the Helidon OpenTelemetry API, and this documentation:
...
This document describes the parts of this hierarchy as well as general topics related to Helidon's support of OpenTelemetry.

== API
There are really two APIs that might be useful to developers working with OpenTelemetry:
...
Typically, an application uses the same `OpenTelemetry` instance throughout. OpenTelemetry offers a global `OpenTelemetry` instance to make it easy for application code to obtain the global instance.
...
The link:{helidon-otelcfg-javadoc}[Helidon OpenTelemetry API Javadoc] page provides an entry point to complete information about the Helidon OpenTelemetry API, and later sections on this page further describe the configuration settings available.
...
If the application code directly uses the Helidon builder API for OpenTelemetry to prepare OpenTelemetry objects change the `<scope>runtime</scope>` line to `<scope>compile</scope>`.
[source,xml]
...
== Configuration
You can control almost all of OpenTelemetry's overall and tracing runtime behavior using Helidon configuration settings. Helidon constructs an `OpenTelemetry` object using the configuration, including an OpenTelemetry `TracerProvider` if the config specifies its settings under `signals.tracing`.
...
The next table offers further detail and shows some example configurations.
...
| defaults | No configuration for either processors or exporters.
...
| Configuration for at most one processor and at most one exporter.
...
----
| maximum flexibility | Configuration for possibly multiple processors and possibly multiple named exporters. Each processor's configuration lists the names of the exporters  it should use; no names means all exporters.
...
* link:https://opentelemetry.io/docs/specs/semconv/http/http-spans/[HTTP semantic conventions]
