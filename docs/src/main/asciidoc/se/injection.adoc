...
=== io.helidon.service.registry.Service.ServicesFactory
The ServicesFactory should be used only for its intended purpose — creating zero OR n instances at runtime, each assigned different qualifiers. This allows for dynamic and flexible service creation while ensuring proper differentiation between instances based on their qualifiers.
...
Keep in mind, that ServiceRegistryManager needs to be shut down, once it is not needed
[source,java]
----
