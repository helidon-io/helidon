...
NOTE: If the JSON-RPC response contains an error, it will also contain
an error code which is independent of the HTTP status code returned.

Every JSON-RPC response will contain either a result or an error, and that is
...
The response to a batch request will include an entry for each of the
