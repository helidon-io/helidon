...
* *Prototyped*: The generated code extending the blueprint.
It is part of the public API and includes the fluent builder implementation and static factory methods.
...
This use case demonstrates generating an immutable class with a builder from a blueprint interface.
The blueprint serves as the input for code generation, and the resulting prototype provides a fluent API for constructing instances.
...
- Its name must end with `Blueprint`.
The prototype's name will be the blueprint's name without this suffix.
...
<2> Must be package-private and named with a `Blueprint` suffix.
The prototype's name will be `ServiceConfig`.
...
<2> Generated interface extending the given blueprint.
The interface name is a blueprint name with "Blueprint" suffix removed.
...
3. Default values can be set using `@Option.Default`.
If a configuration value is missing, the default is used.
...
6. You can customize and validate fields using the @Option API.
Advanced programmatic customization is possible by implementing a custom `BuilderDecorator` via `@Prototype.Blueprint(decorator = MyDecorator.class)`.
...
<2> Marks the field `name` as configurable.
By default, the configuration key is derived from the method name in dash-separated format.
...
This scenario extends the basic functionality of Helidon Builder to create a user-defined runtime type based on a prototype.
This approach is particularly useful when the generated prototype needs to be transformed into a domain-specific runtime type.
...
When using a third party runtime type (for example `PrivateKey`), requirements 2 and 3 do not apply. In that case the setup should be:
...
`Option.Provider.value` identifies the provider contract to discover. For a configured option, implementations of that
contract create option values from configuration. For a non-configured option, the discovered implementations are the
option values themselves.

`Option.Provider.discoverServices` defaults to `true`. For a configured option, providers are always discovered so they
can create explicitly configured entries. Setting this property to `false` only prevents providers from contributing
values without matching configuration. Configuration can override the annotation default using the sibling key
`<option-config-key>-discover-services`. For a non-configured option, `false` prevents discovered implementations from
...
| `identity`
| `TYPE_ONLY` identifies an instance only by provider type, so each type may occur once. `TYPE_AND_NAME` identifies an
instance by the `(type, name)` pair and therefore permits differently named instances of the same provider type. The
default is `TYPE_AND_NAME`.

| `configForm`
| `OBJECT`, `LIST`, or `OBJECT_OR_LIST` selects the accepted outer configuration container. `AUTO` resolves to `OBJECT`
for `TYPE_ONLY` and to `OBJECT_OR_LIST` for `TYPE_AND_NAME`. The default is `AUTO`.
|===

The configuration form does not select the identity model, does not depend on whether the Java option is a single value
or a `List`, and does not impose a generic shape on provider-specific content within an entry. Object form does not
define materialization order; list form preserves configured order.

With `TYPE_AND_NAME`, an object key supplies the instance name, so names are globally unique in object form. The
following object configures `(metrics, primary)` and `(metrics, backup)`; their materialization order is unspecified:
...
Unlike object form, list form may reuse a name for a different provider type. An exact duplicate `(type, name)` pair in
the list is invalid and causes configuration to fail. Object form is therefore intentionally less expressive than list
form for `TYPE_AND_NAME`; the two forms are not equivalent representations with only an ordering difference.
