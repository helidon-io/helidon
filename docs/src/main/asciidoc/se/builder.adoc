...
| `Option.Provider`      | Satisfied by a provider implementation, see javadoc for details. For configured provider lists, each configured entry must have a unique provider `type` and `name`; duplicate `(type, name)` entries fail configuration validation.
