...
This example implements a simple Neo4j REST service. For this example a working Neo4j database is required. The Neo4j Movie database is used for this example.
...
Neo4j's metrics can be propagated to your application's metrics endpoint. This is implemented in a separate Maven module. Just add:
...
Health checks for Neo4j are implemented in a separate Maven module:
