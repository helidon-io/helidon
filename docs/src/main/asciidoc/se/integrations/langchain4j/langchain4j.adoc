...
Helidon provides `MetricsChatModelListener` which when enabled, generates metrics that follow the https://opentelemetr.io/docs/specs/semconv/gen-ai/gen-ai-metrics[Semantic Conventions for GenAI Metrics].

For example, adding the following lines to `application.yaml` for `OpenAiChatModel`, will enable Helidon's `MetricsChatModelListener`.
