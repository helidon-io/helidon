...
* `auto`(default value): cycles through the list of authentication types from `helidon.oci.allowed-authentication-methods` and chooses the first one capable of providing data. In case the `helidon.oci.allowed-authentication-methods` list is empty, all available strategies will be tried, ordered by their {@link io.helidon.common.Weight}.
...
Services can have a dependency on `com.oracle.bmc.Region`, and it can be looked up using ServiceRegistry lookup methods. This can be used to determine the current region where the service is running.
