...
NOTE: If you are interested in our MP based support of OCI then this xref:{rootdir}/mp/guides/oci-guide.adoc[Guide for Helidon MP application on OCI] describes the basics of developing and deploying a Helidon MP application based on our link:https://helidon.io/starter/{helidon-version}[Project Starter].
...
* ociAdName - Physical Availability Domain Name where the Instance exists.
...
Follow these steps to arrange automatic transmission of Helidon metrics to an OCI Monitoring backend system.

. Prepare an instance of the OCI `Monitoring` type. Either:
   * Configure the OCI authentication details (<<#_auth-config,illustrated below>>) so Helidon can use them to create a `Monitoring` instance automatically, or
   * Create an instance of `Monitoring` in your code and pass it to the `OciMetricsService` builder.
. Prescribe how you want Helidon to transmit the metrics data. Either:
   * Add configuration of the `oci` metrics publisher (see the <<_metrics-config,example>> below), or
   * Use the `OciMetricsService` builder to set up the transmission behavior.
...
[#_metrics-config]
To set up automatic transmission to an OCI metrics backend, you can use Helidon configuration (such as in `application.yaml`) to specify a metrics publisher.

[source,yaml]
.Example OCI Metrics transmission config in `application.yaml`
----
metrics:
  publishers:
    - type: oci                         # <1>
      compartment-id: my-compartment    # <2>
      resource-group: my-resource-group # <2>
      delay: PT2.5S                     # <3>
      initial-delay: PT10S              # <4>
----
<1> Indicates you want to use an `oci` publisher.
<2> Adds the identifying compartment and resource group.
<3> Sets up periodic transmissions to the OCI monitoring backend each 2.5 seconds.
<4> Waits for 10 seconds before sending the first metrics transmission.

include::{rootdir}/config/io_helidon_integrations_oci_metrics_OciMetricsService.adoc[tag=config,levelOffset=+3]
