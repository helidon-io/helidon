...
The endpoint must declare exactly one `@Grpc.Proto` method. The method must have no parameters and return
...
- Server streaming: `void method(Req, StreamObserver<Res>)`
- Client streaming: `StreamObserver<Req> method(StreamObserver<Res>)`
- Bidirectional streaming: `StreamObserver<Req> method(StreamObserver<Res>)`
...
To create a typed gRPC client, create an interface annotated with `GrpcClient.Endpoint` and `Grpc.GrpcService`, and at
least one method annotated with one of the gRPC method annotations.

Annotations on endpoint type:

- link:{grpc-client-javadoc-base-url}/io/helidon/webclient/grpc/GrpcClient.Endpoint.html[`io.helidon.webclient.grpc.GrpcClient.Endpoint`] - required annotation
- link:{grpc-api-javadoc-base-url}/io/helidon/grpc/api/Grpc.GrpcService.html[`io.helidon.grpc.api.Grpc.GrpcService`] - gRPC service name; use the fully-qualified service name when the proto declares a package
...
- Unary: `Res method(Req)` or `void method(Req, StreamObserver<Res>)`
- Server streaming: `Iterator<Res> method(Req)` or `void method(Req, StreamObserver<Res>)`
- Client streaming: `Res method(Iterator<Req>)` or `StreamObserver<Req> method(StreamObserver<Res>)`
- Bidirectional streaming: `Iterator<Res> method(Iterator<Req>)` or
  `StreamObserver<Req> method(StreamObserver<Res>)`

To inject a typed gRPC client, use the annotated interface with the `@GrpcClient.Client` qualifier.

[source,java]
----
@Service.Inject
MyService(@GrpcClient.Client GreetingClient client) {
}
----

The `GrpcClient.Endpoint.value()` defines the target URI for generated backing clients and supports configuration
...
can be modified using the `configKey` property of the `@GrpcClient.Endpoint` annotation.
...
2. Otherwise, if `GrpcClient.Endpoint.clientName()` is set and a matching named `GrpcClient` exists in the service
   registry, use that registry client.
3. Otherwise, if `GrpcClient.Endpoint.clientName()` is set and no matching named `GrpcClient` exists, create a new
   `GrpcClient` using the endpoint URI from `GrpcClient.Endpoint.value()`.
4. Otherwise, if an unnamed `GrpcClient` exists in the service registry, use that registry client.
5. Otherwise, create a new `GrpcClient` using the endpoint URI from `GrpcClient.Endpoint.value()`.
