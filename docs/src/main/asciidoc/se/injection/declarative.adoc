...
The `Grpc.GrpcProxy`, `Grpc.GrpcChannel`, `Grpc.RequestType`, and `Grpc.ResponseType` annotations are MP-only
compatibility annotations. They remain in the SE gRPC API for Helidon 27 compatibility, but are deprecated for future
removal from this API.
