...
To add a declarative Health Check, create a service that implements `io.helidon.health.HealthCheck` (or is a factory of it).
The WebServer Health observer will find all such services and use them to provide Health response. The lookup is done only once
(i.e. you must not use `@Service.PerRequest` scope, recommended is `@Service.Singleton` scope).
