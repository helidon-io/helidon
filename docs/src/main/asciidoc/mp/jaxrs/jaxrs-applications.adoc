...
The application path, also known as context path, is the base URI used to serve all resource URIs provided
by `@Path` annotation. This section describe how to set it with annotation or configuration files.
...
The served resources can be reached through `/myApplication/{myResources}` endpoint. It can be overridden
...
[source,properties]
----
io.helidon.examples.MyApplication.routing-path.path=/myApplication
----

If no `Application` is not provided, a _synthetic_ subclass is created and can be configured using
this property:

[source,properties]
----
jakarta.ws.rs.core.Application.routing-path.path=/myApplication
----
