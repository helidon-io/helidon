...
<3> The OCI authentication token. See link:{oci-doc-url}/Content/Registry/Tasks/registrygettingauthtoken.htm[Getting an Auth Token]
...
----
<1> The `oci.test-stream.name` value you defined earlier in application.yaml.
<2> ID for the stream group
<3> Kafka client's property link:{kafka-client-base-url}#consumerconfigs_bootstrap.servers[bootstrap.servers] configuration for all channels using the connector. The format is ``${oci.test-stream.endpoint}:${oci.test-stream.port}``.
<4> username in the format ``${oci.tenant}/${oci.user}/${oci.test-stream.streampool-ocid}``.
<5> OCI authentication token.
