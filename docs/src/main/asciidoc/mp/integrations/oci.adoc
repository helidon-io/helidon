...
To configure authentication, add the ``oci.auth-strategy`` property to ``/server/src/main/resources/META-INF/microprofile-config.properties``
...
ocimetrics:
  initialDelay: 30 <3>
  delay: 180 <4>
  scopes: [base, vendor, application] <5>
  enabled: true <6>
----
...
----
oci.monitoring.compartmentId=cmpt-id <1>
