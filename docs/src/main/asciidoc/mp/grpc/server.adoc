...
== Overview
The gRPC MicroProfile (MP) API is an extension to xref:{rootdir}/mp/introduction.adoc[Helidon MP]
that enables building gRPC services integrated with MicroProfile APIs. Using Helidon MP simplifies
the process of developing gRPC services compared to traditional approaches. Services can be
implemented as plain POJOs, which are then automatically discovered and deployed at
runtime—just like other Helidon MP web services.

Building gRPC services with Helidon gRPC MP is straightforward, allowing developers to focus
on application logic rather than boilerplate code.
...
the link:{helidon-github-examples-url}/microprofile/grpc[gRPC MP Example]
...
link:{helidon-github-examples-url}/microprofile/grpc/pom.xml[pom.xml]
...
gRPC protocol, so add `helidon-microprofile-grpc-security` to secure MP gRPC services.

MP gRPC security in this Helidon version is configuration-based. Security annotations on
gRPC service classes or methods, such as `@RolesAllowed`, are not interpreted by the MP
gRPC integration. Use `grpc.grpc-services.security` configuration to select the secured
...
about gRPC configuration in SE, see
