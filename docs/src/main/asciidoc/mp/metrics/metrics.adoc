...
The following example sends the OTLP metrics data once a second to a Jaeger backend server running locally at the configured endpoint.
...
<4> Configures an OTLP metrics exporter to transmit metrics data to a local Jaeger backend.
