...
For your project to also emit the OpenTelemetry HTTP server semantic conventions, add the following dependency:
[source,xml]
----
<dependency>
...
Your project can export OpenTelemetry metrics that comply with the OpenTelemetry metrics semantic conventions.

First, include the additional dependency on `helidon-webserver-observe-telemetry-metrics` described above.

Next, configure OpenTelemetry to transmit the metrics data from your service to a backend using Helidon configuration. This step is necessary because for the semantic conventions metrics Helidon must use the OpenTelemetry metrics API, not the Microprofile Metrics API; the semantic conventions use the dot "." character in metric and tag names, an illegal character in MP Metrics.

The following example sends OTLP metrics data once a second to a Prometheus server running locally at the configured
endpoint. Start Prometheus with the `--web.enable-otlp-receiver` option to enable its OTLP receiver.

NOTE: Although these settings might be suitable for a test or demonstration, you should use different settings--certainly a longer interval--for a production deployment. See the xref:../../se/telemetry/open-telemetry.adoc[Helidon Telemetry documentation] for more information.

[source,yaml]
.Example configuration for a test of demo of OpenTelemetry metrics semantic conventions
----
telemetry:                              # <1>
...
<4> Configures an OTLP metrics exporter to transmit metrics data to a local Prometheus server.
<5> Configures a metrics reader which gathers data periodically, once per the configured interval, and uses the configured exporter to transmit that gathered metrics data.
<6> Enables the updated HTTP metrics semantic-conventions behavior. It defaults to `false` to preserve the output from earlier Helidon 4.x releases.
