...
Currently, Helidon on OCI is only supported on Helidon MP.
...
. Go to the link:{https://helidon.io/starter/4.2.1}[Helidon Project Starter] page on the Helidon website.
...
Alternatively, you can also use the Helidon OCI Maven archetype or the Helidon CLI to generate the Helidon MP OCI project instead.
...
You can deploy the Helidon application on either OCI OKE or OCI Compute. Depending on your requirements, one option might be better suited to your needs than the other.

* **Oracle Cloud Infrastructure Compute** lets you provision and manage Compute hosts, known as instances, to run your applications. Instances can be either bare metal or virtual machines (VM). See link:{oci-doc-url}/Content/Compute/home.htm[Compute] in OCI documentation for more information.

* **Oracle Cloud Infrastructure Kubernetes Engine (OKE)** is a fully-managed, scalable, and highly available service that you can use to deploy your containerized applications to the cloud. See link:{oci-doc-url}/Content/ContEng/home.htm[Kubernetes Engine] in OCI documentation for more information.


Once you have selected your OCI environment, you can get started with deploying your Helidon applications:
...
The Helidon MP OCI Project Starter is already configured to integrate with several common OCI services but the Helidon OCI SDK extension supports injecting clients for any link:{oci-javasdk-url}#Services_Supported[OCI service supported by the OCI SDK for Java].

See xref:../integrations/oci.adoc[OCI Integration] for more information on how to configure additional OCI services with your Helidon application.
