...
.. To that `@OPTIONS` Java method add a Helidon link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.html[`@Cors.*`] annotation(s) that describes the cross-origin sharing you want for that resource.

[NOTE]
.Using @Cors.* Correctly
====
Use the `@Cors.*` annotations _only_ on methods which also have the `@OPTIONS` annotation. Remember that the CORS settings apply to a given path and therefore to all Java resource methods which share that path.

Helidon MP aborts the server start-up if you use the `@Cors.*` annotations on a resource method other than an `@OPTIONS` method.

For an informal look at the reasons for applying the `@Cors.*` annotations to the `@OPTIONS` method, instead of another
method, see xref:{rootdir}/mp/cors/why-options.adoc[Why `@OPTIONS`?].
====

The following annotations are available:

- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.Defaults.html[`@Cors.Defaults] - has no values, applies all defaults (do not combine with annotations below)
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.AllowedOrigins.html[`@Cors.AllowedOrigins] - value is the allowed origins, defaults to all origins
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.AllowedHeaders.html[`@Cors.AllowedHeaders] - value is the allowed HTTP header names, defaults to all headers
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.AllowedMethods.html[`@Cors.AllowedMethods] - value is the allowed HTTP method names, defaults to all methods
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.ExposeHeaders.html[`@Cors.ExposeHeaders] - value is the exposed HTTP header names, defaults to none
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.AllowCredentials.html[`@Cors.AllowCredentials] - value is a boolean, defaults to false
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.MaxAge.html[`@Cors.MaxAge] - value is the max age as a Duration pattern, defaults to `PT1H` (1 hour)
...
<5> The `enabled` setting lets you control whether the system uses that set of CORS configuration. Defaults to `true`.
...
<4> New `@OPTIONS` method for `/greet`. (Just like the `@GET` method `getDefaultMessage`, this `@OPTIONS` method does not have a `@Path` annotation; both "inherit" the class-level `@Path` setting `/greet`.) The `@Cors.Defaults` annotation declares default cross-origin sharing which permits sharing via all HTTP methods to all origins.
<5> New `@OPTIONS` method for `/greet/greeting`. The `@Cors.AllowedMethods` annotations specifies sharing only via the `PUT` HTTP method, and the `@Cors.AllowedOrigins` specifies sharing only to the two listed origins.
