...
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.AllowOrigins.html[`@Cors.AllowOrigins] - value is the allowed origins, defaults to all origins
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.AllowHeaders.html[`@Cors.AllowHeaders] - value is the allowed HTTP header names, defaults to all headers
- link:{webserver-cors-javadoc-base-url}/io/helidon/webserver/cors/Cors.AllowMethods.html[`@Cors.AllowMethods] - value is the allowed HTTP method names, defaults to all methods
...
<5> New `@OPTIONS` method for `/greet/greeting`. The `@Cors.AllowMethods` annotations specifies sharing only via the `PUT` HTTP method, and the `@Cors.AllowOrigins` specifies sharing only to the two listed origins.
