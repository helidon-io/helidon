...
There are some good reasons why it is `@OPTIONS` methods that you decorate with the Helidon
`@Cors.*` annotations. Take an informal look at the rationale for this choice.
...
In Helidon, the `@Cors.*` annotations map directly to those CORS sharing attributes.
It would be natural, then, to use `@Cors.*` to annotate the single Java element in the application that represents a resource.
...
So we could annotate any one of those endpoint methods with `@Cors.*` and unambiguously link
...
the `@OPTIONS` method is a reasonable place to ask you to annotate with `@Cors.*` annotations.
