...
is in the list of features. The GraphQL endpoint is available at `http://host:port/graphql`, and
the corresponding schema at `http://host:port/graphql/schema.graphql`, once access is authenticated
or `graphql.permit-all` is set to `true`. See <<Configuration>> for additional information on how
to change the location of these resources.
