...
ifdef::se-flavor[]
:jaeger-metrics-page: {rootdir}/se/tracing/jaeger-metrics.adoc
endif::[]
ifdef::mp-flavor[]
:jaeger-metrics-page: {rootdir}/tracing.adoc#jaeger-tracing-metrics
endif::[]

Jaeger tracks its own behavior using metrics. See xref:{jaeger-metrics-page}[Metrics Support for Jaeger] to read
 how to integrate Jaeger metrics with Helidon.
