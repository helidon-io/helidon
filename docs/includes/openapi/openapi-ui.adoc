...
== Usage
ifdef::se-flavor[]
Make sure your application creates a Helidon `OpenAPISupport` instance and registers it for routing (described in detail on link:{openapi-page}[the Helidon OpenAPI page]). `OpenAPISupport` automatically prepares the OpenAPI U/I if you also declare a dependency on the Helidon OpenAPI U/I integration component as explained above. The <<API,API>> section below illustrates adding OpenAPI to your application and customizing the U/I behavior.
endif::se-flavor[]
...
The example below shows the U/I if you modify the Helidon QuickStart {flavor-uc} greeting application.
...
Helidon uses standard content negotiation at `/my-openapi`.
...
[NOTE]
====
The `ui.web-context` setting assigns the _entire_ web-context for the U/I, not the suffix appended the `OpenAPISupport` endpoint.
====
