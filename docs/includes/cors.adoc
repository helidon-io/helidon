...
|boolean |`false`|Sets the allow credentials flag. Enabled configs cannot use wildcard origins. |`Access-Control-Allow-Credentials`
...
If `allow-credentials` is `true`, configure explicit allowed origins instead of the default wildcard origin.
