...
[[tenant-enable]]
=== Multiple tenants
OIDC provider also supports multiple tenants. To enable this feature it is required to do several steps.

1. To enable default multi tenant support, add `multi-tenant: true` option to your
OIDC provider configuration
2. Specify desired way of how to provide tenant name information. This is done over adding
`tenant-id-style` config option. More information can be found in the table below
3. Add `tenants` section to the OIDC provider config
...
There are 4 ways how to provide required tenant information to Helidon by default.
...
|`token-handler`
|Tenant name information is expected to be provided via configured custom header value
|[source, yaml]
----
tenant-id-handler:
...
It is also possible to implement custom way of discovering the tenant name and tenant configuration.
Custom tenant name discovery from request can be done via implementing SPI:

`io.helidon.security.providers.oidc.common.spi.TenantIdProvider`

and custom tenant configuration discovery can be provided via implementing SPI:
