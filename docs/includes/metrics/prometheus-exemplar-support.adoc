...
Add Helidon {h1-prefix} support for  link:https://github.com/OpenObservability/OpenMetrics/blob/main/specification/OpenMetrics.md#exemplars[OpenMetrics (Prometheus) exemplars] for histograms, counters, and simple timers to your application simply by adding dependencies to your project's `pom.xml`.
...
ifdef::se-flavor[]
Also, include either xref:{rootdir}/se/tracing/zipkin.adoc[Helidon Zipkin] or
xref:{rootdir}/se/tracing/jaeger.adoc[Helidon Jaeger] support:
endif::[]
ifdef::mp-flavor[]
Also, include either xref:{rootdir}/mp/tracing.adoc#zipkin-tracing[Helidon Zipkin] or
xref:{rootdir}/mp/tracing.adoc#jaeger-tracing[Helidon Jaeger] support:
endif::[]
...
----
<1> The exemplar for a `SimpleTimer` is the same for the `total` and the `elapsedTime` sub metrics: always the most recent sample which updated the `SimpleTimer`.
