...
Also, include the Helidon integration module for a tracing implementation...for example
ifdef::se-flavor[]
xref:{rootdir}/se/tracing/zipkin.adoc[Helidon Zipkin]
endif::[]
ifdef::mp-flavor[]
...
ifdef::se-flavor[]
include::{rootdir}/se/tracing/tracing.adoc[tag=tracing-dependency]
endif::[]
ifdef::mp-flavor[]
...
ifdef::se-flavor[]
Helidon includes an example application, based on the QuickStart application, which link:{helidon-github-tree-url}/examples/metrics/exemplar[illustrates exemplar support].
endif::[]
