...
include::{rootdir}/includes/config/io.helidon.metrics.serviceapi.MetricsSupport.adoc[leveloffset=-1,settings]
...
* xref:config-disable[Disable metrics entirely.]
* xref:config-selective[Selectively enable or disable metrics by metric registry type and, within type, by name.]
* xref:config-kpi[Choose whether to collect extended key performance indicator metrics.]
* xref:config-rest-request[Control `REST.request` metrics collection.]
