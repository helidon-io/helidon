...
/**
 * A builder for tracing {@link io.opentracing.Tracer tracer}.
 * <p>
 * Each tracer implementation may use different options, though these are the common ones.
 * To keep service implementation abstracted from specific tracer, use {@link #config(io.helidon.config.Config)}
...
public interface OpenTracingTracerBuilder<T extends OpenTracingTracerBuilder<T>> extends Builder<T, Tracer> {
     ...
     * @return a new builder instance
     */
    static OpenTracingTracerBuilder<?> create(Config config) {
        return OpenTracingProviderHelper.findTracerBuilder()
                .config(config);
    }
