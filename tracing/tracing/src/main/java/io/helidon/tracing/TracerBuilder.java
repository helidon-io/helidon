...
public interface TracerBuilder<T extends TracerBuilder<T>> extends Builder<T, Tracer> {
    ...
    static TracerBuilder<?> create(io.helidon.common.config.Config config) {
        return TracerProviderHelper.findTracerBuilder()
                .config(config);
    }
    ...
    default T config(Config config) {
        ...
        DeprecationSupport.requireOverride(this, TracerBuilder.class, "config", io.helidon.common.config.Config.class);
