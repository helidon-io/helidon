...
class ImperativeJdbcClientTest {

    @Test
    void executesImperativeQueriesUpdatesGeneratedKeysAndNullBinding() {
        ServiceRegistryManager manager = ServiceRegistryManager.start();
        try {
            ImperativeContactStore contacts = manager.registry().get(ImperativeContactStore.class);
            ...
            long id = contacts.insert("imperative", null);
            assertThat(contacts.findByName("imperative"),
                       is(Optional.of(new ContactView(id, "imperative", Optional.empty()))));
            assertThat(contacts.rename(id, "imperative-renamed"), is(1L));
            assertThat(contacts.findByName("imperative"), is(Optional.empty()));
            assertThat(contacts.findByName("imperative-renamed"),
                       is(Optional.of(new ContactView(id, "imperative-renamed", Optional.empty()))));
        } finally {
            manager.shutdown();
        }
    }
    ...
    @Test
    void participatesInLocalTransactions() {
        ServiceRegistryManager manager = ServiceRegistryManager.start();
        try {
            ImperativeContactStore contacts = manager.registry().get(ImperativeContactStore.class);

            assertThrows(TxException.class, () -> Tx.transaction(Tx.Type.REQUIRED, () -> {
                contacts.insert("imperative-rolled-back", "rollback@example.test");
                throw new IllegalStateException("force rollback");
            }));

            assertThat(contacts.findByName("imperative-rolled-back"), is(Optional.empty()));

            Tx.transaction(Tx.Type.REQUIRED, () -> {
                contacts.insert("imperative-committed", "commit@example.test");
                return null;
            });
            assertThat(contacts.findByName("imperative-committed").isPresent(), is(true));
        } finally {
            manager.shutdown();
        }
    }
