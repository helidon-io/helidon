...
import io.helidon.data.annotation.event.PrePersist;
...
/**
 * Functional version or {@link io.helidon.data.annotation.event.PrePersist} event handlers.
 *
...
public interface PrePersistEventListener<T> extends EntityEventListener<T> {
     ...
     */
    boolean prePersist(T entity);

    @Override
    default boolean prePersist(EntityEventContext<T> context) {
        return prePersist(context.getEntity());
    }
