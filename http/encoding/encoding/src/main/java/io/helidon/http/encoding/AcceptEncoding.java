...
public final class AcceptEncoding {
    ...
    private static final AcceptEncoding INVALID = new AcceptEncoding(true, false, Map.of(), null);
    ...
    public static AcceptEncoding create(Headers headers) {
        ...
        addStandardEquivalentEntries(entries);

        return new AcceptEncoding(true, true, Collections.unmodifiableMap(entries), wildcard);
    }
    ...
    public Optional<CodingQuality> best(List<String> codings) {
        ...
        for (int i = 0; i < codings.size(); i++) {
            ...
            boolean equivalentAlias = entry != null && !coding.equals(entry.coding());
            BestCandidate candidate = new BestCandidate(q, wildcardMatch, equivalentAlias, order, i);
    ...
    private static void addStandardEquivalentEntries(Map<String, Entry> entries) {
        addEquivalentEntry(entries, "gzip", "x-gzip");
        addEquivalentEntry(entries, "compress", "x-compress");
    }

    private static void addEquivalentEntry(Map<String, Entry> entries, String firstCoding, String secondCoding) {
        Entry first = entries.get(firstCoding);
        Entry second = entries.get(secondCoding);
        if (first == null && second != null) {
            entries.put(firstCoding, second);
        } else if (second == null && first != null) {
            entries.put(secondCoding, first);
        }
    }
    ...
    private static boolean betterBestCoding(BestCandidate candidate, BestCandidate current) {
        ...
        if (candidate.equivalentAlias() != current.equivalentAlias()) {
            return !candidate.equivalentAlias();
        }
    ...
    private static CodingQuality codingQuality(String coding, Entry entry, int q, int order, boolean wildcard) {
        if (entry != null && !wildcard && coding.equals(entry.coding())) {
    ...
    private List<CodingQuality> createAcceptedCodings() {
        ...
        for (Map.Entry<String, Entry> mappedEntry : entries.entrySet()) {
            Entry entry = mappedEntry.getValue();
            if (!mappedEntry.getKey().equals(entry.coding())) {
                continue;
            }
    ...
    private record BestCandidate(int q, boolean wildcard, boolean equivalentAlias, int order, int serverOrder) {
    }
