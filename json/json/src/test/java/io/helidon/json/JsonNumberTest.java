...
class JsonNumberTest {
    ...
    @Test
    void classifiesExplicitPositiveExponentExpansion() {
        assertRequiresExpansion("1.23e+2", false);
        assertRequiresExpansion("1.23E+2", false);
        assertRequiresExpansion("1.23e+3", true);
        assertRequiresExpansion("1.23E+3", true);
    }
    ...
    private static void assertRequiresExpansion(String value, boolean expected) {
        byte[] bytes = value.getBytes(StandardCharsets.US_ASCII);
        assertThat(JsonNumber.requiresExpansion(bytes, 0, bytes.length), is(expected));
    }
