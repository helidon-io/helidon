...
class Continue100Test {
    ...
    static void beforeAll() {
        ...
        Routing routing = Routing.builder()
                ...
                .any("/", (req, res) -> {
                    if (Boolean.parseBoolean(req.headers().first(Http.Header.create("test-fail-before-read")).orElse("false"))) {
                        res.status(Http.Status.EXPECTATION_FAILED_417).send();
                        return;
                    }

                    Optional<String> blockId = req.headers().first(Http.Header.create("test-block-id"));
    ...
    void continue100ImmediatelyPost() throws Exception {
        ...
        try (SocketHttpClient socketHttpClient = SocketHttpClient.create(contImmediatelyPort)) {
            String content = "looong payload data";
            socketHttpClient
                    .manualRequest("""                            
                            POST / HTTP/1.1
