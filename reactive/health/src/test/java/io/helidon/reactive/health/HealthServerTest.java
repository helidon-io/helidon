...
class HealthServerTest {
    ...
    private static void checkResponse(Supplier<WebClientRequestBuilder> requestFactory,
        ...
        try {
            ...
            Http.Status expectedStatus = expectContent ? Http.Status.OK_200 : Http.Status.NO_CONTENT_204;
            assertThat("health response status", response.status(), is(expectedStatus));
