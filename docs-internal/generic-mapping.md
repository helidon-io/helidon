...
TODO an alternative name would be `Converter` instead of `Mapper`
...
The mapping function gets either a pair of `Class` objects, or a pair of `GenericType` objects
 that define the `SOURCE` and the `TARGET` of the mapping.
 
For `Class` parameteres, lookup is done as follows:
