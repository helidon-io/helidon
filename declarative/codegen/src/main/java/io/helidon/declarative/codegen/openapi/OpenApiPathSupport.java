    ...
    private static PathTemplate validateOpenApiPathTemplate(RestMethod method, String path, PathValidationMode mode) {
        ...
        for (int i = 0; i < path.length(); i++) {
            ...
            case '{' -> {
                ...
                if (!pathParameters.add(parameter.name())) {
                    throw openApiPathException(method,
                                               path,
                                               "path parameter '" + parameter.name() + "' appears more than once",
                                               mode);
                }
                result.append('{')
            ...
            case '?', '#' -> throw openApiPathException(method,
                                                        path,
