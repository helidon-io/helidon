...
class ChannelMessagingTypes {
    ...
    static class ShutdownMessagingConnectorProvider implements MessagingConnectorProvider {
        ...
        static void reset() {
            SOURCE_STARTED.set(new CountDownLatch(1));
        }

        static boolean awaitSourceStarted() throws InterruptedException {
            return SOURCE_STARTED.get().await(5, TimeUnit.SECONDS);
        }
