...
class DeclarativeMessagingTest {
    ...
    void testGeneratedEmitterWrapsCheckedHandlerFailure() {
        ...
        BatchDeliveryException failure = assertBatchFailure(
                consumer.failure(),
                () -> producer.emitCheckedFailingChannel("test message"));

        assertThat(failure.getCause(), instanceOf(SupplierException.class));
        assertThat(failure.getCause().getMessage(), containsString("Messaging handler"));
    }
    ...
    void testIncomingConnectorRetriesRuntimeHandlerFailure() throws InterruptedException {
        ...
        BatchDeliveryException failure = assertBatchFailure(consumer.failure(), thrown);
        assertThat(failure.getCause(), sameInstance(consumer.failure()));
        assertThat(consumer.attempts(), is(3));
    }
    ...
    void testTypedHeaderValuesAreInjectedWithoutConversion() {
                ...
                assertThrows(BatchDeliveryException.class,
                             () -> runtime.emit(ChannelMessagingTypes.TYPED_HEADER_CHANNEL,
                                                Message.builder("missing required")
                                                        .header("optional",
                                                                MessageHeaderValue.TextValue.create("present"))
                                                        .addHeader("repeated",
                                                                   MessageHeaderValue.IntegerValue.create(1))
                                                        .build()));
        ...
        List<MessageHeaderValue> repeated = List.of(
                ...
                MessageHeaderValue.BinaryValue.create(new byte[] {1, 2}),
                MessageHeaderValue.BooleanValue.create(true),
                MessageHeaderValue.IntegerValue.create(42),
                MessageHeaderValue.DecimalValue.create(new BigDecimal("12.30")),
                MessageHeaderValue.Float32Value.create(1.5F),
                MessageHeaderValue.Float64Value.create(2.5D),
                MessageHeaderValue.TimestampValue.create(Instant.parse("2026-08-26T10:15:30Z")),
        ...
        MessageConfig.Builder<String> message = Message.builder("typed values")
                ...
                .addHeader("optional", MessageHeaderValue.NullValue.create());
        for (int i = 0; i < repeated.size(); i++) {
            message.addHeader("repeated", repeated.get(i));
            if (i == 4) {
                message.addHeader("interleaved", MessageHeaderValue.TextValue.create("ignored"));
            }
        }
        ...
        runtime.emit(ChannelMessagingTypes.TYPED_HEADER_CHANNEL,
                     Message.builder("explicit null")
                             .header("required", MessageHeaderValue.NullValue.create())
                             .build());

        assertThat(consumer.deliveries(),
                   is(List.of(new TypedHeaderDelivery("typed values",
                                                      MessageHeaderValue.IntegerValue.create(99),
                                                      Optional.of(MessageHeaderValue.NullValue.create()),
                                                      repeated),
        ...
        assertThrows(UnsupportedOperationException.class,
                     () -> consumer.deliveries()
                             .getFirst()
                             .repeated()
                             .add(MessageHeaderValue.TextValue.create("mutable")));
    }
