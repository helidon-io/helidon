...
class CorsEndpoint2 {
    ...
    @Cors.AllowedOrigins("${app.cors.allow-origins:http://foos.bar,http://bars.foo}")
    @Cors.AllowedHeaders({"X-foo", "X-bar"})
    @Cors.AllowedMethods({Method.DELETE_NAME, Method.PUT_NAME, "LIST"})
    @Cors.AllowCredentials
    @Cors.MaxAgeSeconds(0)
    void options(ServerResponse response) {
