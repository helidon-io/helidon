...
class DeclarativeOpenApiTest {
    ...
    @Test
    void generatedDocumentUsesBuiltInBigIntegerSchema() {
        Map<String, Object> operation = operation(document(), "/greetings/big-integer", "get");
        Map<String, Object> schema = content(response(operation, "200"), MediaTypes.APPLICATION_JSON_VALUE);

        assertThat(schema.get("type"), is("integer"));
        assertThat(schema, not(hasKey("$ref")));
    }
    ...
    void generatedDocumentInfersRequestBodyStatusAndOptionalResponses() {
        ...
        assertThat(inferredBody.get("required"), is(true));
        ...
        String expectedCookieStyle = expectedOpenApiVersion().startsWith("3.2") ? "cookie" : "form";
        ...
        assertThat(session.get("style"), is(expectedCookieStyle));
        ...
        assertThat(tracking.get("style"), is(expectedCookieStyle));
