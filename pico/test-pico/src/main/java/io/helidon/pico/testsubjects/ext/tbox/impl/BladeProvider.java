...
public class BladeProvider implements InjectionPointProvider<AbstractBlade> {
    ...
    @Override
    public AbstractBlade get(InjectionPointInfo ipInfoCtx, ServiceInfo criteria, boolean expected) {
        assert (Objects.nonNull(ipInfoCtx));
        assert (Objects.nonNull(criteria));
        assert (!ipInfoCtx.listWrapped());
    ...
    @Override
    public List<AbstractBlade> getList(InjectionPointInfo ipInfoCtx, ServiceInfo criteria, boolean expected) {
        assert (Objects.nonNull(ipInfoCtx));
        assert (Objects.nonNull(criteria));
        assert (ipInfoCtx.listWrapped());

        List<AbstractBlade> result = new LinkedList<>();
        if (criteria.qualifiers().contains(all) || criteria.qualifiers().contains(coarse)) {
