 ...
 */
public class ConfigDrivenProcessor extends PicoAnnotationProcessor {
    ...
    private static final TypeName NAMED_INSTANCE = TypeName.create("io.helidon.pico.configdriven.api.NamedInstance");
    ...
    private String generateCreateConfigBeans(ConfigBean configBean, TypeName namedInstanceWithGeneric) {
        ...
        if (atLeastOne) {
            ...
            method.append("\t\t\treturn List.of(new io.helidon.pico.configdriven.api.NamedInstance<>(")
                    .append(configBean.typeName().fqName())
                    .append(".create(io.helidon.common.config.Config.empty()), io.helidon.pico.configdriven.api.NamedInstance"
                                    + ".DEFAULT_NAME));\n");
        } else {
