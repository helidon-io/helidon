 ...
 * {@link io.helidon.pico.config.api.ConfigBean}s that are active, along with whether the application is configured to
...
public interface ConfigBeanRegistry {
     ...
     *
     * @return true if the config bean registry has been initilized
     */
    boolean isReady();
     ...
     * not associated with config beans. Use {@link #getConfiguredServiceProviders()} for configured service instances.
     *
     * @param <T> the service type
     * @param <CB> the config bean type
     * @return the map of configurable services to the meta config beans each expects.
     */
    <T, CB> Map<ConfiguredServiceProvider<T, CB>, ConfigBeanInfo> getConfigurableServiceProviders();
     ...
     */
    <T, CB> List<ConfiguredServiceProvider<T, CB>> getConfiguredServiceProviders();

    /**
     * Returns all the know config beans in order of rank given the short config key / alias. Callers should understand
     * that this list might be incomplete until ready state is reached (see {@link #isReady()}.
     *
     * @param key           the config options key - note that this is a partial key - and not relative to the parent - the same key used by {@link io.helidon.pico.config.api.ConfigBean#key()}.
     * @param fullConfigKey optionally, the full config key for the config - if not passed will return the list of all matches using just the key
     * @param <CB> the config bean type
     * @return the list of known config keys
     */
    <CB> List<CB> getConfigBeansByConfigKey(String key, String fullConfigKey);

    /**
     * Similar to {@link #getConfigBeansByConfigKey(String, String)}, but instead returns all the know config beans in a map
     ...
     * @return the list of known config keys
     */
    <CB> Map<String, CB> getConfigBeanMapByConfigKey(String key, String fullConfigKey);
     ...
     */
    <CB> Map<String, CB> getAllConfigBeans();
