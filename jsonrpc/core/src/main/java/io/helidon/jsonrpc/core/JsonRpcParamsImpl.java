...
public class JsonRpcParamsImpl implements JsonRpcParams {
    ...
    public <T> T as(Class<T> type) throws Exception {
        return JsonUtil.jsonpToJsonb(params.asJsonObject(), type);
    }

    @Override
    public <T> T getAs(String name, Class<T> type) {
        return JsonUtil.jsonpToJsonb(get(name).asJsonObject(), type);
    }

    @Override
    public <T> T getAs(int index, Class<T> type) {
        return JsonUtil.jsonpToJsonb(get(index).asJsonObject(), type);
    }
}
