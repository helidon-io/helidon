...
@Readiness
@ApplicationScoped
public class RSocketHealthCheck implements HealthCheck {

    private RSocketHealthCheck() {
        //Private constructor.
    }

    /**
     * Create new Health check for RSocket.
     *
     * @return RSocketHealthCheck
     */
    public static RSocketHealthCheck create() {
        return new RSocketHealthCheck();
    }
    ...
    private HealthCheckResponse runHealthCheckQuery(HealthCheckResponseBuilder builder) {
        ...
        for (Map.Entry<String, ServerTransport.ConnectionAcceptor> entry : connectionAcceptorMap.entrySet()) {
            responseBuilder.withData(entry.getKey(), "UP");
        }
        return responseBuilder.up().build();
    }

    /**
     * Get the HealthCheckResponse.
     *
    ...
    public HealthCheckResponse call() {
        ...
        var builder = HealthCheckResponse.named("RSocket connection health check");
        try {
