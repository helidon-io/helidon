...
class MainClassCreator {
    ...
    private void codegen(CompilerOptions compilerOptions, WrappedServices services, TypeName generatedType) {
        ...
        classModel.addMethod(main -> main
                ...
                .addParameter(args -> args
                        .type(String[].class)
                        .name("args"))
                .update(it -> mainMethodBody(classModel, it, services)));
    ...
    private void mainMethodBody(ClassModel.Builder classModel, Method.Builder main, WrappedServices services) {
        main.addContentLine("var config = configBuilder(args);")
                .addContentLine("");
        ...
        serviceLoaded.stream()
                ...
                .forEach(it -> addServiceLoader(classModel, main, providerConstants, constantCounter, it));
        ...
        serviceDescriptors.stream().sorted()
                .forEach(it -> main.addContent("config.addServiceDescriptor(")
                        .addContent(it)
                        .addContentLine(".INSTANCE);"));

        main.addContentLine("")
                .addContentLine("init(config.build());");
    }
    ...
    private void addServiceLoader(ClassModel.Builder classModel,
        ...
        //                                           TypeName.create("io.helidon.config.yaml.YamlConfigParser"),
        //                                           90.0));
        ...
        main.addContent("config.addServiceDescriptor(serviceLoader(")
                .addContent(constantName)
                .addContentLine(",")
                .increaseContentPadding()
                .increaseContentPadding()
                .increaseContentPadding()
                .addContentCreate(sl.serviceType())
                .addContentLine(",")
