...
class UnsupportedTypesExtension implements InjectCodegenExtension {
    ...
    UnsupportedTypesExtension(ServiceCodegenContext ctx) {
        this.ctx = ctx;
    }
    ...
    public void process(RoundContext roundContext) {
        ...
        if (ServiceOptions.IGNORE_UNSUPPORTED_ANNOTATIONS.value(ctx.options())) {
            // not interested
        ...
        if (toProcess.contains(CdiTypes.APPLICATION_SCOPED)) {
            msg.append("'")
                    ...
                    .append(INJECTION_INJECT.fqName())
        ...
        msg.append("Use -A")
                .append(ServiceOptions.IGNORE_UNSUPPORTED_ANNOTATIONS)
