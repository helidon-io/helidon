...
public class CliExpertTest {
    ...
    CliExpertTest(@Service.Named("mock-mcp-test-model") MockChatModel mockChatModel) {
        mockChatModel.activeRules().add(new CliToolCallingRule());
    }

    @Test
    void cliExpertRespondsWithCommand() {
        var response = Services.get(CliExpert.class).answer(PROMPT);
