...
class InMemoryEmbeddingStoreFactory implements Service.ServicesFactory<EmbeddingStore<TextSegment>> {
    ...
    InMemoryEmbeddingStoreFactory(Config config) {
        this.config = config;
        this.embeddingStoreNames = ConfigUtils.modelNames(config, ConfigUtils.Kind.EMBEDDING_STORE, "helidon");
    }
    ...
    public static Optional<EmbeddingStore<TextSegment>> create(String name, Config c) {
        var mergedConfig = ConfigUtils.create(c, ConfigUtils.Kind.EMBEDDING_STORE, name);
