...
public class UCPBackedDataSourceExtension extends AbstractDataSourceExtension {
    ...
    private static PoolDataSource createDataSource(final Instance<Object> instance,
        ...
        if (!propertyNames.isEmpty()) {
            ...
            for (final String propertyName : propertyNames) {
                if (propertyName != null) {
                    ...
                    if (!handled) {
                        ...
                        //
                        // (Sadly, "serviceName" and "pdbRoles" are properties that are special to certain
                        // connection factories (such as Oracle database-oriented DataSources), and to the
                        // oracle.ucp.jdbc.UCPConnectionBuilder class, which underlies getConnection(user,
                        ...
                        // below, not here.)
                        //
                        // Sadly, the Universal Connection Pool cannot tunnel arbitrary Java Beans-conformant
                        // property values destined for the underlying connection factory (which is usually a
                        // DataSource or ConnectionPoolDataSource implementation, but may be other things)
                        // through to that underlying connection factory with arbitrary type information set
                        // properly.  Because the PoolDataSource is in charge of instantiating the connection
                        // factory, you can't pass a fully configured DataSource into it, nor can you access an
                        // unconfigured instance of it that you can work with. The only configuration the
                        // Universal Connection Pool supports is via a Properties object, whose values are
                        // retrieved by the PoolDataSource implementation, as Strings.  This limits
                        // the kinds of underlying connection factories (DataSource implementations,
                        // usually) that can be fully configured with the Universal Connection Pool to Strings
                        // and those Strings which can be converted by the PoolDataSourceImpl#toBasicType(String,
                        // String) method.
            ...
            // Set the PoolDataSource's serviceName property so that it appears to have been set via the undocumented XML
            // configuration that the PoolDataSource can apparently be configured with in certain (irrelevant for Helidon)
            // application server cases.
            ...
            // Set the PoolDataSource's pdbRoles property so that it appears to have been set via the undocumented XML
            // configuration that the PoolDataSource can apparently be configured with in certain (irrelevant for Helidon)
            // application server cases.
    ...
    private void cleanup(@Observes final AfterDeploymentValidation event) {
        this.xa.clear();
    }
