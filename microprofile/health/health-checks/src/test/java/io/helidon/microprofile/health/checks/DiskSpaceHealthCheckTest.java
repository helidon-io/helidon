...
class DiskSpaceHealthCheckTest {
    ...
    @BeforeEach
    void init() {
        path = Mockito.mock(File.class);
        Mockito.when(path.getUsableSpace()).thenReturn(TOTAL_DISK - USED_DISK_START);
        Mockito.when(path.getTotalSpace()).thenReturn(TOTAL_DISK);
    }

    private void setDiskUsage(long used) {
        Mockito.when(path.getUsableSpace()).thenReturn(TOTAL_DISK - used);
    }
    ...
    void testThatHealthCheckNameDoesNotChange() {
        ...
        assertThat("diskSpace", equalTo(response.getName()));
    }

    @ParameterizedTest
    @MethodSource("belowThresholdParams")
    void belowThreshold(long used, String text) {
        setDiskUsage(used);
        DiskSpaceHealthCheck check = new DiskSpaceHealthCheck(path, THRESHOLD_PERCENT);
        HealthCheckResponse response = check.call();
    ...
    void aboveThreshold(long used, String text) {
        setDiskUsage(used);
        DiskSpaceHealthCheck check = new DiskSpaceHealthCheck(path, THRESHOLD_PERCENT);
        HealthCheckResponse response = check.call();
    ...
    void testBytesToUnits(long bytes, String text) {
        assertThat(text, equalTo(DiskSpaceHealthCheck.format(bytes)));
    }
}
