...
public class MetricsTest extends MetricsBaseTest {
    ...
    private static final long PERF_TEST_FAILURE_THRESHOLD_NS = Integer.getInteger(
            "helidon.test.metrics.perf-test-failure-threshold-ns", 400 * 1000 * 1000);
    ...
    public void testCounted2Perf() {
        /*
         * Informal experience shows that, without the performance fix in InterceptorBase, this test measures more than 1 second
         * (1,000,000,000 ns) to perform 10000 intercepted calls to the bean. With the fix in InterceptorBase it takes about
         * .06-.08 seconds.
         */
        CountedBean bean = newBean(CountedBean.class);
