...
class BasicPublisher<K, V> extends EmittingPublisher<KafkaMessage<K, V>> {
     ...
     *
     * @param requestsCallback to notify new requests
     * @param publisher {@link java.util.function.Consumer}
     */
    BasicPublisher(Optional<Callback<Long>> requestsCallback, Consumer<Subscriber<? super KafkaMessage<K, V>>> publisher) {
        super(requestsCallback);
