...
public class CorsCdiExtension implements Extension {
    ...
    private Optional<CorsPathConfig> corsConfig(AnnotatedMethod<?> am, String path) {
        ...
        if (am.isAnnotationPresent(Cors.AllowedHeaders.class)) {
            found = true;
            builder.allowHeaders(Set.of(am.getAnnotation(Cors.AllowedHeaders.class).value()));
        }
        if (am.isAnnotationPresent(Cors.AllowedMethods.class)) {
            found = true;
            builder.allowMethods(Set.of(am.getAnnotation(Cors.AllowedMethods.class).value()));
        }
