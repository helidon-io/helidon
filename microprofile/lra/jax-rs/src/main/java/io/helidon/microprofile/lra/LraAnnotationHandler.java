...
class LraAnnotationHandler implements AnnotationHandler {
    ...
    private URI start(String clientId, PropagatedHeaders headers, long timeOut) {
        return awaitCoordinator(() -> coordinatorClient.start(clientId, headers, timeOut), coordinatorTimeout);
    }

    private URI start(URI parentLraId, String clientId, PropagatedHeaders headers, long timeOut) {
        return awaitCoordinator(() -> coordinatorClient.start(parentLraId, clientId, headers, timeOut), coordinatorTimeout);
    }
    ...
    private void join(ContainerRequestContext reqCtx,
        ...
        awaitCoordinator(() -> coordinatorClient.join(lraId, propagatedHeaders, timeLimit, participant), coordinatorTimeout)
    ...
    private void close(URI lraId, PropagatedHeaders headers) {
        awaitCoordinator(() -> coordinatorClient.close(lraId, headers), coordinatorTimeout);
    }

    private void cancel(URI lraId, PropagatedHeaders headers) {
        awaitCoordinator(() -> coordinatorClient.cancel(lraId, headers), coordinatorTimeout);
    }
