...
class JParser {
    ...
    private JParser(DataReader reader) {
        this.reader = reader;
    }
    ...
    static JValue<?> parse(InputStream stream) {
        ...
        return new JParser(dr).read(true);
    }
    ...
    private JValue<?> readArray() {
        ...
        while (true) {
            ...
            if (next == ARRAY_END) {
                skip();
                // end of array
                return JArray.create(values);
            }

            JValue<?> value = switch (next) {
    ...
    private JValue<?> readValue() {
        ...
        if ("null".equals(value)) {
            return JValues.NullValue.INSTANCE;
        }
        try {
            return JValues.NumberValue.create(new BigDecimal(value));
        } catch (NumberFormatException e) {
