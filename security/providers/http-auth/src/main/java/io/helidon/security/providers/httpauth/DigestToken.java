    ...
    boolean validateLogin(SecureUserStore.User user) {
        return user.digestHa1(realm, algorithm)
                .map(ha1 -> {
                    ...
                    boolean valid = MessageDigest.isEqual(digest.getBytes(StandardCharsets.UTF_8),
                            response.getBytes(StandardCharsets.UTF_8));

                    if (!valid && LOGGER.isLoggable(Level.DEBUG)) {
                        LOGGER.log(Level.DEBUG, "Digest authentication failed");
                    }
