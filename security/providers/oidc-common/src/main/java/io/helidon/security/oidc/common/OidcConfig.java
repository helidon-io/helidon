 ...
 *
 * This configuration class is usable from all resources that utilize OIDC
 *  specification, such as security provider, web server extension and IDCS
 *  connectivity.
 *
 * Some of the configuration options below use "resource" type. The following
 * configuration can be used for a resource (example for oidc-metadata key):
 *
 * [source, yaml]
 * --
 ...
 * --
 *
 * Configuration options required (under `security.providers.providerName`):
 *
 ...
 *
 * | frontend-uri
 * | Fully URI of the frontend for redirects back from OIDC server
 ...
 * | redirect-uri
 * | /oidc/redirect
 * | URI to register web server component on, used by the OIDC server to redirect
 *  authorization requests to after a user logs in or approves scopes. Note that
 *  usually the redirect URI configured here must be the same one as configured
 *  on OIDC server.
 *
 * | scope-audience
 * | empty string
 * | Audience of the scope required by this application. This is prefixed to the
 *  scope name when requesting scopes from the identity server.
 *
 * | cookie-use
 * | true
 * | Whether to use cookie to store JWT. If used, redirects happen only in case
 *  the user is not authenticated or has insufficient scopes
 *
 * | cookie-name
 * | JSESSIONID
 * | Name of the cookie
 ...
 * | cookie-same-site
 * | Strict
 * | When using cookie, used to set the SameSite cookie value. Can be `Strict` or
 *  `Lax`
 *
 * | query-param-use
 * | false
 ...
 * |true
 * | If set to true, metadata will be loaded from default (well known) location,
 *  unless it is explicitly defined using oidc-metadata-resource. If set to false,
 *  it would not be loaded even if oidc-metadata-resource is not defined. In such
 *  a case all URIs must be explicitly defined (e.g. token-endpoint-uri).
 *
 * | oidc-metadata
 * | identity-uri/.well-known/openid-configuration
 * | Resource configuration for OIDC Metadata containing endpoints to various
 *  identity services, as well as information about the identity server
 *
 * | token-endpoint-uri
 * | token_endpoint in OIDC metadata, or identity-url/oauth2/v1/token if not
 *  available
 * | URI of a token endpoint used to obtain a JWT based on the authentication code.
 *
 * | authorization-endpoint-uri
 * | "authorization_endpoint" in OIDC metadata, or identity-uri/oauth2/v1/authorize
 *  if not available|URI of an authorization endpoint used to redirect users to
 *  for logging-in.
 *
 * | validate-with-jwk
 * | true
 * | When true - validate against jwk defined by "sign-jwk", when false validate
 *  JWT through OIDC Server endpoint "validation-endpoint-uri"
 *
 * |sign-jwk
 * |"jwks-uri" in OIDC metadata, or identity-uri/admin/v1/SigningCert/jwk if not
 *  available, only needed when jwt validation is done by us|A resource pointing
 *  to JWK with public keys of signing certificates used to validate JWT
 *
 * | introspect-endpoint-uri | "introspection_endpoint" in OIDC metadata, or
 *  identity-uri/oauth2/v1/introspect|When validate-with-jwk is set to "false",
 *  this is the endpoint used
 *
 * | base-scopes
 * | {@value #DEFAULT_BASE_SCOPES}
 * | Configure scopes to be requested by default. If the scope has a qualifier,
 ...
 *
 * | realm
 * | HelidonRealm returned in HTTP response if redirect is not enabled or possible.
 * |===
 */
