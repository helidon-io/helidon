 ...
 * redirect-uri     | /oidc/redirect     | URI to register web server component on, used by the OIDC server to redirect
 *                                          authorization requests to after a user logs in or approves scopes. Note that usually
 *                                          the redirect URI configured here must be the same one as configured on OIDC server.
 * scope-audience   | empty string       | Audience of the scope required by this application. This is prefixed to
 *                                          the scope name when requesting scopes from the identity server.
 * cookie-use       | true               | Whether to use cookie to store JWT. If used, redirects happen only in case the user is
 *                                          not authenticated or has insufficient scopes
