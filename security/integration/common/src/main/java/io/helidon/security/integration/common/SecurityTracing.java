...
public final class SecurityTracing extends CommonTracing {
    ...
    private final TracedSpan atnSpanConfig;
    private final TracedSpan atzSpanConfig;
    private final TracedSpan outboundSpanConfig;
    private final TracedSpan responseSpanConfig;
    ...
    private SecurityTracing(Optional<SpanContext> parentSpanContext,
                            ...
                            TracedComponent tracedConfig) {
        super(parentSpanContext,
              parentSpan,
              securitySpan,
              securitySpan,
              tracedConfig.span(SPAN_SECURITY).orElse(TracedSpan.ENABLED));

        this.atnSpanConfig = tracedConfig.span(SPAN_AUTHENTICATION).orElse(TracedSpan.ENABLED);
        this.atzSpanConfig = tracedConfig.span(SPAN_AUTHORIZATION).orElse(TracedSpan.ENABLED);
        this.outboundSpanConfig = tracedConfig.span(SPAN_OUTBOUND).orElse(TracedSpan.ENABLED);
        this.responseSpanConfig = tracedConfig.span(SPAN_RESPONSE).orElse(TracedSpan.ENABLED);
    }
    ...
    private static SecurityTracing createTracing(Optional<Context> context) {
        TracedComponent componentConfig = context.flatMap(ctx -> ctx.get(TracedConfig.class))
                .orElse(TracedConfig.ENABLED)
                .component(COMPONENT)
                .orElse(TracedComponent.ENABLED);
    ...
    private static Optional<Span> createSecuritySpan(TracedComponent componentConfig, Optional<SpanContext> parentSpan) {
        return tracer()
                // if there is no tracer registered, ignore tracing
                .flatMap(tracer -> {
                    TracedSpan spanConfig = componentConfig.span(SPAN_SECURITY).orElse(TracedSpan.ENABLED);
    ...
    private static Optional<Span> newSpan(TracedSpan spanConfig,
                                          String spanName,
