...
public final class EncryptedJwt {
    ...
    public static EncryptedJwt parseToken(String token) {
        ...
        if (matcher.matches()) {
            ...
            // these all can fail
            String headerJsonString = decode(headerBase64, collector, "JWE header");
            ...
            JsonObject headers = JSON.createReader(new StringReader(headerJsonString)).readObject();
            return new EncryptedJwt(token, headers, iv, encryptedKey, authTag, encryptedPayload);
        } else {
    ...
    public Optional<SignedJwt> decrypt(JwkKeys jwkKeys, Jwk defaultJwk, Errors.Collector collector) {
        String headerBase64 = encode(headers.toString().getBytes(StandardCharsets.UTF_8));
        String alg = JwtUtil.getString(headers, "alg").orElse(null);
        String kid = JwtUtil.getString(headers, "kid").orElse(null);
        String enc = JwtUtil.getString(headers, "enc").orElse(null);
        Jwk jwk = null;
    ...
    public static class Builder implements io.helidon.common.Builder<EncryptedJwt> {
        ...
        private final JsonObjectBuilder headersBuilder = Json.createObjectBuilder();
        private Jwk jwk;
        ...
        public EncryptedJwt build() {
            headersBuilder.add("alg", algorithm.toString());
            headersBuilder.add("enc", encryption.toString());
            headersBuilder.add("cty", "JWT");
            PublicKey publicKey;
            ...
            String headersBase64 = encode(headers.toString());
