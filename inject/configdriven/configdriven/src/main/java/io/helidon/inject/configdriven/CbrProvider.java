...
class CbrProvider extends ServiceProviderBase<ConfigBeanRegistryImpl>
    ...
    private class CbrBinder extends NoOpBinder {
        ...
        @Override
        public ServiceInjectionPlanBinder.Binder bindOptional(Ip injectionPoint, ServiceInfo... serviceInfo) {
            // there is only one supported injection point, and it is optional
            if (serviceInfo.length == 1) {
                config = serviceInfo[0];
            }
            return this;
        }
        ...
        public String toString() {
            return CbrProvider.this.description();
        }
    }
}
