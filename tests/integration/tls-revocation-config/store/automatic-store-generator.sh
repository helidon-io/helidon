...
#  keytool -genkeypair -keyalg RSA -keysize 2048 -alias client -dname "C=CZ,ST=CZ,CN=$NAME-client,OU=Prague,O=Oracle" -validity 21650 -keystore client.jks -storepass password -keypass password -deststoretype pkcs12
  ...
  openssl pkcs12 -in ca.p12 -out ca.crt -nokeys -passin pass:password -passout pass:password
  echo 'Signing client and server certificates...'
#  openssl ca -batch -config ca.conf -notext -in client.csr -out client-signed.cer -passin pass:password -days 21650
  openssl x509 -req -in client.csr -CA ca.pem -CAkey ca.key -CAcreateserial -out client-signed.cer -days 21650 -passin pass:password -extfile crl.conf
  ...
  openssl ca -config ca.conf -revoke client-signed.cer -keyfile ca.key -cert ca.crt -passin pass:password

	openssl ca -config ca.conf -gencrl -keyfile ca.key -cert ca.crt -out rt.crl.pem -passin pass:password
  openssl crl -inform PEM -in rt.crl.pem -outform DER -out out/ca.crl
  rm rt.crl.pem
}
