...
class JdbcPersistenceUnitFactoryHikariTest {
    ...
    /**
     * Proves bootstrap SQL failure returns the leased connection and reports
     * the failing statement position without exposing SQL.
     */
    @Test
    void returnsScriptConnectionAfterSqlFailure() {
        try (HikariDataSource source = dataSource("invalid_script", true)) {
            Config config = Config.just(ConfigSources.create(Map.of(
                    ...
                    "data.persistence-units.jdbc.0.init-script.resource-path", "db/h2/jdbc-bootstrap-invalid.sql")));
            ...
            assertThat(failure.getMessage(), containsString("failed to execute the statement beginning at line 18"));
            assertThat(failure.getMessage(), containsString("SQLSTATE condition is 'Base table not found'"));
            assertThat(failure.getMessage(), containsString("SQLSTATE '42S02' and vendor code 42102"));
    ...
    /**
     * Proves successful bootstrap commits when the configured datasource
     * supplies manual-commit connections.
     */
    @Test
    ...
    /**
     * Proves failed bootstrap rolls back manual-commit DML and returns the
    ...
    void rollsBackBootstrapWorkWhenDatasourceDisablesAutoCommit() throws Exception {
        try (HikariDataSource source = dataSource("manual_bootstrap_failure", false)) {
            ...
            Config config = Config.just(ConfigSources.create(Map.of(
                    ...
                    "db/h2/jdbc-bootstrap-transaction-invalid.sql")));
            JdbcPersistenceUnitFactory factory = new JdbcPersistenceUnitFactory(
