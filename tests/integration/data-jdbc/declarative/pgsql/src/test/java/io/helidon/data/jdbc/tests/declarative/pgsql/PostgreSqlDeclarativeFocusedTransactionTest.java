...
/**
 * Executes focused generated-repository transaction behavior against PostgreSQL.
 */
@Testcontainers(disabledWithoutDocker = true)
class PostgreSqlDeclarativeFocusedTransactionTest extends AbstractJdbcFocusedTransactionContract {
    @Container
    static final org.testcontainers.containers.JdbcDatabaseContainer<?> POSTGRESQL =
            PostgreSqlDeclarativeTestSupport.POSTGRESQL;

    @Override
    protected void beforeStartApplication() {
        TestConfigFactory.config(PostgreSqlDeclarativeTestSupport.config());
    }

    @Override
    protected Class<? extends FocusedTransactionOperations> operationsType() {
        return DeclarativeFocusedTransactionOperations.class;
    }
}
