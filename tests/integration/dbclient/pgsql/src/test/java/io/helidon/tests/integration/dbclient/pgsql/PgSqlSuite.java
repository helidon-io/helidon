...
public class PgSqlSuite implements SuiteProvider, SuiteResolver {
    private static final System.Logger LOGGER = System.getLogger(PgSqlSuite.class.getName());
    ...
    public void beforeSuite() {
        ...
        URI uri = ConfigUtils.uriFromDbUrl(
                config.get("db.connection.url")
                        .asString()
                        .orElse("jdbc:postgresql://127.0.0.1:5432/pokemon"));
        ContainerConfig containerConfig = ContainerConfig.builder()
                .image(config.get("db.container").asString().orElse("postgres:latest"))
                .addEnvironment(Map.of(
                        "POSTGRES_USER", config.get("db.connection.username").asString().orElse("user"),
                        "POSTGRES_PASSWORD", config.get("db.connection.password").asString().orElse("p4ssw0rd"),
                        "POSTGRES_DB", ConfigUtils.dbNameFromUri(uri)))
                .exposedPorts(new int[] {uri.getPort() >=0 ? uri.getPort() : 5432})
                .build();
        ...
        LOGGER.log(System.Logger.Level.TRACE,
                   () -> String.format("Starting PostgreSQL database from image %s", containerConfig.image()));
        container.start();
        // Update config with current PostgreSQL container configuration
        int dbPort = container.getMappedPort(containerConfig.exposedPorts()[0]);
        try {
            URI newUri = new URI("postgresql",
                                 null,
                                 uri.getHost(),
                                 dbPort,
                                 uri.getPath(),
            ...
            config = Config.create(
                    ConfigSources.create(
                            Map.of(
                                    ...
                                    "db.connection.username", containerConfig.environment().get("POSTGRES_USER"),
                                    "db.connection.password", containerConfig.environment().get("POSTGRES_PASSWORD"),
                                    "db.connection.url", "jdbc:" + newUri.toString())),
                    ConfigSources.create(config));
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
        // Wait for PostgreSQL database to start accepting JDBC connections
    ...
    public void afterSuite(DbClient dbClient) {
        ...
        LOGGER.log(System.Logger.Level.TRACE, "Stopping PostgreSQL database");
