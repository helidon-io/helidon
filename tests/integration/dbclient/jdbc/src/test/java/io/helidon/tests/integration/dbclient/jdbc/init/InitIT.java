...
public class InitIT extends AbstractIT {
    ...
    public void testListPokemonTypes() throws ExecutionException, InterruptedException {
        ...
        for (DbRow row : rowsList) {
            ...
            Integer typeId = row.column(3).as(Integer.class);
            String typeName = row.column(4).as(String.class);
            assertThat(pokemonName, POKEMONS.get(pokemonId).getName().equals(pokemonName));
            assertThat(POKEMONS.get(pokemonId).getTypes(), hasItem(TYPES.get(typeId)));
            LOG.log(Level.INFO, "Pokemon id={0} name={1} type id={2} name={3}",
                    new String[] {String.valueOf(pokemonId), pokemonName, String.valueOf(typeId), typeName});
        }
    }
