...
public class Utils {
    ...
    /**
     * Run DB client statements using provided DbExecute instance.
     * {@link DbExecute} instance is closed before returning result so this method may not
     * be used with query returning {@link java.util.stream.Stream}.
     *
     * @param exec {@link Function} to execute with provided DbExecute instance
     */
    public static <T> T executeOnce(Function<DbExecute, T> exec) {
        return exec.apply(DB_CLIENT.execute());
    }

    /**
     * Run DB client statements using provided DbTransaction instance and commit the transaction.
     * {@link DbTransaction} instance is closed before returning result so this method may not
     * be used with query returning {@link java.util.stream.Stream}.
     *
     * @param exec {@link Function} to execute with provided DbExecute instance
     */
    public static <T> T transaction(Function<DbTransaction, T> exec) {
        DbTransaction dbTransaction = DB_CLIENT.transaction();
        T result = exec.apply(dbTransaction);
    ...
    public static void verifyPokemonsIdRange(Stream<DbRow> rows, int idMin, int idMax) {
        ...
        Map<Integer, AbstractIT.Pokemon> valid = new HashMap<>(POKEMONS.size());
        for (Map.Entry<Integer, AbstractIT.Pokemon> entry : POKEMONS.entrySet()) {
            int id = entry.getKey();
            AbstractIT.Pokemon pokemon = entry.getValue();
        ...
        for (DbRow row : rowsList) {
            ...
            LOGGER.log(System.Logger.Level.INFO, () -> String.format("Pokemon id=%d, name=%s", id, name));
