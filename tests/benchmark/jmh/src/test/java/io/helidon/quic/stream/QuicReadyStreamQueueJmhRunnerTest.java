...
class QuicReadyStreamQueueJmhRunnerTest {
    private static final String PREFIX = "quic.ready.stream.queue.jmh.";

    @Test
    void run() throws RunnerException {
        String include = System.getProperty(PREFIX + "include", ".*QuicReadyStreamQueueJmhBenchmark.*");
        String result = System.getProperty(PREFIX + "result", "./target/quic-ready-stream-queue-jmh-result.json");

        ChainedOptionsBuilder optionsBuilder = new OptionsBuilder()
                .include(include)
                .forks(Integer.getInteger(PREFIX + "forks", 0))
                .threads(Integer.getInteger(PREFIX + "threads", 8))
