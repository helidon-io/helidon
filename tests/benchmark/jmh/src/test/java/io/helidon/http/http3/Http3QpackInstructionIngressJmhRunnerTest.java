...
class Http3QpackInstructionIngressJmhRunnerTest {
    private static final String PREFIX = "http3.qpack.instruction.ingress.jmh.";

    @Test
    void run() throws RunnerException {
        String include = System.getProperty(PREFIX + "include",
                                            "^" + Pattern.quote(Http3QpackInstructionIngressJmhBenchmark.class.getName())
                                                    + "\\.processEncoderInstruction$");
        ChainedOptionsBuilder optionsBuilder = new OptionsBuilder()
                .include(include)
                .forks(Integer.getInteger(PREFIX + "forks", 1))
                .threads(Integer.getInteger(PREFIX + "threads", 1))
                .resultFormat(ResultFormatType.JSON)
                .result(System.getProperty(PREFIX + "result", "./target/http3-qpack-instruction-ingress-jmh-result.json"))
        ...
        for (String parameter : List.of("nameSize", "valueSize", "fragmentation")) {
