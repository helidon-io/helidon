...
import io.helidon.http.MethodParsingJmhBenchmark;
...
class MethodParsingJmhRunnerTest {
    private static final String METHODS = "(compatibleCanonicalDelete|compatibleLowercaseDelete"
            + "|caseSensitiveCanonicalDelete|caseSensitiveLowercaseDelete)";

    @Test
    void runExactBenchmarks() throws RunnerException {
        String result = System.getProperty("http.method.jmh.result", "target/http-method-parsing-jmh.json");
        String include = "^" + Pattern.quote(MethodParsingJmhBenchmark.class.getName()) + "\\." + METHODS + "$";
