...
class RapidResetTest {
    private static final Duration TIMEOUT = Duration.ofSeconds(10);
    private final WebServer server;

    public RapidResetTest(WebServer server) {
        this.server = server;
    }
    ...
    @Test
    void rapidResetAttack() throws InterruptedException, ExecutionException, TimeoutException {
        ...
        for (int streamId = 1; streamId < 1000; streamId +=2) {
            try {
                ...
                Http2RstStream resetFrame = new Http2RstStream(Http2ErrorCode.PROTOCOL);
                dataWriter.writeData(resetFrame.toFrameData(http2Settings, streamId, Http2Flag.NoFlags.create()),
                                     FlowControl.Outbound.NOOP);
        ...
        assertThat(http2GoAway, is("ENHANCE_YOUR_CALM - Rapid reset attack detected!"));
