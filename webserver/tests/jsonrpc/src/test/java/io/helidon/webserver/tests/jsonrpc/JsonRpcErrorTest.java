...
class JsonRpcErrorTest extends JsonRpcBaseTest {
    ...
    void testBadJson() {
        try (var res = client().post("/rpc/machine")
            ...
            assertThat(error.numberValue("code").orElseThrow().intValue(), is(JsonRpcError.PARSE_ERROR));
        }
    }
    ...
    void testInvalidVersion() {
        try (var res = client().post("/rpc/machine")
            ...
            JsonObject error = object.objectValue("error").orElseThrow();
            assertThat(error.numberValue("code").orElseThrow().intValue(), is(JsonRpcError.INVALID_REQUEST));
        }
    }
    ...
    void testInvalidMethod() {
        try (var res = client().post("/rpc/machine")
            ...
            assertThat(object.intValue("id").orElseThrow(), is(1));
            JsonObject error = object.objectValue("error").orElseThrow();
            assertThat(error.numberValue("code").orElseThrow().intValue(), is(JsonRpcError.METHOD_NOT_FOUND));
        }
    }
    ...
    void testStartError() {
        try (var res = client().post("/rpc/machine")
            ...
            assertThat(object.intValue("id").orElseThrow(), is(1));
            JsonObject error = object.objectValue("error").orElseThrow();
            assertThat(error.numberValue("code").orElseThrow().intValue(), is(JsonRpcError.INVALID_PARAMS));
