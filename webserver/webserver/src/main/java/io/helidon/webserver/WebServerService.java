    ...
    void updateServerBuilder(WebServerConfig.BuilderBase<?, ?> builder) {
        ...
        for (HttpFeature httpFeature : httpFeatures) {
            ...
            if (useDefault) {
                ...
                List<Builder<?, ? extends Routing>> routingBuilders = builder.namedRoutings()
                        .get(socket);

                HttpRouting.Builder httpBuilder = null;
                boolean routingFound = routingBuilders != null;

                if (routingFound) {
                    routingFound = false;
                    // look specifically for Http routing builder
                    for (Builder<?, ? extends Routing> routingBuilder : routingBuilders) {
                        if (routingBuilder instanceof HttpRouting.Builder httpRouting) {
                            httpBuilder = httpRouting;
                            routingFound = true;
                            break;
                        }
                    }
