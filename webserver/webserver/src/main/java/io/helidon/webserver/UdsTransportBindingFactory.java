...
public final class UdsTransportBindingFactory implements TransportBindingFactory {
    ...
    private UdsTransportBindingFactory(UdsTransportConfig config) {
     ...
     * @return Unix domain socket transport binding factory
     */
    public static UdsTransportBindingFactory create(UdsTransportConfig config) {
        return new UdsTransportBindingFactory(config);
    }
    ...
    public boolean canBind(BindingPlanContext context) {
        ...
        return config.enabled() && config.socket().isPresent();
    }
