...
class Http1ServerRequestTest {
    ...
    void matchingPatternSupplierIsLazyAndReplaceable() {
        Http1ServerRequest request = request();
    ...
    private static Http1ServerRequest request() {
        return Http1ServerRequest.create(mock(ConnectionContext.class), HttpSecurity.create(),
                                         HttpPrologue.create("HTTP/1.1", "HTTP", "1.1", Method.GET, "/resource", false),
                                         WritableHeaders.create(), 1, null);
    }
}
