...
/**
 * The BytesReuseTest verifies whether the {@link io.helidon.common.http.DataChunk} instances get released properly.
 * <p>
 * Note that with {@link io.helidon.common.http.DataChunk#finalize()} which calls {@link io.helidon.common.http.DataChunk#release()},
 * we no longer experience {@link OutOfMemoryError} exceptions in case the chunks aren't freed
 * as long as no references to the {@link io.helidon.common.http.DataChunk} instances are kept.
 */
@Deprecated(since = "3.0.0", forRemoval = true)
public class BytesReuseV2ApiTest {
     ...
     * {@link io.helidon.common.http.DataChunk} instances.
    ...
    /**
     * This test shows that even when the {@link io.helidon.common.http.DataChunk#release()} isn't called, we don't get
     * {@link OutOfMemoryError} thanks to {@link io.helidon.common.http.DataChunk#finalize()} that calls
     * {@link io.helidon.common.http.DataChunk#release()} automatically. This test needs at least 1GB of heap.
     * <p>
    ...
    /**
     * This test shows that when the {@link io.helidon.common.http.DataChunk#release()} is called, WebServer can run
    ...
    private void flood(boolean release, boolean keepChunks, int limit, boolean doAssert) throws Exception {
        try (SocketHttpClient s = new ChunkedSocketHttpClient(BytesReuseV2ApiTest.webServer, limit)) {
