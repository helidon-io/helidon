...
class WebSocketSecurityUpgradeTest {
    ...
    private static final HeaderName FILTER_HEADER = HeaderNames.create("X-Filter-Header");
    private static final HeaderName FILTER_SEEN_HEADER = HeaderNames.create("X-Filter-Seen");
        ...
        serverBuilder.featuresDiscoverServices(false)
                ...
                .routing(routing -> routing
                        .addFilter((chain, req, res) -> {
                            ...
                            if (("/header-filter".equals(path) || "/header-filter-key".equals(path))
                                    && req.headers().contains(HeaderNames.UPGRADE)) {
                                req.header(HeaderValues.create(FILTER_HEADER, "present"));
                            }
                            if ("/header-filter-key".equals(path) && req.headers().contains(HeaderNames.UPGRADE)) {
                                req.header(HeaderValues.create(WS_KEY, FILTER_WS_KEY));
                            }
                            if ("/header-filter-origin".equals(path) && req.headers().contains(HeaderNames.UPGRADE)) {
                ...
                .addRouting(WsRouting.builder()
                                    ...
                                    .endpoint("/header-filter", new FilterHeaderWsListener())
                                    .endpoint("/header-filter-key", new FilterHeaderWsListener())
                                    .endpoint("/header-filter-origin", new ProbeWsListener())
    ...
    @Test
    void webSocketUpgradeWithRequestEntityFallsBackToSecuredHttpRoute() throws Exception {
    ...
    void listenerHeadersCannotOverrideWebSocketHandshake() throws Exception {
        ...
        assertThat(wsResponse, containsString("Sec-WebSocket-Accept: " + ORIGINAL_WS_ACCEPT + "\r\n"));
    ...
    @Test
    void webSocketUpgradeUsesFilterUpdatedHeaders() throws Exception {
        String wsResponse = sendRequest(
                "GET /header-filter HTTP/1.1\r\n"
                        + "Host: localhost:" + port + "\r\n"
                        + "Upgrade: websocket\r\n"
                        + "Connection: Upgrade\r\n"
                        + "Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\n"
        ...
        assertThat(wsResponse, containsString("HTTP/1.1 101"));
        assertThat(wsResponse, containsString("X-Filter-Seen: present\r\n"));
    }

    @Test
    void webSocketUpgradeUsesFilterUpdatedHandshakeKey() throws Exception {
        String wsResponse = sendRequest(
                "GET /header-filter-key HTTP/1.1\r\n"
                        + "Host: localhost:" + port + "\r\n"
                        + "Upgrade: websocket\r\n"
                        + "Connection: Upgrade\r\n"
                        + "Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\n"
        ...
        assertThat(wsResponse, containsString("HTTP/1.1 101"));
        assertThat(wsResponse, containsString("X-Filter-Seen: present\r\n"));
        assertThat(wsResponse, containsString("Sec-WebSocket-Accept: " + FILTER_WS_ACCEPT + "\r\n"));
        assertThat(wsResponse, not(containsString("Sec-WebSocket-Accept: " + ORIGINAL_WS_ACCEPT + "\r\n")));
    }

    @Test
    void webSocketUpgradeRejectsFilterUpdatedOrigin() throws Exception {
        String wsResponse = sendRequest(
                "GET /header-filter-origin HTTP/1.1\r\n"
                        + "Host: localhost:" + port + "\r\n"
                        + "Upgrade: websocket\r\n"
                        + "Connection: Upgrade\r\n"
                        + "Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\n"
    ...
    private static final class FilterHeaderWsListener implements WsListener {
        @Override
        public Optional<Headers> onHttpUpgrade(HttpPrologue prologue, Headers headers) {
            if (headers.contains(FILTER_HEADER)) {
                return Optional.of(WritableHeaders.create()
                                           .set(HeaderValues.create(FILTER_SEEN_HEADER, "present")));
            }
