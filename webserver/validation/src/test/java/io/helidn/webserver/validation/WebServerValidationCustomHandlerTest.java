...
@RoutingTest
public class WebServerValidationCustomHandlerTest {
    private final Http1Client client;

    public WebServerValidationCustomHandlerTest(Http1Client client) {
        this.client = client;
    }

    @SetUpRoute
    public static void setUp(HttpRouting.Builder rules) {
        ...
        rules.get("/fail", (req, res) -> {
                ...
                .error(ValidationException.class, (req, res, ex) -> res.status(Status.BAD_REQUEST_400).send(ex.getMessage()));
    }
    ...
    public void testBadRequest() {
        try (var response = client.get("/fail")
            ...
            assertThat(response.headers(), not(hasHeader(HeaderValues.CONTENT_LENGTH_ZERO)));
        }
    }
