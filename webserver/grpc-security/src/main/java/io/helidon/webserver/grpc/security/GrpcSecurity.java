...
public final class GrpcSecurity implements ServerInterceptor, GrpcServerService, GrpcServiceDescriptor.Configurer {
    ...
    private static final String SERVER_PROTOCOL_SERVICES_CONFIG_KEY = "server.protocols.grpc.grpc-services";
    ...
    public static GrpcSecurity create(Config config) {
        ...
        Config grpcSecurityConfig = grpcSecurityConfig(config);
        Security security = grpcSecurityConfig.get("enabled").asBoolean().orElse(true)
                ? Security.create(config.root().get("security"))
    ...
    public static GrpcSecurity create(Security security, Config config) {
        Objects.requireNonNull(config);
        return new GrpcSecurity(security, grpcSecurityConfig(config));
    }

    private static Config grpcSecurityConfig(Config config) {
        ...
        if (TOP_LEVEL_CONFIG_KEY.equals(key)
            ...
            Config protocolServicesConfig = grpcSecurityListServiceConfig(config.get(SERVER_PROTOCOL_SERVICES_CONFIG_KEY));
            Config topLevelServicesConfig = grpcSecurityListServiceConfig(config.get(TOP_LEVEL_SERVICES_CONFIG_KEY));
            if (protocolConfig.exists()) {
                grpcSecurityConfig = protocolConfig;
            } else if (protocolServicesConfig.exists()) {
                grpcSecurityConfig = protocolServicesConfig;
            } else if (topLevelConfig.exists()) {
                grpcSecurityConfig = topLevelConfig;
            } else {
                grpcSecurityConfig = topLevelServicesConfig.exists() ? topLevelServicesConfig : Config.empty();
            }
        } else if (isGrpcSecurityServiceConfig(config)) {
            grpcSecurityConfig = config;
        } else {
    ...
    private static Config grpcSecurityListServiceConfig(Config servicesConfig) {
        if (servicesConfig.isList()) {
            for (Config serviceConfig : servicesConfig.asNodeList().orElseGet(List::of)) {
                if (isGrpcSecurityServiceConfig(serviceConfig)) {
                    return serviceConfig;
                }
                List<Config> children = serviceConfig.asNodeList().orElseGet(List::of);
                if (children.size() == 1 && isGrpcSecurityServiceConfig(children.getFirst())) {
    ...
    private static boolean isGrpcSecurityServiceConfig(Config config) {
        ...
        String key = config.key().toString();
        return TYPE.equals(config.get(TYPE_CONFIG_KEY).asString().orElse(null))
                || (TYPE.equals(config.name()) && key.contains("grpc-services"));
    }
