...
class HttpTransportMetricsSocketConfigTest {
    ...
    @SetUpRoute
    static void setUpDefaultRoute(HttpRouting.Builder routing) {
        routing.get("/test", (_, response) -> response.send("default"));
    }
    ...
    @Test
    void doesNotObserveExcludedSocket() {
        try (HttpClientResponse response = defaultClient.get("/test").request()) {
