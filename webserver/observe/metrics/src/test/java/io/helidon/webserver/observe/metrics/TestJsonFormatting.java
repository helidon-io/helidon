...
class TestJsonFormatting {
    ...
    void testRetrievingAll() {
        ...
        assertThat("Counter 1",
                   app.intValue("c1;t1=v1").orElseThrow(),
                   is(4));
        assertThat("Counter 2",
                   app.intValue("c1").orElseThrow(),
                   is(1));
        var timerJson = app.objectValue("t1");
        assertThat("Timer", timerJson, OptionalMatcher.optionalPresent());
        assertThat("Timer count", timerJson.orElseThrow().intValue("count"), OptionalMatcher.optionalValue(is(1)));
    }
    ...
    void testRetrievingByName() {
        ...
        JsonObject jsonOutput = checkAndCast(formatter.format());
        JsonObject app = jsonOutput.objectValue("application").orElseThrow();
        assertThat("Counter 2", app.intValue("c2").orElseThrow(), is(1));

        assertThat("Timer", app.objectValue("t2"), OptionalMatcher.optionalEmpty());
    ...
    void testTimerUnits() {
        ...
        assertThat("Timer", timerJson.doubleValue("elapsedTime").orElseThrow(), is(0.256d));
    ...
    void testTimerUnitsWithConfigSetting() {
        ...
        assertThat("Timer with explicit microseconds units",
                   timerWithMicroSecondsJson.doubleValue("elapsedTime").orElseThrow(),
                   is(3256000d));
        assertThat("Timer with no explicit units",
                   timerWithNoUnitsJson.doubleValue("elapsedTime").orElseThrow(),
                   is(128d));
