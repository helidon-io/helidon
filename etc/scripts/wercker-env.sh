...
if [ "${WERCKER}" = "true" ] ; then
    echo "WERCKER environment variable is set to true: ${WERCKER}"
    ...
    echo Checking for presence of "${WERCKER_CACHE_DIR}/local_repository/com/oracle/oci/sdk"
    if [ ! -d "${WERCKER_CACHE_DIR}/local_repository/com/oracle/oci/sdk" ] ; then
        echo "Working around https://github.com/oracle/oci-java-sdk/issues/25"
        ...
        echo "Not present.  Listing."
        ls -al "${WERCKER_CACHE_DIR}/local_repository/com/oracle/oci/sdk"
    fi
fi
