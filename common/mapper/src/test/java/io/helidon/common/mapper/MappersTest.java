...
class MappersTest {
    ...
    void testUsingServiceLoader() {
        ...
        // uses the highest weight - ServiceRegistryMapper
    ...
    void testUsingCustomProviders() {
        Mappers mm = Mappers.builder()
                .mapperProvidersDiscoverServices(false)
                .mapperProviders(List.of(new ServiceLoaderMapper()))
    ...
    void testUsingServiceLoaderAndCustomMappers() {
        ...
        // using generic types
        // uses the highest weight - the ServiceRegistryMapper
        result = mm.map(source, ServiceRegistryMapper.STRING_TYPE, ServiceRegistryMapper.INTEGER_TYPE, "default");
        assertThat(result, is(11));
    ...
    void testCustomMapper() {
        // make sure the custom mapper is discovered and maps values
        Mappers mappers = Mappers.create();
        Size result = mappers.map("12 KB", String.class, Size.class);

        assertThat(result, is(Size.create(12, Size.Unit.KIB)));
    }
