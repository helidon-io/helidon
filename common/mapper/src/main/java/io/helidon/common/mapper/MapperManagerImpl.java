...
final class MapperManagerImpl implements MapperManager {
    ...
    private <SOURCE, TARGET> Optional<Mapper<SOURCE, TARGET>> fromProviders(Class<SOURCE> sourceType,
                                                                            Class<TARGET> targetType) {
    ...
    private <SOURCE, TARGET> Optional<Mapper<SOURCE, TARGET>> fromProviders(GenericType<SOURCE> sourceType,
                                                                            GenericType<TARGET> targetType) {
        return providers.stream()
                .flatMap(provider -> OptionalHelper.from(provider.mapper(sourceType, targetType)).stream())
                .findFirst();
