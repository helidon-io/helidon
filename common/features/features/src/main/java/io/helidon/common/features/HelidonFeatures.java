...
public final class HelidonFeatures {
     ...
     * @deprecated use {@link #print(io.helidon.common.features.metadata.HelidonFlavor, String, boolean)} instead
     */
    @SuppressWarnings("removal")
    @Deprecated(forRemoval = true, since = "4.3.0")
    public static void print(io.helidon.common.features.api.HelidonFlavor flavor, String version, boolean details) {
        // print features in another thread, so we do not block the main thread of the application (to reduce startup time)
        new Thread(() -> features(mapFlavor(flavor), version, details), "features-thread")
                .start();
    }
     ...
     * @deprecated use {@link #flavor(io.helidon.common.features.metadata.HelidonFlavor)} instead
     */
    @SuppressWarnings("removal")
    @Deprecated(forRemoval = true, since = "4.3.0")
    public static void flavor(io.helidon.common.features.api.HelidonFlavor flavor) {
        CURRENT_FLAVOR.compareAndSet(null, mapFlavor(flavor));
    }
    ...
    @SuppressWarnings("removal")
    private static HelidonFlavor mapFlavor(io.helidon.common.features.api.HelidonFlavor flavor) {
        return HelidonFlavor.valueOf(flavor.name());
    }
