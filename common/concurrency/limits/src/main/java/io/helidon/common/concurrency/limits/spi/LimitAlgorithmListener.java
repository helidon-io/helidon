 ...
 * <p>
 * Helidon uses one listener instance from each listener provider for each instance of incoming work. Components which use
 * concurrency limiting can populate each listener with information specific to that component (such as HTTP request
 * headers) which are different for different items of work (i.e., different header values for different requests).
 * Such components then pass these listener instances when they invoke the limit {@code tryAcquire} or {@code invoke} methods.
 * <p>
...
public interface LimitAlgorithmListener {
    ...
    /**
     * Marker interface for listeners which update metrics related to concurrency limit processing so limit configuration
     * which might enable or disable metrics can be applied.
     */
    interface Metrics {
    }

    /**
     * Marker interface for listeners which manage spans related to concurrency limit processing so limit configuration
     * which might enable or disable tracing can be applied.
     */
    interface Tracing {
    }
