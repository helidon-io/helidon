...
public class OutputStreamMulti extends OutputStream implements Multi<ByteBuffer> {
     ...
     * publishes any data written to it as {@link java.nio.ByteBuffer} events.
     */
    protected OutputStreamMulti() {
        emittingPublisher.onCancel(() -> demandUpdated.cancel(true));
        emittingPublisher.onRequest((n, demand) -> this.demandUpdated.complete(null));
    }
     ...
     * @return this output stream multi
     */
    ...
    private void await(long startTime, long waitTime, CompletableFuture<?> future)
