...
/**
 * A typed value of a {@link io.helidon.config.Config} node.
 * <p>
 * You can use accessor methods on {@link io.helidon.config.Config} to obtain this value, such as {@link io.helidon.config.Config#as(Class)}.
 ...
 * @see io.helidon.config.Config#as(java.util.function.Function)
 * @see io.helidon.config.Config#as(io.helidon.common.GenericType)
 */
public interface ConfigValue<T> {
     ...
     * containing node:
     * <ul>
     * <li>from a {@link io.helidon.config.Config.Type#OBJECT} node the token for a child is the
     * <strong>name of the object member</strong>;</li>
     * <li>from a {@link io.helidon.config.Config.Type#LIST} node the token for a child is a zero-based
     ...
     * @return current config node key
     * @see #key()
     * @see io.helidon.config.Config.Key#name()
     */
    default String name() {
        return key().name();
    }
     ...
     * As this class implements all methods of {@link java.util.Optional}, this is only a utility method if an actual {@link java.util.Optional}
     * instance is needed.
     *
     * @return value as type instance as {@link java.util.Optional}, {@link java.util.Optional#empty() empty} in case the node does not have
     * a direct value
     * @throws io.helidon.config.ConfigMappingException in case the value cannot be converted to the expected type
     * @see #get()
     */
    Optional<T> asOptional() throws ConfigMappingException;

    /**
     * Typed value of the represented {@link io.helidon.config.Config} node.
     * Throws {@link io.helidon.config.MissingValueException} if the node is {@link io.helidon.config.Config.Type#MISSING} type.
     *
     * @return direct value of this node converted to the expected type
     * @throws io.helidon.config.MissingValueException  in case the node is {@link io.helidon.config.Config.Type#MISSING}.
     * @throws io.helidon.config.ConfigMappingException in case the value cannot be converted to the expected type
     */
     ...
     * <p>
     * Note that {@link java.util.function.Supplier#get()} can throw a {@link io.helidon.config.ConfigMappingException} or {@link io.helidon.config.MissingValueException} as the
     * {@link #get()} method.
     *
     ...
     * <p>
     * Note that {@link java.util.function.Supplier#get()} can throw a {@link io.helidon.config.ConfigMappingException} as the
     * {@link #orElse(Object)} method.
     *
     * @param defaultValue a value to be returned if the supplied value represents a {@link io.helidon.config.Config} node that has no direct
     *                     value
     * @return a supplier of a typed value
     */
    ...
    /**
     * Returns a {@link java.util.function.Supplier} of an {@link java.util.Optional Optional&lt;T&gt;} of the configuration node.
     *
     * Supplier returns a {@link java.util.Optional#empty() empty} if the node does not have a direct value.
     *
     * @return a supplier of the value as an {@link java.util.Optional} typed instance, {@link java.util.Optional#empty() empty} in case the node
