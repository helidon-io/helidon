...
package io.helidon.nima.tests.benchmark.techempower;
    ...
    void testPlaintext() {
        try (Http1ClientResponse response = client.get("/plaintext").request()) {
            ...
            Assertions.assertAll(
                    () -> assertThat(headers, hasHeader(TechEmpowerMain.PlaintextHandler.CONTENT_TYPE)),
                    () -> assertThat(headers, hasHeader(TechEmpowerMain.PlaintextHandler.CONTENT_LENGTH)),
                    () -> assertThat(headers, hasHeader(TechEmpowerMain.PlaintextHandler.SERVER)),
    ...
    void testJson() {
        try (Http1ClientResponse response = client.get("/json").request()) {
            ...
            Assertions.assertAll(
                    ...
                    () -> assertThat(headers, hasHeader(TechEmpowerMain.JsonHandler.CONTENT_LENGTH)),
                    () -> assertThat(headers, hasHeader(TechEmpowerMain.JsonHandler.SERVER)),
                    () -> assertThat(headers, hasHeader(HeaderValues.CONNECTION_KEEP_ALIVE)),
                    () -> assertThat(headers, hasHeader(Http.Header.DATE)),
