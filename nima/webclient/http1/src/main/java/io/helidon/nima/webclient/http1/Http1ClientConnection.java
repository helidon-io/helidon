...
class Http1ClientConnection implements ClientConnection {
    ...
    Http1ClientConnection(HttpClientConfig clientConfig,
                          Http1ClientProtocolConfig protocolConfig,
                          ConnectionKey connectionKey) {
        this(clientConfig, protocolConfig, null, connectionKey);
    }
    ...
    Http1ClientConnection connect() {
        ...
        try {
            socket = new Socket();
            socket.setSoTimeout((int) clientConfig.socketOptions().readTimeout().toMillis());
            clientConfig.socketOptions().configureSocket(socket);
    ...
    private int connectToProxy(InetSocketAddress remoteAddress, InetSocketAddress proxyAddress) {
        ...
        ClientUri uriHelper = ClientUri.create();
        uriHelper.scheme("http");
        uriHelper.host(proxyAddress.getHostName());
        uriHelper.port(proxyAddress.getPort());

        ClientRequestImpl clientRequest = new ClientRequestImpl(clientConfig,
                                                                protocolConfig,
                                                                Method.CONNECT,
                                                                uriHelper,
                                                                UriQueryWriteable.create(),
                                                                Map.of());
        clientRequest.connection(proxyConnection);
        clientRequest.header(Header.HOST, hostPort);
        clientRequest.header(Header.ACCEPT, "*/*");
        Http1ClientResponse response = clientRequest.request();
    ...
    private int connectTimeout() {
        return (int) clientConfig.socketOptions().connectTimeout().toMillis();
    }
    ...
    private enum ConnectionStrategy {
        PLAIN {
            ...
            protected void connect(Http1ClientConnection connection, InetSocketAddress remoteAddress) throws IOException {
                connection.socket.connect(remoteAddress, connection.connectTimeout());
        ...
        PROXY_PLAIN {
            @Override
            protected void connect(Http1ClientConnection connection, InetSocketAddress remoteAddress) {
