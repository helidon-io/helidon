...
public interface Filter extends ServerLifecycle {
     ...
     * {@link io.helidon.nima.webserver.http.FilterChain#proceed()} may throw an exception. The following exceptions
     * mean the connection will close (and the request cannot finish):
     * <ul>
     *     <li>{@link io.helidon.nima.webserver.CloseConnectionException}</li>
     *     <li>{@link java.io.UncheckedIOException}</li>
     * </ul>
     * Filters may throw {@link io.helidon.common.http.HttpException} (or its subclasses), all filters before the current
     * filter will not receive such an exception - it will be processed before the method
     * {@link io.helidon.nima.webserver.http.FilterChain#proceed()} returns.
     *
