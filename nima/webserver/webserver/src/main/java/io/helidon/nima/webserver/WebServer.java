...
public interface WebServer extends RuntimeType<ServerConfig> {
    ...
    //                            connConfig.get("connect-timeout-seconds").asInt().ifPresent(it -> socketOptionsBuilder
    //                            .connectTimeout(
    //                                    Duration.ofSeconds(it)));
    //                            connConfig.get("send-buffer-size").asInt().ifPresent
    //                            (socketOptionsBuilder::socketSendBufferSize);
    ...
    //         * If an explicit media support is configured using {@link #addMediaSupport(io.helidon.nima.http.media
    //         .MediaSupport)},
