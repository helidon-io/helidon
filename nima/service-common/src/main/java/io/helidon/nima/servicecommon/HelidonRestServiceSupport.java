 ...
 *
 * <p>
 *     Concrete implementations must implement {@link #postConfigureEndpoint(HttpRules, HttpRules)} to do any
...
public abstract class HelidonRestServiceSupport implements RestServiceSupport {
     ...
     */
    protected HelidonRestServiceSupport(System.Logger logger, Builder<?, ?> builder, String serviceName) {
        this(logger, builder.restServiceSettingsBuilder.build(), serviceName);
    }

    protected HelidonRestServiceSupport(System.Logger logger, RestServiceSettings restServiceSettings, String serviceName) {
        this.logger = logger;
     ...
     * {@link #routing(io.helidon.nima.webserver.http.HttpRules)} (e.g. you should not
     ...
     */
    @Override
    public final void configureEndpoint(HttpRules defaultRules, HttpRules serviceEndpointRoutingRules) {
        // CORS first
        defaultRules.any(corsEnabledServiceHelper.processor());
        if (defaultRules != serviceEndpointRoutingRules) {
            serviceEndpointRoutingRules.any(corsEnabledServiceHelper.processor());
        }
     ...
     *
     * @param defaultRules                default {@code HttpRules} to be updated
     * @param serviceEndpointRoutingRules actual rules (if different from the default ones) to be updated for the service endpoint
     */
    protected abstract void postConfigureEndpoint(HttpRules defaultRules, HttpRules serviceEndpointRoutingRules);
