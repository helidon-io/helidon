...
class GrpcBaseClientCallTest {
    ...
    @Test
    void serverStreamingCallDrainsCoalescedTerminalMessages() {
        byte[] response = BufferData.create(grpcData("one"), grpcData("two")).readBytes();
        ...
        try {
            ...
            assertThat(responses.hasNext(), is(true));
            assertThat(responses.next(), is("one"));
            assertThat(responses.hasNext(), is(true));
            assertThat(responses.next(), is("two"));
            assertThat(responses.hasNext(), is(false));
        } finally {
            server.stop();
        }
    }
    ...
    @Test
    void serverStreamingCallCancelsWhenCoalescedMessagesExceedDemand() throws InterruptedException {
        byte[] response = BufferData.create(grpcData("one"), grpcData("two"), grpcData("three")).readBytes();
        WebServer server = responseServer(response);
        try {
            GrpcClient client = grpcClient(server);
            ClientCall<String, String> call = client.channel()
                    .newCall(stringDescriptor(MethodDescriptor.MethodType.SERVER_STREAMING), CallOptions.DEFAULT);
            ...
            assertThat(firstMessage.get(), is("one"));
    ...
    private static GrpcClient grpcClient(WebServer server) {
        return GrpcClient.create(builder -> builder
