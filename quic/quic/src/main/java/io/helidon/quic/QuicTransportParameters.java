 ...
 * and not thread safe.
 *
 * A parameter is considered absent if {@link #parameter(TransportParameterId)}
 * yields {@link Optional#empty()}. The parameter is present otherwise.
 * Parameters can be removed by calling {@link #clearParameter(TransportParameterId)}.
 * The methods {@link #booleanParameter(TransportParameterId)} and
 * {@link #intParameter(TransportParameterId)} allow easy access to
...
public final class QuicTransportParameters {
     ...
     */
    QuicTransportParameters(QuicTransportParameters params) {
        values = new EnumMap<>(params.values);
    }

    /**
     * Creates a new empty collection of QUIC transport parameters.
     *
     * @return empty transport-parameters collection
     */
    public static QuicTransportParameters create() {
        return new QuicTransportParameters();
    }

    /**
     * Creates a new collection of QUIC transport parameters initialized from the supplied collection.
     *
     * @param params source transport parameters
     * @return copied transport-parameters collection
     */
    public static QuicTransportParameters copyOf(QuicTransportParameters params) {
        return new QuicTransportParameters(params);
    }

    /**
     * Returns a version-information value for the specified versions.
     *
    ...
    public static VersionInformation buildVersionInformation(
        ...
        return VersionInformation.create(chosenVersion.versionNumber(), available);
    }
     ...
     * @see io.helidon.quic.QuicTLSEngine#remoteQuicTransportParametersConsumer(QuicTransportParametersConsumer)
     *        (ByteBuffer)
     * @see io.helidon.quic.QuicTransportParametersConsumer#accept(ByteBuffer)
     * @see #encode(ByteBuffer)
     * @see <a href="https://www.rfc-editor.org/rfc/rfc9000#section-18">
     *        RFC 9000, Section 18</a>
     */
    public static QuicTransportParameters decode(BufferData buffer)
            throws QuicTransportException {
        QuicTransportParameters parameters = new QuicTransportParameters();
        while (buffer.available() > 0) {
            long id = VariableLengthEncoder.decode(buffer);
            ParameterId pid = TransportParameterId.valueOf(id)
                    .orElse(null);
            String name = pid == null ? Long.toString(id) : pid.text();
            ...
            if (pid == null) {
                if (LOGGER.isLoggable(System.Logger.Level.DEBUG)) {
                    LOGGER.log(System.Logger.Level.DEBUG, "Ignoring unsupported transport parameter: " + name);
                }
                continue;
            }
            try {
     ...
     * The returned list contains at most one IPv4 address and/or one IPv6 address.
     *
     * @param id    the parameter id
     * @param value the value of the parameter
     * @return a list of {@link InetSocketAddress}
     * <p>Note: To obtain the list of addresses encoded in the
     *        {@link ParameterId#preferred_address} parameter, map its
     *        {@link #parameter(TransportParameterId) optional value} through this method.
     * @see ParameterId#preferred_address
     */
    public static List<InetSocketAddress> preferredAddress(
        ...
        System.arraycopy(value, IPV4_ADDR_OFFSET, ipv4, 0, IPV4_SIZE);
        byte[] ipv6 = new byte[IPV6_SIZE];
        System.arraycopy(value, IPV6_ADDR_OFFSET, ipv6, 0, IPV6_SIZE);
        InetSocketAddress ipv4addr = new InetSocketAddress(byAddress(id, ipv4),
                                                           readUnsignedShort(value, IPV4_PORT_OFFSET));
        InetSocketAddress ipv6addr = new InetSocketAddress(byAddress(id, ipv6),
                                                           readUnsignedShort(value, IPV6_PORT_OFFSET));
     ...
     * @see ParameterId#preferred_address
     */
    public static BufferData preferredConnectionIdData(byte[] preferredAddressValue) {
        Objects.requireNonNull(preferredAddressValue, "preferredAddressValue");
        int length = preferredConnectionIdLength(ParameterId.preferred_address,
                                                    preferredAddressValue);
        return BufferData.createReadOnly(preferredAddressValue, CID_OFFSET, length);
    }
     ...
     * @see ParameterId#preferred_address
     */
    public static byte[] preferredStatelessResetToken(byte[] preferredAddressValue) {
        Objects.requireNonNull(preferredAddressValue, "preferredAddressValue");
        int length = preferredConnectionIdLength(ParameterId.preferred_address,
                                                    preferredAddressValue);
        int offset = CID_OFFSET + length;
        byte[] statelessResetToken = new byte[TOKEN_SIZE];
        System.arraycopy(preferredAddressValue, offset, statelessResetToken, 0, TOKEN_SIZE);
        return statelessResetToken;
    }

    static QuicTransportParameters decode(ByteBuffer buffer)
            throws QuicTransportException {
        QuicTransportParameters parameters = new QuicTransportParameters();
    ...
    static ByteBuffer preferredConnectionId(byte[] preferredAddressValue) {
        Objects.requireNonNull(preferredAddressValue, "preferredAddressValue");
        int length = preferredConnectionIdLength(ParameterId.preferred_address,
                                                    preferredAddressValue);
    ...
    /**
     * Returns whether the given parameter is present.
     * <p>
     * This is equivalent to {@link #parameter(TransportParameterId)
     * parameter(id).isPresent()}, but avoids cloning the parameter value.
     *
     * @param id the parameter id
    ...
    /**
     * Returns the value of the given parameter, as a byte array, when present.
     *
     * @param id the parameter id
     * @return the value of the given parameter, as a byte array, when present
     */
    public Optional<byte[]> parameter(TransportParameterId id) {
        byte[] value = values.get((ParameterId) id);
        return Optional.ofNullable(value == null ? null : value.clone());
    }

    /**
     * Returns whether the value of the given parameter matches the given connection ID.
     *
    ...
    public boolean matches(TransportParameterId id, QuicConnectionId connectionId) {
        byte[] value = values.get((ParameterId) id);
        return connectionId.matches(BufferData.create(value));
    }
     ...
     */
    public void parameter(TransportParameterId id, byte[] value) {
        Objects.requireNonNull(value, "value");
    ...
    /**
     * Clears the value of the given parameter.
     *
     * @param id the parameter id
     */
    public void clearParameter(TransportParameterId id) {
        values.remove((ParameterId) id);
    }

    /**
     * Returns the value of the given parameter, as an unsigned int in the range {@code [0, 2^62 - 1]}.
     * If the parameter is not present its default value (as specified in the RFC) is returned.
     *
     * @param id the parameter id
     * @return the value of the given parameter, as an unsigned int in the range {@code [0, 2^62 - 1]}
     * @throws IllegalArgumentException if the value of the given parameter
     *                                 cannot be decoded as a variable length unsigned int
     */
    public long intParameter(TransportParameterId id) {
        return intParameter((ParameterId) id);
    }
     ...
     */
    public long intParameterOrDefault(TransportParameterId id, long defaultValue) {
        ...
        return switch (pid) {
            case max_idle_timeout, max_udp_payload_size, initial_max_data,
                ...
                if (value == null) {
                    ...
                    res = decodeVLIntFully(pid, BufferData.create(value));
                }
                yield res;
            }
            default -> throw new IllegalArgumentException(pid.text());
        };
    }
     ...
     */
    public void intParameter(TransportParameterId id, long value) {
        ParameterId pid = (ParameterId) id;
        switch (pid) {
        case max_idle_timeout, max_udp_payload_size, initial_max_data,
             initial_max_stream_data_bidi_local, initial_max_stream_data_bidi_remote,
            ...
            byte[] v = null;
            if (value >= 0) {
                int length = VariableLengthEncoder.encodedSize(value);
                if (length <= 0) {
                    throw new IllegalArgumentException("failed to encode " + value);
                }
     ...
     *                                 is not a boolean
     */
    public boolean booleanParameter(TransportParameterId id) {
        ParameterId pid = (ParameterId) id;
     ...
     * <p>
     * It is not possible to distinguish between a boolean parameter whose value
     * is absent and a parameter whose value is false. Both are represented by a
     * {@code null} value in the parameter array.
     *
     * @param id    the parameter id
     ...
     *                                 not a boolean
     */
    public void booleanParameter(TransportParameterId id, boolean value) {
        ParameterId pid = (ParameterId) id;
        if (pid != ParameterId.disable_active_migration) {
            throw new IllegalArgumentException(id.text());
        }
        putOrRemove(pid, value ? EMPTY_BYTES : null);
    }

    /**
     * Returns the value of the given parameter, as {@link VersionInformation} when present.
     *
     * @param id the parameter id
     * @return the value of the given parameter, as {@link VersionInformation} when present
     * @throws IllegalArgumentException if the value of the given parameter
     ...
     */
    public Optional<VersionInformation> versionInformationParameter(TransportParameterId id)
            throws QuicTransportException {
        ParameterId pid = (ParameterId) id;
        ...
        BufferData buffer = BufferData.create(val);
        int chosen = buffer.readInt32();
        if (chosen == 0) {
            throw new QuicTransportException(
                    "[version_information] Chosen Version = 0",
                    0, QuicTransportErrors.TRANSPORT_PARAMETER_ERROR);
        }
        int[] available = new int[buffer.available() / 4];
        for (int i = 0; i < available.length; i++) {
        ...
        return Optional.of(VersionInformation.create(chosen, available));
    }
     ...
     */
    public void versionInformationParameter(TransportParameterId id, VersionInformation value) {
        ParameterId pid = (ParameterId) id;
        if (pid != ParameterId.version_information) {
            throw new IllegalArgumentException(id.text());
        }
        BufferData buffer = BufferData.create(value.availableVersions.length * 4 + 4);
        buffer.writeInt32(value.chosenVersion);
        for (int available : value.availableVersions) {
            buffer.writeInt32(available);
        }
        values.put(pid, buffer.readBytes());
    }
    ...
    public void preferredAddressParameter(TransportParameterId id,
        ...
        if (!ipv4.isAnyLocalAddress()) {
            System.arraycopy(ipv4.getAddress(), 0, value, IPV4_ADDR_OFFSET, IPV4_SIZE);
        }
        writeUnsignedShort(value, IPV4_PORT_OFFSET, port4);
        if (!ipv6.isAnyLocalAddress()) {
            System.arraycopy(ipv6.getAddress(), 0, value, IPV6_ADDR_OFFSET, IPV6_SIZE);
        }
        writeUnsignedShort(value, IPV6_PORT_OFFSET, port6);
        value[CID_LEN_OFFSET] = (byte) cidlen;
        copy(connectionId, value, CID_OFFSET);
        copy(statelessToken, value, CID_OFFSET + cidlen);
        values.put(pid, value);
    }

    /**
     * Returns the size in bytes required to encode the parameter array.
     *
     * @return the size in bytes required to encode the parameter array
     */
    public int size() {
        int size = 0;
        for (var kv : values.entrySet()) {
            ...
            size += VariableLengthEncoder.encodedSize(i);
            size += VariableLengthEncoder.encodedSize(value.length);
            size += value.length;
        }
        return size;
    }
     ...
     *                                provided buffer
     * @see io.helidon.quic.QuicTLSEngine#localQuicTransportParameters(BufferData)
     * @see <a href="https://www.rfc-editor.org/rfc/rfc9000#section-18">
     *        RFC 9000, Section 18</a>
     * @see <a href="https://www.rfc-editor.org/rfc/rfc9001">
     *        RFC 9001 [QUIC-TLS]</a>
     */
    public int encode(BufferData buffer) {
        int start = buffer.available();
        for (var kv : values.entrySet()) {
            var i = kv.getKey().idx();
            var value = kv.getValue();
            if (value == null) {
        ...
        var written = buffer.available() - start;
        return written;
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder("Quic Transport Params[");
        for (var kv : values.entrySet()) {
            var param = kv.getKey();
            var value = kv.getValue();
            if (value != null) {
    ...
    /**
     * Returns a string representation including parameter names and decoded values.
     *
     * @return transport parameters with values
     */
    public String toStringWithValues() {
        ...
        for (var kv : values.entrySet()) {
            ...
            if (value != null) {
                ...
                sb.append(param);
                String valAsString = valueToString(param);
    ...
    void preferredAddressParameter(TransportParameterId id,
                                      Inet4Address ipv4, int port4,
                                      Inet6Address ipv6, int port6,
        ...
        writeUnsignedShort(value, IPV6_PORT_OFFSET, port6);
        value[CID_LEN_OFFSET] = (byte) cidlen;
        connectionId.get(connectionId.position(), value, CID_OFFSET, cidlen);
        statelessToken.get(statelessToken.position(), value, CID_OFFSET + cidlen, tklen);
        values.put(pid, value);
    }
    ...
    private static int preferredConnectionIdLength(TransportParameterId id, byte[] value) {
        if (value.length < MIN_PREF_ADDR_SIZE) {
            throw new IllegalArgumentException(id.text()
                                                       + ": not enough bytes in value; found " + value.length);
        }
    ...
    private static InetAddress byAddress(TransportParameterId id, byte[] address) {
        try {
    ...
    /**
     * Verifies that the {@code value} is acceptable for the {@code tpid}.
     *
    ...
    private static ParameterId checkParameterValue(TransportParameterId tpid, byte[] value) {
        ...
        if (value != null) {
            ...
            case preferred_address -> preferredConnectionIdLength(id, value);
            ...
            default -> {
                long intvalue;
                try {
                    intvalue = decodeVLIntFully(id, BufferData.create(value));
                ...
                case max_udp_payload_size -> {
                    ...
                    if (intvalue < 1200) {
                        throw new IllegalArgumentException(id.text()
                                                                   + ": value must be at least 1200; found "
                                                                   + intvalue);
                    }
                }
                ...
                default -> {
                    // Other integer parameters have no extra range validation.
                }
                }
            }
        }
    ...
    private static long decodeVLIntFully(ParameterId id, BufferData buffer) {
    ...
    private static void copy(BufferData source, byte[] target, int targetOffset) {
        for (int i = 0; i < source.available(); i++) {
            target[targetOffset + i] = (byte) source.get(i);
        }
    }

    private long intParameter(ParameterId pid) {
        return switch (pid) {
            case max_idle_timeout, max_udp_payload_size, initial_max_data,
                 initial_max_stream_data_bidi_local, initial_max_stream_data_bidi_remote,
                 initial_max_stream_data_uni, initial_max_streams_bidi,
    ...
    private void putOrRemove(ParameterId pid, byte[] value) {
        if (value != null) {
            values.put(pid, value);
        } else {
            values.remove(pid);
    ...
    private String valueToString(ParameterId parameterId) {
        try {
            return switch (parameterId) {
                ...
                case max_idle_timeout, max_udp_payload_size, initial_max_data,
                     ...
                     initial_max_stream_data_uni, initial_max_streams_bidi,
                     initial_max_streams_uni, ack_delay_exponent, max_ack_delay,
                     active_connection_id_limit -> String.valueOf(intParameter(parameterId));
                default -> '"' + HexFormat.of().formatHex(values.get(parameterId)) + '"';
            };
        } catch (RuntimeException e) {
    ...
    public enum ParameterId implements TransportParameterId {
         ...
         * }</pre></blockquote>
         */
        // No values are defined here, but reserved transport parameters are still ignored if received.
    ...
    public sealed interface TransportParameterId {
        /**
         * Returns the parameter ID corresponding to the given identifier.
         *
        ...
        /**
         * Returns the transport parameter name.
        ...
        /**
         * Returns the canonical transport parameter text.
         *
         * @return the canonical transport parameter text
         */
        String text();

        /**
         * Returns the transport parameter identifier.
         *
    ...
    /**
     * Version Information transport parameter value.
     */
    public static final class VersionInformation {
        private final int chosenVersion;
        private final int[] availableVersions;

        private VersionInformation(int chosenVersion, int[] availableVersions) {
            this.chosenVersion = chosenVersion;
            this.availableVersions = Objects.requireNonNull(availableVersions, "availableVersions").clone();
        }

        /**
         * Creates a new version information value.
         *
         * @param chosenVersion     negotiated version
         * @param availableVersions versions advertised as available
         * @return new version information value
         */
        public static VersionInformation create(int chosenVersion, int[] availableVersions) {
            return new VersionInformation(chosenVersion, availableVersions);
        }
         ...
         */
        public int chosenVersion() {
            return chosenVersion;
        }

        /**
         * Returns the advertised available versions.
         *
         * @return the advertised available versions
         */
        public int[] availableVersions() {
            return availableVersions.clone();
        }
        ...
        public boolean equals(Object obj) {
            ...
            if (!(obj instanceof VersionInformation other)) {
                return false;
            }
            return chosenVersion == other.chosenVersion
                    && Arrays.equals(availableVersions, other.availableVersions);
        }

        @Override
        public int hashCode() {
            return 31 * Integer.hashCode(chosenVersion) + Arrays.hashCode(availableVersions);
        }

        @Override
        public String toString() {
            return "VersionInformation[chosenVersion="
                    + chosenVersion
                    + ", availableVersions="
                    + Arrays.toString(availableVersions)
                    + ']';
        }
    }
}
