 ...
 * {@link BuffersReader#release()} or {@link BuffersReader#readAndRelease(long)} should
...
public abstract sealed class BuffersReader {
    ...
    /**
     * Returns the limit of the reader.
     *
     ...
     *                                 the limit
     */
    public abstract List<ByteBuffer> readAndRelease(long bytes);

    /**
     * Returns true if the reader has remaining bytes to the read.
     *
    ...
    /**
     * Returns the number of bytes that remain to read.
     *
    ...
    /**
     * Returns the cumulated amount of data that has been read in this
    ...
    /**
     * Returns true if this {@code BuffersReader} is empty.
     *
    ...
    public static final class SingleBufferReader extends BuffersReader {
        ...
        @Override
        public List<ByteBuffer> readAndRelease(long bytes) {
            return List.of(readAndReleaseBuffer(bytes));
        }
        ...
        /**
         * This method has the same semantics than {@link #readAndRelease(long)}
         * except that it avoids creating a list.
         *
         * @param bytes number of bytes to expose and release
         * @return a buffer containing the next {@code bytes}.
         */
        public ByteBuffer readAndReleaseBuffer(long bytes) {
            var released = single;
    ...
    public static final class ListBuffersReader extends BuffersReader {
         ...
         * @param buffer a byte buffer
         * @return this reader
         * <p>Note: This class doesn't make defensive copies of the provided buffers,
        ...
        public List<ByteBuffer> readAndRelease(long bytes) {
            ...
            while (bytes > 0) {
                ...
                if (remaining <= bytes) {
                    ...
                    current = new Buffer(buf);
                    buffers.set(nextIndex - 1, current);
                    readAndReleased += relased;
                    if (list != null) {
                        list.add(slice);
                    } else {
        ...
        public void position(long pos) {
            ...
            if (pos < start) {
                throw new IllegalArgumentException(pos + " < " + start);
            }
            if (pos == position) {
                return; // happy case!
            }
